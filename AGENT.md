...
**См. полный список в** `docs/REFACTORING_PLAN.md` → "ЛОЖНЫЕ УТВЕРЖДЕНИЯ"

### 3.5. 🛡️ Протокол Верификации Утверждений

> **ОБЯЗАТЕЛЬНО** перед любым предложением о рефакторинге или утверждением о состоянии кода.

**Шаги верификации:**

1. **Существование компонента:**
   ```bash
...
2. **Статус реализации:**
   - Прочитать файл целиком (не полагаться на память)
   - Проверить `docs/REFACTORING_PLAN.md` → "УЖЕ РЕАЛИЗОВАНО"

3. **Наличие тестов:**
   ```bash
   ls tests/architecture/
   grep -r "test_feature_name" tests/
   ```
...
| Фраза | Что проверить |
|-------|---------------|
| "Класс X должен иметь метод Y" | Существует ли класс X вообще? |
| "Нет реализации Z" | Grep по кодовой базе |
| "Нарушает паттерн W" | Прочитать код и понять контекст |
| "God object" | Посчитать строки, проверить делегирование |

**При расхождении:**
1. ❌ НЕ повторять ложное утверждение
2. ✅ Обновить `docs/REFACTORING_PLAN.md` → "ЛОЖНЫЕ УТВЕРЖДЕНИЯ"
3. ✅ Зафиксировать в коммите
