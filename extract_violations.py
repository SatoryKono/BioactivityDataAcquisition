...
def get_git_files(pattern):
    try:
        output = subprocess.check_output(['git', 'ls-files', pattern], stderr=subprocess.STDOUT)
        files = output.decode('utf-8').strip().split('\n')
        return [f for f in files if f and f.endswith('.py') or f.endswith('.yaml') or f.endswith('.md')]
    except Exception:
        return []
...
sectors = {
    ...
    'S7-Configs': {'name': 'Configs', 'patterns': ['configs/**/*.yaml']},
...
for sector_id, info in sectors.items():
    ...
    results[sector_id] = {
        ...
        'sample_issues': issues[:5] # top 5 real issues
    }
