...
## 2026-05-25 - [Schema-Driven Iteration Performance]
**Learning:** When transforming records (e.g. Bronze -> Silver), inputs often contain many unused fields. Iterating `schema.names` instead of `record.items()` is O(M) vs O(N) where M << N. Benchmarks showed ~4.8x speedup (0.13s vs 0.63s).
**Action:** Always prefer iterating the target schema when constructing records from a superset source.
