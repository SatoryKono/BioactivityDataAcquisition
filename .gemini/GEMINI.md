...
*   **`docs/00-project/agents/AGENT.md`**: Specialized instructions for AI agents.

## 8. Commit Convention
*   **Format:** Conventional Commits (`type(scope): description`)
*   **Types:** `feat`, `fix`, `docs`, `style`, `refactor`, `test`, `chore`.
*   **Scope:** Component affected (e.g., `auth`, `pipeline`, `adapter`).

## 9. Anti-Patterns to Avoid
*   ❌ Importing `infrastructure` in `domain` or `application`.
*   ❌ Hardcoding secrets or paths.
*   ❌ Using `print()` instead of `structlog`.
*   ❌ Returning sentinel values like `-1` or `"N/A"` (Use `None`).
