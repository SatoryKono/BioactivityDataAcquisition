...
test-confidence-contract:
	$(RUN) python -m scripts.engineering.qa check-vcr-replay-preflight --strict
	VCR_RECORD_MODE=none $(RUN) pytest tests/contract/ tests/unit/contracts/ -q -m "no_api or not network" --record-mode=none -p no:xdist
