...
1. `report.md` (человекочитаемый)
2. `metrics.json` (машинно-читаемый)

(См. шаблоны в основной инструкции)
```
...
Файл `flakiness-database.json` создаётся L1-оркестратором путём агрегации данных от всех L2/L3-агентов. (См. формат в исходных требованиях).
...
## Executive Summary
<2-3 предложения о состоянии тестирования проекта. Ключевые достижения и оставшиеся риски.>
...
## Режимы работы
*   **full_audit** (полный аудит): Выполнить все 5 фаз. Рекомендуется для первого запуска.
