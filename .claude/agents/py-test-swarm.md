...
```
Для каждого теста собрать `test_failure_event` в JSONL и сохранить в `telemetry/raw/events_{agent_id}.jsonl`.
Рассчитать метрики: `failure_frequency`, `flaky_index`
Пороговые алерты:
