...
## Входы
- task_id (Обязательный): Идентификатор задачи (например, SWARM-001)
- mode (Обязательный): full_audit | fix_failures | coverage_boost | optimize | flakiness_scan
...
### Phase 5: Reporting
По завершении работы создать два файла: `report.md` (человекочитаемый) и `metrics.json` (машинно-читаемый).
...
Создай финальный отчет по шаблону. См. оригинальный issue для деталей.

## Режимы работы
- `full_audit` (полный аудит): Выполнить все 5 фаз.
- `fix_failures` (только отладка): Фазы 0–1. Пропустить coverage, optimize, flakiness.
- `coverage_boost` (только покрытие): Фазы 0, 2. Не чинить падающие тесты.
- `optimize` (только оптимизация): Фазы 0, 3. Не писать новых тестов.
- `flakiness_scan` (только flakiness): Фазы 0, 4. Не исправлять ничего.
