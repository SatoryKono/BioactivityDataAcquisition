...
├── entities/
│   └── {provider}/
│       └── {entity}.yaml           # Unified entity config (pipeline + DQ + filter + schema)
├── composites/
...
|------|----------|----------|
| `configs/entities/chembl/activity.yaml` | created | Новый entity config |
