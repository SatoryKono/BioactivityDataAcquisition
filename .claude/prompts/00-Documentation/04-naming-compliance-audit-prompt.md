...
*BioETL Project — RULES.md v5.22 Alignment*
...
**Source of Truth (обязательный):** `docs/00-project/governance/02-naming-policy.md`.
Все выводы, классификации MUST/SHOULD/MAY и рекомендации в отчёте должны ссылаться на эту политику как первичный нормативный источник.
...
### 2.4. Количественная проверка по коду (обязательно)

Для отчёта необходимо посчитать метрики покрытия правил на реальном коде:

- Модули (module/file names)
- Классы (class names + suffix rules)
- Функции/методы (function names + prefix rules)
- Константы (UPPER_SNAKE_CASE)
- Проверки суффиксов классов и префиксов функций как отдельные категории

Минимум: абсолютные значения `Всего / Проверено / Нарушений / % compliance` по каждой категории.

Рекомендуемые команды:

```bash
# Модули
find src/bioetl -type f -name "*.py" | wc -l

# Классы
rg -n "^class\s+[A-Za-z_][A-Za-z0-9_]*" src/bioetl | wc -l

# Функции/методы
rg -n "^\s*(async\s+def|def)\s+[A-Za-z_][A-Za-z0-9_]*" src/bioetl | wc -l

# Константы (приблизительно: UPPER_SNAKE assignment)
rg -n "^[A-Z][A-Z0-9_]*\s*=\s*" src/bioetl | wc -l
```
...
| Правило | RFC 2119 (MUST/SHOULD/MAY) | Автоматизируемость | Текущий инструмент проверки |
|--------|------------------------------|--------------------|-----------------------------|
| Пример: PascalCase для классов | MUST | Полная | `scripts/naming_audit.py` |
| Пример: Семантический префикс функции соответствует назначению | SHOULD | Частичная | `manual review + naming_audit.py` |

Требования к D1:
- Явно помечать правила, которые **нельзя надёжно автоматизировать**.
- Такие правила переводить в статус **guideline** (SHOULD/MAY) и обозначать как **non-blocker**.
- Для автоматизируемых MUST-правил указывать blocking status в CI (blocker).

### D2. Количественная матрица проверок

Отдельная таблица с числами по категориям (модули, классы, функции, константы, префиксы, суффиксы).

### D3. Укороченная policy (1–2 страницы)

В финале приложить сокращённую версию naming-policy на 1–2 страницы:
- только обязательные нормы,
- чёткие примеры good/bad,
- список исключений.

### D4. Изменения конфигов линтеров

Отдельный раздел с предложенными/внесёнными правками в линтер-конфиги
(`ruff`, `mypy`, `pylint`, pre-commit hooks или эквиваленты), чтобы автоматизируемые правила проверялись в CI.
...
- [ ] Добавлена таблица `правило / MUST-SHOULD-MAY / автоматизируемость / текущий инструмент проверки`
- [ ] Выполнена количественная проверка: модули, классы, функции, константы, префиксы/суффиксы
- [ ] Неавтоматизируемые правила помечены как guideline (non-blocker)
- [ ] Построена карта зависимостей для breaking changes
- [ ] Определены фазы рефакторинга
- [ ] Подготовлена укороченная policy на 1–2 страницы
- [ ] Зафиксированы изменения конфигов линтеров
