...
Стадия должна корректно освобождать ресурсы, используемые в процессе выполнения. Метод `dispose()` вызывается оркестратором после завершения стадии для закрытия соединений, файлов, сетевых ресурсов и других объектов, требующих явного освобождения. Реализация должна гарантировать идемпотентность `dispose()` — повторные вызовы не должны приводить к ошибкам.
...
class StageABC(Generic[StageInT, StageOutT]):
    ...
    def dispose(self) -> None:
        """Освободить ресурсы, используемые стадией. Вызывается оркестратором после завершения стадии."""
        pass
```
