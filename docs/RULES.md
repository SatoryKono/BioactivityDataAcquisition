...
### 4.2. Соглашения об именовании (Naming Conventions)
- **HTTP Clients**: Методы внутри классов клиентов могут именоваться `fetch()` (контекст ясен из имени класса).
- **Standalone Functions**: Функции верхнего уровня, выполняющие сетевые запросы, **MUST** иметь префикс `fetch_` (например, `fetch_records`).

### 4.3. Политика Тестирования
