...
1. **BioETL Architecture Auditor — System Prompt** (пользовательские инструкции)
...
1. `print()` вместо структурного логирования
1. Публичный API без type annotations
1. Нарушение layer boundaries
...
1. Сформировать общий план решения через `pyPlanBot`.
1. Сохранить план в `reports/plans/<taskid>/`.
1. Каждому планируемому рефакторингу присвоить чёткий идентификатор (`RF-001`, `RF-002`, ...).
...
1. `docs/02-architecture/decisions/ADR-010-local-only-deployment.md`
1. `docs/02-architecture/decisions/ADR-007-*`
1. `docs/00-project/glossary.md`
