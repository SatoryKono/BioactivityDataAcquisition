...
| **Tests**                       | `tests.yml`         | `smoke-check`, `governance-preflight`, `config-schema-preflight`, `contract-confidence`, `flaky-telemetry`, `repo-backed-unit`, `test-fast`, `test-matrix`, `coverage-verify`, `duration-telemetry` | VCR cassettes, config validation, governance preflight, offline contract replay, repeated-run determinism telemetry, serial repo-backed contracts, fast unit feedback, full test matrix, scoped pytest/Hypothesis cache fingerprints, final combined 85% coverage gate, slow-test telemetry artifact |
