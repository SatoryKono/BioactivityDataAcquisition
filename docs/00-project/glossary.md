...
| **Runtime Pandera Support Validation** | Retired runtime validation seam for Python 3.14+ Pandera support. `bioetl.composition.bootstrap.runtime.pipeline.apply_runtime_compatibility_patches` remains as a no-op public hook; `bioetl.infrastructure.compat.pandera_compat` must not return without a new compatibility decision. Import-time monkeypatching is forbidden. См. [ADR-048](../02-architecture/decisions/ADR-048-domain-schema-boundary-and-runtime-pandera-compat.md). | `domain patching`, `import-time monkey patch in __init__` |
