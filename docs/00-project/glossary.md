...
| **Runtime Compatibility Patching (Pandera)** | Runtime-активация compatibility-патча Pandera через единственный sanctioned seam `bioetl.composition.bootstrap.runtime.pipeline.apply_runtime_compatibility_patches`; implementation ownership остаётся в infrastructure (`bioetl.infrastructure.compat.pandera_compat`). См. [ADR-048](../02-architecture/decisions/ADR-048-domain-schema-boundary-and-runtime-pandera-compat.md). | `domain patching`, `import-time monkey patch in __init__` |
