...
| **Domain Schema Contract Boundary** | Граница, в которой Pandera/Pandas используются как domain schema contracts: только `src/bioetl/domain/schemas/` и `src/bioetl/domain/contracts/` по [ADR-048](../02-architecture/decisions/ADR-048-domain-schema-boundary-and-runtime-pandera-compat.md); не включает runtime bootstrap и не является infrastructure adapter mapping. | `schema layer` (без уточнения boundary), `infrastructure schema` |
| **Runtime Pandera Support Validation** | Explicit runtime validation of Python 3.14+ Pandera support through the retained seam `bioetl.composition.bootstrap.runtime.pipeline.apply_runtime_compatibility_patches`, delegating to `bioetl.infrastructure.compat.pandera_compat.validate_supported_pandera_runtime`. Unsupported matrices fail fast; import-time monkeypatching is forbidden. См. [ADR-048](../02-architecture/decisions/ADR-048-domain-schema-boundary-and-runtime-pandera-compat.md). | `domain patching`, `import-time monkey patch in __init__` |
