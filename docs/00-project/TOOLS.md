...
| Директория   | Назначение              | Критерий                                | Статус          |
| ------------ | ----------------------- | --------------------------------------- | --------------- |
| `src/tools/` | Утилиты проекта         | **Импортирует** `bioetl` модули         | **Deprecated**  |
| `scripts/`   | CI/операционные скрипты | **Standalone**, НЕ импортирует `bioetl` | **Canonical**   |

**Примечание:** Директория `src/tools/` была упразднена в рамках реорганизации скриптов (2026-08). Все утилиты проекта теперь находятся в `scripts/` с соответствующей категоризацией по критерию импорта `bioetl`.
