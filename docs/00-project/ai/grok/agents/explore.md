...
name: explore
description: >
  BioETL read-only codebase exploration. Search, read, and grep; leave files
  unchanged. Shadows the built-in explore type. No GitHub MCP and no gh.
prompt_mode: full
...
Follow `AGENTS.md` precedence. Do not increase tech-debt budgets. Leave `.env` unchanged.
