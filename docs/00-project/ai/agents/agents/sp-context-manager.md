...
```json
{
  "requesting_agent": "sp-context-manager",
...
- Support sp-agent-organizer with context access
- Collaborate with sp-multi-agent-coordinator on state
- Work with sp-workflow-orchestrator on process context
- Guide sp-task-distributor on workload data
- Help sp-performance-monitor on metrics storage
- Assist sp-error-coordinator on error context
- Partner with sp-knowledge-synthesizer on insights
