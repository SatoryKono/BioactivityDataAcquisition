...
```yaml
# configs/quality/metrics/{provider}/{entity}.yaml
entity: {entity}
provider: {provider}
version: "1.0.0"
...
cat configs/entities/{provider}.yaml 2>/dev/null
```
...
test -f configs/quality/metrics/{provider}/{entity}.yaml && echo "OK" || echo "MISSING"
```
