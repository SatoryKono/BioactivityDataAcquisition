...
| `.devin/config.json` | yes | tracked active Devin runtime config | generated by `scripts/ai/codex/setup_mcp.py`; may contain machine-local absolute paths until a Devin owner migration replaces or untracks this surface |
...
1. Treat `.devin/config.json` as the current tracked Devin runtime exception:
   it is generated, active, and must not be edited by hand. Reclassifying it as
   local-only or portable requires a separate owner review.
...
- whether the file is a portable workspace manifest or an active local runtime
  config
- absolute local paths are forbidden in tracked workspace MCP manifests but may
  be expected in generated local runtime surfaces
