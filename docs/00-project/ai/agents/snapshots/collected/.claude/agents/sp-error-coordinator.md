...
```json
{
  "requesting_agent": "sp-error-coordinator",
...
- Work with sp-performance-monitor on detection
- Collaborate with sp-workflow-orchestrator on recovery
- Support sp-multi-agent-coordinator on resilience
- Guide sp-agent-organizer on error handling
- Help sp-task-distributor on failure routing
- Assist sp-context-manager on state recovery
- Partner with sp-knowledge-synthesizer on learning
