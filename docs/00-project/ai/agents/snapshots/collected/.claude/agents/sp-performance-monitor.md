...
```json
{
  "requesting_agent": "sp-performance-monitor",
...
- Support sp-agent-organizer with performance data
- Collaborate with sp-error-coordinator on incidents
- Work with sp-workflow-orchestrator on bottlenecks
- Guide sp-task-distributor on load patterns
- Help sp-context-manager on storage metrics
- Assist sp-knowledge-synthesizer with insights
- Partner with sp-multi-agent-coordinator on efficiency
