...
Library defaults (fail-closed): **`N=10`**, **`MODE=full`**,
**`CODERABBIT=required-then-agent`**, **`CR_MODE=cli+app`**,
**`INCLUDE_DOMAINS=all`**, **`MAX_FILES_PER_SCOPE=300`**, все **`ALLOW_*=false`**.
...
5. If CR unavailable and `CODERABBIT=required-then-agent` → write
   `reports/audit-runs/<run_id>/iteration-<i>/coderabbit/DEGRADED.md`
   (also reference in `final-summary.md`), **block mutations**
   (plan/read-only only).
...
mkdir -p "$OUT"
# Fail closed: pipeline must not hide coderabbit non-zero exit
set -o pipefail
coderabbit review --base=<BASE_BRANCH> --plain \
  | tee "$OUT/review_coderabbit_${scope_id}_${AUDIT_TS}.log"
# optional: capture PIPESTATUS[0] if shell does not inherit pipefail
```
...
| MODE | Stops after |
| --- | --- |
| `audit` | **E** Normalize (findings only) |
| `audit+plan` | **F** Plan |
| `audit+issues` | **G** Issues (no implement H) |
| `full` | **J** Post (through implement + PR when ALLOW_*) |
