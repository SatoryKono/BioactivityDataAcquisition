...
2) Разрешены только операции чтения/анализа.
3) Никаких авто-фиксингов, форматирования и удаления файлов.
4) Все выводы подтверждай ссылками на конкретные пути файлов.
...
5) План консолидации и очистки (по этапам):
   ...
   - критерии Done.
6) Отдельный раздел «Кандидаты на удаление»:
   Script | Why candidate | Last known usage evidence | Safe removal preconditions
7) Отдельный раздел «Кандидаты на консолидацию»:
   Group | Scripts | Proposed canonical script | Compatibility strategy
8) Финальный roadmap на 2–4 итерации с приоритетами и ожидаемым эффектом.
