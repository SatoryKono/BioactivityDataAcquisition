...
## Config Mode Contracts

- Critical mode/enum fields in YAML schemas **MUST** be required with documented allowed values (no silent implicit defaults). Canonical list: `run_mode`, `environment`, `run_type`, `data_layer`, Silver/Gold write modes — see `02-code-style.mdc` §Configuration
- Breaking YAML key renames/removals/semantics **MUST** ship migration notes with the change
