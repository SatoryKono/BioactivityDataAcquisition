...
class LinkCheckerPlugin(BasePlugin):
    ...
    def on_post_build(self, config, **kwargs):
        ...
        if self.config["fail_on_error"] and self.broken_links > 0:
            raise SystemExit(f"Link checking failed: {self.broken_links} broken links found")
    ...
    def _check_links_in_file(self, html_file: str, base_url: str, relative_path: str) -> List[Dict]:
    ...
    def _check_link(self, href: str, text: str, source_file: str, source_path: str) -> Optional[Dict]:
        ...
        is_internal = href.startswith("/") or href.startswith("./") or href.startswith("../")
        ...
        try:
            if is_internal:
                ...
                if clean_href.endswith(".html"):
                    ...
                    target_path = os.path.join(os.path.dirname(source_path), clean_href.lstrip("/"))
    ...
    def _generate_json_report(self) -> Dict:
        ...
        return {
            ...
            "summary": {
                ...
                "internal_links": sum(1 for link in self.link_results if link.get("is_internal")),
                "external_links": sum(1 for link in self.link_results if not link.get("is_internal"))
            },
    ...
    def _generate_badge(self) -> str:
        ...
        health_score = round((self.valid_links / self.links_checked) * 100) if self.links_checked > 0 else 0
