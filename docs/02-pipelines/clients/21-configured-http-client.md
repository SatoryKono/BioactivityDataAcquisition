...
Клиент наследуется от `BaseClient` и предоставляет базовую реализацию для всех настроенных HTTP-клиентов.
...
- **RequestsBackend**: реализация бэкенда на основе requests (см. `docs/02-pipelines/chembl/16-requests-backend.md`)
- **ChemblClient**: использует ConfiguredHttpClient (см. `docs/02-pipelines/chembl/14-chembl-client.md`)
