...
`ActivityWriter` — стадия Load/Write для пайплайна ChEMBL Activity. Отвечает за сохранение обработанных данных на диск и генерацию QC-отчётов. Использует `UnifiedOutputWriter` для атомарной записи данных в различных форматах (CSV/Parquet) и создания артефактов (метаданные, манифест, QC-отчёты).
