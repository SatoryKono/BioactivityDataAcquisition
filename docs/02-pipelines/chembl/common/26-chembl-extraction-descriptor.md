# 26 ChEMBL Extraction Descriptor
...
- **DataclassExtractionStrategy**: стратегия извлечения через dataclass (см. `docs/02-pipelines/chembl/common/25-dataclass-extraction-strategy.md`)
- **BatchPlan**: план батчирования запросов
- **ChemblCommonPipeline**: использует дескриптор для извлечения данных (см. `docs/02-pipelines/chembl/common/18-chembl-common-pipeline.md`)
