...
Выполняет единичный HTTP-запрос (GET/POST) для ресурса и возвращает один результат (JSON).
...
Выполняет запрос(ы) и итеративно возвращает записи (для ChEMBL объединяет результаты, напр. для списка *activities*) (в упрощенной реализации может вернуть первую страницу).
...
Возвращает итератор по страницам ответа (заглушка в простой реализации).
