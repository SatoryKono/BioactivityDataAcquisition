...
## Запрет корневых папок

- Создание новых папок в корне репозитория запрещено.
- Все утилиты и вспомогательные скрипты размещаются в `src/tools/`.
- В корне допускаются только установленные топ-уровневые каталоги: `src/`, `tests/`, `docs/`, `configs/`, `data/`, `.github/`, `.cursor/`, `.trae/`, `.windsurf/`, `qc/`.
- Временные отчёты пишутся в `reports/` (если включено), и не коммитятся (см. `.gitignore`).
