...
- **Валидация после рефакторинга**: Чеклист из 8 пунктов (тесты, mypy, импорты, документация, экспорты, deprecation, CHANGELOG, PR description). Рефакторинг считается завершённым только после прохождения всего чеклиста.

## 8. Проверка покрытия тестами (Test Coverage)
...
- **Действия**:
  1. Проверить существующие тесты: `ls tests/domain/test_X.py`
  2. Запустить coverage: `pytest tests/ --cov=bioetl.domain.X --cov-report=term-missing`
  3. Создать тесты для непокрытых путей (Happy path, Edge cases, Error cases).
  4. Убедиться что тесты проходят: `pytest tests/domain/test_X.py -v`
- **Требования**: Coverage > 80% перед началом; все тесты green; тесты проверяют поведение, не реализацию.