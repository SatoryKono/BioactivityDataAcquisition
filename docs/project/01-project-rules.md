...
2.4.4. Добавление или изменение пайплайнов должно проходить ревью с использованием чек-листа `docs/templates/pipeline-review-checklist.md`.
...
8.1.4. Каноническая модель конфигурации провайдера — `BaseProviderConfig` в модуле `bioetl.domain.configs.pipeline`. Все конфигурации провайдера (например, `ChemblSourceConfig`, `DummyProviderConfig`) должны наследоваться от неё; определять альтернативные базовые классы конфигурации провайдера в других модулях запрещено.
8.1.5. Каноническая модель конфигурации нормализации — `NormalizationConfig` в модуле `bioetl.domain.configs.normalization`. Доступ к настройкам нормализации в рантайме должен идти через `PipelineConfig.quality.normalization` и протокол `NormalizationConfigProviderProtocol`.
8.1.6. Любой новый код, работающий с конфигурациями провайдера или нормализации, должен импортировать соответствующие модели из `bioetl.domain.configs` (или из `bioetl.infrastructure.config.models` для инфраструктурного слоя) и не должен вводить собственные DTO/датаклассы с пересекающимся набором полей.
...
## 10. Рефакторинг модулей

### 10.1. Подготовка к рефакторингу

10.1.1. Перед началом рефакторинга модуля X необходимо составить карту зависимостей.

10.1.2. Обязательные шаги анализа:

1. **Найти все импорты модуля:**
   ```bash
   grep -r "from bioetl.domain.X import" src/
   grep -r "import bioetl.domain.X" src/
   grep -r "from bioetl.application.X import" src/
   grep -r "from bioetl.infrastructure.X import" src/
   ```
...
4. **Проверить re-exports в __init__.py:**
   ...
   grep -r "X" src/bioetl/application/__init__.py
   grep -r "X" src/bioetl/infrastructure/__init__.py
   ```

### 10.2. Формат вывода карты зависимостей

10.2.1. Результат анализа должен включать:

- Список файлов, импортирующих модуль (с указанием конкретных классов/функций)
- Список файлов, использующих конкретные классы/функции из модуля
- Список тестов, требующих обновления
- Рекомендации по порядку миграции (от наименее зависимых к наиболее зависимым)

### 10.3. Правила рефакторинга

10.3.1. Рефакторинг без предварительной карты зависимостей запрещён.

10.3.2. Тесты должны обновляться до изменения реализации.

10.3.3. Во время переходного периода должна поддерживаться обратная совместимость.

10.3.4. Все breaking changes документируются в `CHANGELOG.md`.

10.3.5. Изменения в публичных API требуют обновления документации в `docs/`.

### 10.4. Проверка покрытия тестами перед рефакторингом

10.4.1. Перед рефакторингом модуля X необходимо убедиться в наличии тестов.

10.4.2. Обязательные шаги проверки:
...
### 10.5. Требования к покрытию тестами

10.5.1. Coverage должен быть > 80% перед началом рефакторинга.

10.5.2. Все тесты должны быть green перед началом рефакторинга.

10.5.3. Тесты должны проверять поведение, а не реализацию.

10.5.4. Рефакторинг без достаточного покрытия тестами запрещён.

### 10.6. Валидация после рефакторинга

10.6.1. После завершения рефакторинга модуля X необходимо валидировать корректность изменений.

10.6.2. Обязательный чеклист валидации:
...
2. **Mypy без ошибок:**
   ```bash
   mypy src/bioetl/domain/ --strict
   ```

3. **Нет circular imports:**
   ```bash
   python -c "from bioetl.domain import *; print('OK')"
   ```

4. **Документация обновлена:**
   - Проверить актуальность документации в `docs/`
   - Обновить примеры использования, если изменился API

5. **`__init__.py` экспорты актуальны:**
   - Проверить все re-exports в `__init__.py` модуля
   - Убедиться, что публичный API доступен через правильные импорты

6. **Deprecation warnings добавлены где нужно:**
   - Для устаревших методов/классов добавить `@deprecated` или `warnings.warn()`
   - Указать версию удаления и альтернативу

7. **CHANGELOG обновлён:**
   - Зафиксировать все изменения в `CHANGELOG.md`
   - Указать breaking changes, если есть

8. **PR description содержит breaking changes:**
   - Если есть breaking changes, они должны быть явно указаны в описании PR
   - Предоставить инструкции по миграции

10.6.3. Дополнительные команды для проверки:

```bash
# Lint
ruff check src/bioetl/domain/

# Проверка импортов
python -c "import bioetl.domain"
```

10.6.4. Рефакторинг считается завершённым только после прохождения всего чеклиста валидации.