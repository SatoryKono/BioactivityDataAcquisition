...
8. Диаграммы:
   ...
   - Соблюдать `docs/architecture/diagrams/00-diagramming-policy.md`: хранить первичный текстовый формат (Mermaid/PlantUML), один файл — одна диаграмма, обновлять Class/Sequence/Flowchart при любых архитектурных или pipeline-изменениях.
...
9. Код-стиль:
   ...
   - Минимальный порог покрытия `pytest --cov=src/bioetl` — 85%; падение ниже блокирует PR.

10. Рефакторинг модулей:

   - Перед рефакторингом модуля X обязательна карта зависимостей:
     - Найти все импорты: `grep -r "from bioetl.domain.X import" src/` и аналогично для других слоёв
     - Найти все использования: `grep -r "X\." src/ --include="*.py"`
     - Найти упоминания в тестах: `grep -r "X" tests/ --include="*.py"`
     - Проверить re-exports в `__init__.py`
   - Формат вывода: список файлов-импортёров, список файлов-пользователей, список тестов, рекомендации по порядку миграции
   - Рефакторинг без карты зависимостей запрещён; тесты обновляются до изменения реализации
   - Проверка покрытия тестами перед рефакторингом:
     - Проверить существующие тесты: `ls tests/domain/test_X.py`
     - Запустить coverage: `pytest tests/ --cov=bioetl.domain.X --cov-report=term-missing`
...
- Загружать большие объёмы данных без контрольных сумм и sidecar.
 - Коммитить секреты/ключи, выводить их в логах.
- Рефакторить модули без предварительной карты зависимостей.
...
- [ ] Все внешние вызовы через унифицированные клиенты; троттлинг/ретраи/CB настроены.
