...
*Синхронизировано с RULES.md v5.8 (2025-12-29)*
...
### 3.6. Bronze Lifecycle & Delta Maintenance

- Bronze формат версионируется в пути (`bronze/{format_version}/...`). Изменение формата требует новой ветки пути (
  `/v2/`). In-place миграции запрещены.
- Delta Lake обслуживание: еженедельный запуск `VACUUM` с `retention_period=7 days` (обязательно).
- Forensic Retention: по умолчанию 7 дней; для критичных таблиц допускается до 30 дней через конфиг
  `forensic_retention: true`.

### 3.7. Стратегия партиционирования
...
- Hard limit: 50,000 партиций → fail. Запрещены ключи с высокой кардинальностью (UUID/Hash/Free-text).
