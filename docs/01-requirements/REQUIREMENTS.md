...
- **Уровень**: MUST
- **Описание**: Log Schema содержит обязательные поля: `timestamp`, `level`, `run_id`, `pipeline`, `stage`
...
- **Уровень**: MUST
- **Описание**: Валидация DataFrame-контрактов выполняется через Pandera строго в boundary, определённой [ADR-048](../02-architecture/decisions/ADR-048-domain-schema-boundary-and-runtime-pandera-compat.md): domain schema/contracts описываются в `src/bioetl/domain/schemas/` и `src/bioetl/domain/contracts/`, а `bioetl.composition.bootstrap.runtime.pipeline.apply_runtime_compatibility_patches` сохранён как явный no-op compatibility seam после удаления Pandera-specific runtime shim. Делегирование в `bioetl.infrastructure.compat.pandera_compat` или восстановление runtime Pandera guard запрещены без нового принятого ADR.
...
- **1.9** (2026-05-26, updated 2026-06-22, synced 2026-07-06): ADR-048 alignment. Зафиксирована граница Domain schema boundary: Pandera/Pandas разрешены только в `domain/schemas` + `domain/contracts`; runtime Pandera monkeypatching retired, а seam `apply_runtime_compatibility_patches` в composition/bootstrap сохранён как no-op compatibility seam без делегирования в infrastructure Pandera shim. Обновлены формулировки REQ-ARCH-003 и REQ-STACK-003, добавлены прямые cross-reference на ADR-048.
