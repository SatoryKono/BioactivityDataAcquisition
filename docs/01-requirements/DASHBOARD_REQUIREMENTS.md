...
| First-window no-scroll (`DASH-FIT-004` all first-window panels) | `tests/integration/test_dashboard_first_window_noscroll.py` — required on every code, test, or documentation change (workflow `dashboard-first-window-noscroll.yml` is not path-filtered; also Tests semantic gate + pre-push) |
