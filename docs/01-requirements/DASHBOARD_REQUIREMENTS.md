...
| `DASH-FIT-001` | The always-visible root stack (non-row panels + collapsed `row` headers) MUST have `max(y+h) <= VIEWPORT_ROWS` (render-calibrated, ~16–18 at `1366×768`). | blocked (render calibration + Incident relayout) |
...
| `DASH-COPY-003` | Every non-row, non-`text`, non-shell content-panel title MUST start with a canonical action verb (design-system §3.1); parsing is colon-tolerant. Until the pending-verb decision, titles MUST use a verb from the canonical or pending set. | advisory (unknown verbs blocked; canonical set not yet exclusive) |
...
| `bioetl-run-explorer-v1` | `Inspect Run Identity` (`9402`) | `Browse Recent Runs` (`3010`) is the empty-selection utility, not the answer |
...
| Geometry & purpose regression locks (`DASH-LAYOUT-003/004`, `DASH-FIT-002/003`, `DASH-COPY-004/005/006/007`, `DASH-PERF-003`; `DASH-COPY-003` advisory) | `tests/integration/test_dashboard_geometry_and_purpose_contracts.py` + [`layout-budgets.yaml`](../03-guides/dashboards/contracts/layout-budgets.yaml) |
| Geometry & purpose — blocked (`DASH-FIT-001` no-scroll) | not enforced until `VIEWPORT_ROWS` is render-calibrated and Incident always-visible stack is re-laid out |
