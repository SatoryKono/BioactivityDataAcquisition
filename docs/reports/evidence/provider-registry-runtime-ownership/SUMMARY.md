...
2. That seam is now directly exercised by unit tests and protected by an
   architecture ratchet.
3. Explicit `ProviderRegistry` instance threading is clearly beneficial in
   datasource and nearby factory seams where provider lookup is naturally local.
4. Runtime/bootstrap entrypoints still look like owners of pipeline-registry
