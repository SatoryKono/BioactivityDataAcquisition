...
| EV-governance-signals-application-core-remains-the-primary-structural-pressure-zone | `application/core` now has 8 duplication clusters, 14 files at or above 250 LOC, and max internal fan-in 15; it remains the primary tracked family and is governed by non-growth budgets.                                                                                                            | 0.96       |
| EV-governance-signals-bootstrap-runtime-pressure-is-contained                       | `composition/bootstrap/runtime` now has 8 duplication clusters with bounded non-regression caps for file-growth and family-internal fan-in, so its posture is controlled but still cleanup-bearing.                                                                                                  | 0.93       |
| EV-governance-signals-pipeline-factories-show-localized-duplication                 | `composition/factories/pipeline` now has 2 duplication clusters and bounded non-regression caps for file-growth and family-internal fan-in after runner assembly helper consolidation.                                                                                                               | 0.95       |
...
| EV-governance-signals-rf023-first-downward-move-landed                              | The latest reviewed `2026-05-20` snapshot expands the tracked family set to five and records 51 total duplication clusters; governance now uses these as non-growth budgets rather than stale zero claims.                                                                                           | 0.98       |
...
## RF-023 hotspot family baseline and updated trend check (2026-05-20)

| Family                           | Duplication clusters | Files >=250 LOC | Helper function ratio | Max fan-in | Owner                  | Expected action                                                                           | Trend                                                                                                                                             |
| -------------------------------- | -------------------: | --------------: | --------------------: | ---------: | ---------------------- | ----------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| `application/core`               |                    8 |              14 |                 0.374 |         15 | `@bioetl-architecture` | Active non-growth ratchet; cap duplication, file-growth, and fan-in at the reviewed baseline. | Refreshed from stale zero claim to current `R0801` baseline; zero-duplication is now a future ratchet target. |
| `composition/bootstrap/runtime`  |                    8 |               5 |                 0.347 |          7 | `@bioetl-platform`     | Active non-growth ratchet; cap duplication, file-growth, and fan-in at the reviewed baseline. | Refreshed from stale zero claim to current `R0801` baseline; file-growth stays below budget while fan-in is at budget. |
| `composition/factories/pipeline` |                    2 |               4 |                 0.431 |          4 | `@bioetl-platform`     | Active non-growth ratchet; cap duplication, file-growth, and fan-in at the reviewed baseline. | Runner assembly helper consolidation reduced the local pipeline-factory hotspot, but the family is not zero-clean. |
| `application/services/control_plane` |                20 |              21 |                 0.504 |          6 | `@bioetl-platform`     | Reviewed baseline; cap duplication, file-growth, and fan-in while diagnostics debt is paid down. | Diagnostics helper extraction moved this family from 23 to 20 duplicate clusters. |
...
- The refreshed RF-023 layer shows five tracked families with explicit non-growth budgets rather than stale zero-duplication claims. `application/core` still remains the primary watch zone because it is at both file-growth and fan-in budget.
...
- Trend capture is now operationalized through an append-only hotspot history artifact, and the latest reviewed comparison point totals `51` duplication clusters across five tracked families.
