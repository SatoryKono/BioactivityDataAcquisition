# Evidence Collection Complete: governance-signals
...
| EV-governance-signals-composition-duplication-snapshot-has-28-r0801-occurrences | An ad hoc duplication scan reports 28 `R0801` occurrences in `composition`. | 0.88 |
...
- The size ratchet is stricter than the raw tree inventory: the enforceable file-size exemption budget is zero, but the codebase still has a broad size tail (`95` files `>10 KB`, `17` files `>350 LOC`).
