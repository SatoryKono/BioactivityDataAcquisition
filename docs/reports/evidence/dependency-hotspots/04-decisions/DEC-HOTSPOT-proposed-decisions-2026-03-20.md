...
2. Treat size-only hotspots as phase-two backlog items rather than ignoring them.
3. Start the first decomposition wave in `src/bioetl/infrastructure/adapters`.
4. Use a two-track prioritization model: dense-file hotspots and dependency-pressure seams stay visible as separate signals.
...
2. Accept `DEC-HOTSPOT-003` next to define the first package execution wave.
3. Accept `DEC-HOTSPOT-002` so the phase-two inventory is not silently dropped.
...
2. Keep a separate phase-two ledger for size-only hotspots such as `silver_publications.py`, `runner.py`, and `composition/factories/pipeline/configs.py`.
3. Annotate backlog items with dependency-pressure context, especially where CLI/composite seams are involved.
4. Keep hotspot refactors wave-based and package-aware instead of treating the 17 overlap files as one undifferentiated queue.
