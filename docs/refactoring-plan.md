...
| "TTL по умолчанию 3600s не соответствует требованиям" | Фактический TTL = `heartbeat_interval * 3` = 60s по умолчанию (heartbeat=20s). `LockContext.is_valid(ttl_seconds=3600)` — backward-compat check, не фактический TTL. | `domain/config.py:291-293` (`effective_lock_ttl`) |
...
| "MemoryLock TTL=90s, heartbeat=30s нужно применить (OPS-001 action_plan)" | **ЛОЖНОЕ**: MemoryLock НЕ хардкодит значения — принимает TTL как параметр `acquire(ttl=...)`. RuntimeConfig определяет defaults: TTL=60s, heartbeat=20s. | `memory_lock.py:111-128` (верификация 2025-12-31) |
