...
> Curated published reference of sanctioned public seams (not an autogenerated dump). Layer overview: [Infrastructure Layer](../../02-architecture/03-infrastructure-layer.md)
...
| ------------------------------ | ------------------------------------------- | ------------------------------------------------------------- |
| `infrastructure.adapters`      | HTTP client adapters for external APIs      | `CachedBronzeDataSource`, `wrap_with_resilience`; provider adapters live in subpackages |
| `infrastructure.storage`       | Medallion layer writers (Delta Lake)        | `atomic_write`, `StorageError`; Bronze/Silver/Gold writers live in owner submodules |
...
| `infrastructure.checkpoint`    | Checkpoint persistence                      | `LocalCheckpointAdapter`                                      |
| `infrastructure.validation`    | Pandera schema validators                   | `PanderaSilverValidator`, `PanderaGoldValidator`              |
| `infrastructure.quarantine`    | Quarantine storage                          | `UnifiedQuarantineAdapter`                                    |
...
| `infrastructure.adr`           | ADR document management                     | No active package-root facade; import defining submodules directly |
...
| `infrastructure.quality`       | Technical debt tracking                     | `evaluate_debt_scorecard`, `build_exemption_inventory`        |
| `infrastructure.time`          | Time utilities                              | `SystemClock`                                                 |
