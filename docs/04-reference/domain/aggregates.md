...
| --- | --- | --- |
| `Batch` | `src/bioetl/domain/aggregates/_batch_aggregate.py` | Write lifecycle only; storage adapters execute outside the aggregate. |
| `PipelineRun` | `src/bioetl/domain/aggregates/pipeline_run.py` | Stage evidence is part of the aggregate model, not only logging. |
