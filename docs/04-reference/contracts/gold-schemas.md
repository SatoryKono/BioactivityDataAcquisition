...
## SemVer Policy for Gold Contracts

Contract versioning follows semantic versioning and is evaluated on the exported JSON fields (`name`, `json_type`, `nullable`, `description`) plus constraint semantics.

- **MAJOR**
  - Field rename or field removal.
  - Nullable narrowing: `true → false`.
  - Type narrowing/incompatible type change.
- **MINOR**
  - Adding a new nullable field.
  - Description-only updates.
  - Constraint widening that remains backward compatible.
- **PATCH**
  - Non-semantic metadata fixes.

### Changelog metadata in generated contracts

Each generated `docs/04-reference/contracts/gold/*_v1.0.json` includes:

- `changelog.generated_at`
- `changelog.summary.breaking`
- `changelog.summary.non_breaking`
- `changelog.changes[]` entries with:
  - `field`
  - `change`
  - `breaking` (`true` or `false`)
  - `details`

This metadata classifies each change as **breaking** or **non_breaking** for downstream release planning.
