...
class FilterableDataSourcePort(DataSourcePort, Protocol):
    ...
    async def fetch_filtered(
        self, filters: list[FilterSpec], limit: int | None = None
    ) -> list[dict]:
        """Fetch records with server-side filtering."""
        ...

    async def fetch_multi_filtered(
        self, filter_groups: list[list[FilterSpec]], limit: int | None = None
    ) -> list[dict]:
...
1. **Filter Specification**: Uses `FilterSpec` objects to define filter criteria:
...
1. **Performance**: Server-side filtering is preferred for performance, but client-side fallback ensures functionality.
