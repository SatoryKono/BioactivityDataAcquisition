...
class NormalizerFactory:
    ...
    def create_normalizer(self, field_config: FieldConfig) -> NormalizationStrategy:
        ...
        if not strategy_class:
            raise ValueError(
                f"Unknown normalization type: {field_config.normalization_type}"
            )
...
class PipelineOrchestrator:
    ...
    def run(
        self, config: Dict[str, Any], output_path: str, streaming: bool = False
    ) -> Dict[str, Any]:
        """
