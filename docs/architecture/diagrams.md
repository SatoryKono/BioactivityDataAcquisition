...
---
graph TB
    subgraph Interfaces ["bioetl.interfaces (Driving Layers)"]
        CLI[CLI Application]
        Server[REST API Server]
        MQ[MQ Consumer]
    end

    subgraph Application ["bioetl.application (Orchestration)"]
        Pipelines[Pipelines]
        AppContainer[DI Container]
        Hooks[Hooks & Middleware]
    end

    subgraph Domain ["bioetl.domain (Business Logic)"]
        SchemaDef[Pandera Schemas]
        Models[Domain Models]
        Contracts[Service Contracts]
        Logic[Transform Logic]
    end

    subgraph Infrastructure ["bioetl.infrastructure (Driven Adapters)"]
        APIClients[API Clients]
        Persistence[File Writers]
        Logging[Unified Logging]
        ConfigImpl[Config & Secrets]
    end
