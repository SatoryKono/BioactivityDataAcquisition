...
- `scripts/qa/generate_hotspot_degradation_report.py` (`python -m scripts.qa report-hotspots`, rolling-window degradation report)
...
python -m scripts.qa calibrate-hotspots \
...
python -m scripts.qa report-hotspots \
  --observations /tmp/hotspot-observations.jsonl \
  --budgets tests/performance/hotspot_budgets.json \
  --window-size 5 \
