...
| Data quality policy readiness (DQ thresholds/hash/medallion) | Data Quality Owner | DQ configs/tests + policy evidence | `.github/workflows/tests.yml` (DQ-related jobs), `.github/workflows/schema-governance.yml`; RULES §2.4/§2.8 | **PASS** if DQ checks green and the release scope validates the active threshold surface explicitly (`configs/base/quality.yaml` currently hard-fail 25%; contract/runtime fallbacks currently hard-fail 20%). **FAIL** on any blocking DQ gate. |
