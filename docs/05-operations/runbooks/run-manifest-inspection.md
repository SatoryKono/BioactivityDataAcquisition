...
Catalog readers share a bounded, process-local metadata index. Each lookup
checks file identity, size, mode and nanosecond modification/change timestamps;
changed files are reread, and read failures never fall back to cached values.
Report catalog entries retain only identity and schema metadata, not full report
bodies. Directory enumeration still discovers new and deleted runs. This index
reduces repeated Windows bind-mount reads; it does not replace identity/index
validation or establish artifact integrity for replay. A cold process must still
read the catalog, and slow or unavailable storage remains an explicit API error.
...
- Composite execution has
  `exact_replay_support_boundary=snapshot_backed_source_runs_only` plus
  `replay_family_contract.strict_exact_replay_supported=false`; cached Bronze
  snapshot refs are rebuild/resume evidence only and do not make the run
  exact-replayable;
