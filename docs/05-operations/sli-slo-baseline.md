...
The machine-readable SLO/alert binding lives in
`configs/quality/observability_slo_alert_contract.yaml` and is checked against
`grafana/prometheus-rules/bioetl_observability.yml` by
`tests/integration/test_prometheus_rules_config.py`.

| SLI | Metric source | Window | Target baseline | Alert / runbook linkage |
| --- | --- | --- | --- | --- |
| Monitoring stack availability | Prometheus `up` for `bioetl`, `prometheus`, `grafana`, `pushgateway`, `grafana-image-renderer` (opt-in stack; no Loki/Tempo/quarantine-explorer) | continuous, 2m/5m alert holds | Core collection available; identity HTTP via health server `bioetl:8000` (BioETL Ops HTTP); renderer outages block screenshot acceptance | `BioETL*Unavailable` alerts -> `runbooks/observability-checklist.md`; owner `@bioetl-observability` |
