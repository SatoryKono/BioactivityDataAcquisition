...
- **`DataSourceRegistry`**: находится в `factories/data_source_factory.py` и используется как фасад регистрации провайдеров.
...
runner = bootstrap_composite_pipeline(
    ...
    runtime=composite_runtime,
)
result = await runner.run()
```

- Функция **sync** (не async)
- Принимает `CompositeConfig` и `CompositeRuntimeConfig`
- Возвращает `CompositePipelineRunner`
...
- bootstrap/cli/checkpoint.py

- bootstrap/cli/storage.py
