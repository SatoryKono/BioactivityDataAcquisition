...
| Параметр          | Значение                                                                                                                                                                                                      |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Статус**        | Accepted (Levels 1,4 implemented; Level 2 Silver runtime executor explicitly retired; Levels 3,5 not yet realized — see Implementation Status)                                                                 |
| **Дата**          | 2026-02-06                                                                                                                                                                                                    |
| **Автор**         | BioETL Team                                                                                                                                                                                                   |
| **Ревьюер**       | —                                                                                                                                                                                                             |
...
| 2     | Structural Validation (cross-field)    | **RETIRED FOR SILVER RUNTIME** | Data model and config loading remain as governed config/contract evidence: `domain/config/validation.py` (`CrossFieldValidation`, `ConditionalValidation`), `infrastructure/schemas/dq_config.py`, and YAML rules under `configs/entities/**`. The generic per-record Silver transform executor is explicitly retired and must not be treated as an active implementation gap. New runtime execution of these rules requires a new ADR or an ADR-033 revision with deterministic quarantine, idempotency, and replay semantics. |
...
Levels 1 and 4 (Pandera + Gold range checks) provide **effective coverage** for the most impactful validation scenarios. The Level 2 Silver runtime executor is explicitly retired: the YAML/config model remains valid governance evidence, but it is not a pending runtime feature and must not be used as an unowned migration leftover.
...
**Recommendation:** Keep Levels 1 and 4 as the active enforcement path. Any future cross-field runtime executor must be proposed as a new deterministic design rather than reviving the retired ADR-033 Silver executor implicitly. Levels 3 and 5 should remain documented as future enhancements.
