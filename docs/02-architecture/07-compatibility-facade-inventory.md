...
| ----------------------------------------------------------- | ------------------ | ---------------- | ------ | ----- | ------------- | ------------------ | ----------------------- | -------------- | ------------- |
| `src/bioetl/interfaces/cli/commands/_compat.py` | Internal CLI compatibility shim helper that powers retained top-level command alias modules. | `bioetl.interfaces.cli.commands` | `compat-shim` | `bioetl.interfaces.cli.commands` | `2026-03 RF-024` | `src`: only retained top-level CLI public seam modules under `src/bioetl/interfaces/cli/commands/` may import `alias_module`; `tests`: dedicated CLI boundary coverage may exercise the helper indirectly through those public seam modules | `2026-09-30` | Keep top-level CLI public seams as the sanctioned import targets; do not import `_compat.py` outside the retained command-seam modules. | `_compat.py` remains confined to the reviewed top-level command seams and can be removed once those seams no longer need helper-based alias forwarding. |
...
| `src/bioetl/domain/services/__init__.py` | Bounded legacy import bridge from `bioetl.domain.services` to the canonical domain behavior package. | `bioetl.domain.behavior` | `compat-shim` | `bioetl.domain.behavior` | `2026-05 #3494` | `src`: no production imports of `bioetl.domain.services`; use `bioetl.domain.behavior`. `tests`: only dedicated compatibility or legacy import coverage may exercise the bridge until the review date. | `2026-09-30` | Keep all first-party production imports on `bioetl.domain.behavior`; migrate remaining non-compatibility tests to the canonical package and retain only explicit bridge coverage. | Production callers remain zero, non-compatibility tests no longer import the legacy package, and the bridge is removed or explicitly renewed during the 2026-09-30 compatibility review. |

### Sanctioned Public Entrypoints

| Path                                                           | Compatibility role                                                                                                                                                          | Canonical target                                        | Status              | Owner                                            | Introduced in                          | Allowed call sites                                                                                                                                                                                                                                                                                                                                                                                                                             | Remove by / review date | Migration path                                                                                                                                                                                                                                                                                  | Exit criteria                                                                                                                                                                                                                                    |
| -------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------- | ------------------- | ------------------------------------------------ | -------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `src/bioetl/interfaces/cli/commands/run.py` | Permanent public run command seam over the split internal owner module `bioetl.interfaces.cli.commands.domains.run.command`. | `bioetl.interfaces.cli.commands.run` | `public-entrypoint` | `bioetl.interfaces.cli.commands` | `2026-03 RF-024` | `src`: use `bioetl.interfaces.cli.commands.run` as the public seam; direct internal-owner imports stay confined to `src/bioetl/interfaces/cli/commands/domains/run/__init__.py`; `tests`: patch/import coverage may target `bioetl.interfaces.cli.commands.run`, while direct `bioetl.interfaces.cli.commands.domains.run.command` imports stay confined to dedicated boundary coverage | `2026-09-30` | Use `bioetl.interfaces.cli.commands.run` for public CLI wiring and patch targets; do not import `bioetl.interfaces.cli.commands.domains.run.command` directly outside the owning package or dedicated boundary tests. | The top-level module remains the sanctioned public CLI patch/import seam, and direct internal-owner imports remain confined to the owning package plus dedicated boundary tests. |
...
| `src/bioetl/interfaces/cli/commands/plan.py` | Permanent public plan command seam over the internal owner module `bioetl.interfaces.cli.commands.domains.maintenance.plan`. | `bioetl.interfaces.cli.commands.plan` | `public-entrypoint` | `bioetl.interfaces.cli.commands` | `2026-03 RF-024` | `src`: use `bioetl.interfaces.cli.commands.plan` as the public seam; direct internal-owner imports stay confined to `src/bioetl/interfaces/cli/commands/domains/maintenance/__init__.py`; `tests`: patch/import coverage may target `bioetl.interfaces.cli.commands.plan`, while direct `bioetl.interfaces.cli.commands.domains.maintenance.plan` imports stay confined to dedicated boundary coverage | `2026-09-30` | Use `bioetl.interfaces.cli.commands.plan` for public CLI wiring and patch targets; do not import `bioetl.interfaces.cli.commands.domains.maintenance.plan` directly outside the owning package or dedicated boundary tests. | The top-level module remains the sanctioned public CLI patch/import seam, and direct internal-owner imports remain confined to the owning package plus dedicated boundary tests. |
| `src/bioetl/interfaces/cli/commands/archive.py` | Permanent public archive command seam over the internal owner module `bioetl.interfaces.cli.commands.domains.maintenance.archive`. | `bioetl.interfaces.cli.commands.archive` | `public-entrypoint` | `bioetl.interfaces.cli.commands` | `2026-03 RF-024` | `src`: use `bioetl.interfaces.cli.commands.archive` only as the public seam for import/patch stability; direct internal-owner imports stay confined to `src/bioetl/interfaces/cli/commands/domains/maintenance/command.py`; `tests`: patch/import coverage may target `bioetl.interfaces.cli.commands.archive`, while direct `bioetl.interfaces.cli.commands.domains.maintenance.archive` imports stay confined to dedicated boundary coverage | `2026-09-30` | Use `bioetl.interfaces.cli.commands.archive` for public patch targets; do not import `bioetl.interfaces.cli.commands.domains.maintenance.archive` directly outside the owning maintenance package or dedicated boundary tests. | The top-level module remains the sanctioned public CLI patch/import seam, and direct internal-owner imports remain confined to the owning maintenance package plus dedicated boundary tests. |
| `src/bioetl/interfaces/cli/commands/cleanup.py` | Permanent public cleanup command seam over the internal owner module `bioetl.interfaces.cli.commands.domains.maintenance.cleanup`. | `bioetl.interfaces.cli.commands.cleanup` | `public-entrypoint` | `bioetl.interfaces.cli.commands` | `2026-03 RF-024` | `src`: use `bioetl.interfaces.cli.commands.cleanup` only as the public seam for import/patch stability; direct internal-owner imports stay confined to `src/bioetl/interfaces/cli/commands/domains/maintenance/command.py`; `tests`: patch/import coverage may target `bioetl.interfaces.cli.commands.cleanup`, while direct `bioetl.interfaces.cli.commands.domains.maintenance.cleanup` imports stay confined to dedicated boundary coverage | `2026-09-30` | Use `bioetl.interfaces.cli.commands.cleanup` for public patch targets; do not import `bioetl.interfaces.cli.commands.domains.maintenance.cleanup` directly outside the owning maintenance package or dedicated boundary tests. | The top-level module remains the sanctioned public CLI patch/import seam, and direct internal-owner imports remain confined to the owning maintenance package plus dedicated boundary tests. |
| `src/bioetl/interfaces/cli/commands/vacuum.py` | Permanent public vacuum command seam over the internal owner module `bioetl.interfaces.cli.commands.domains.maintenance.vacuum`. | `bioetl.interfaces.cli.commands.vacuum` | `public-entrypoint` | `bioetl.interfaces.cli.commands` | `2026-03 RF-024` | `src`: use `bioetl.interfaces.cli.commands.vacuum` only as the public seam for import/patch stability; direct internal-owner imports stay confined to `src/bioetl/interfaces/cli/commands/domains/maintenance/command.py`; `tests`: patch/import coverage may target `bioetl.interfaces.cli.commands.vacuum`, while direct `bioetl.interfaces.cli.commands.domains.maintenance.vacuum` imports stay confined to dedicated boundary coverage | `2026-09-30` | Use `bioetl.interfaces.cli.commands.vacuum` for public patch targets; do not import `bioetl.interfaces.cli.commands.domains.maintenance.vacuum` directly outside the owning maintenance package or dedicated boundary tests. | The top-level module remains the sanctioned public CLI patch/import seam, and direct internal-owner imports remain confined to the owning maintenance package plus dedicated boundary tests. |
| `src/bioetl/composition/entrypoints.py` | Permanent public composition entrypoint that intentionally shields internal `_pipeline_execution`, `_resource_management`, and `_services` module paths. | `bioetl.composition.entrypoints` | `public-entrypoint` | `bioetl.composition` | `2026-03 entrypoint freeze` | `src`: only explicit execution-focused public symbols from `__all__` may be imported from `bioetl.composition.entrypoints`; deprecated service/resource symbols resolved through `_LEGACY_SYMBOL_TARGETS` are external-compatibility only and are blocked in first-party source. `tests`: public entrypoint imports may appear in interface/composition boundary coverage, but direct internal-module patch targets stay confined to `tests/unit/composition/test_entrypoints.py`, `tests/unit/composition/test_resource_management.py`, and `tests/unit/composition/test_services_entrypoints.py` | `2026-09-30` | Use `bioetl.composition.entrypoints` as the sanctioned public seam; do not import `bioetl.composition._pipeline_execution`, `bioetl.composition._resource_management`, or `bioetl.composition._services` directly outside dedicated entrypoint-boundary coverage. | The public entrypoint remains sanctioned, deprecated service/resource symbol imports in first-party source stay at zero, internal implementation-module imports outside `composition/` stay at zero, and internal-module patch coverage remains confined to the dedicated entrypoint-boundary tests. |
| `src/bioetl/composition/__init__.py` | Retained package-root composition facade that preserves import stability for registry discovery and sanctioned public submodule access while first-party `src/` callers migrate to owner-focused APIs. | `bioetl.composition` | `retained-entrypoint` | `bioetl.composition` | `2026-04 TD-005 TD-006` | `src`: no direct imports; use `bioetl.composition.registry_api`, `execution_api`, `control_plane_api`, `health_api`, `maintenance_api`, `composite_api`, or `observability_api`; `tests`: dedicated compatibility coverage may import the package root to verify the frozen lazy-export budget | `2026-09-30` | Keep `bioetl.composition` only as a retained public compatibility seam for external import stability and discovery. New first-party `src/` imports must point at owner-focused composition APIs instead of the package root. | First-party `src/` imports of `bioetl.composition` stay at zero, the package-root export budget remains frozen, and any future surface change is reviewed through this inventory first. |
| `src/bioetl/composition/bootstrap/__init__.py` | Retained runtime-focused bootstrap package facade that preserves historical import stability while owner-focused execution, composite, and observability APIs serve as the canonical first-party composition seams. | `bioetl.composition.bootstrap` | `retained-entrypoint` | `bioetl.composition.bootstrap` | `2026-04 TD-001` | `src`: no direct imports; use `bioetl.composition.execution_api`, `composite_api`, or `observability_api`; `tests`: dedicated compatibility and runtime-boundary coverage may import the bootstrap package root | `2026-09-30` | Keep `bioetl.composition.bootstrap` as a retained compatibility seam for runtime-facing import stability, but route new first-party `src/` imports through owner-focused composition APIs instead of the bootstrap package root. | First-party `src/` imports of `bioetl.composition.bootstrap` stay at zero, the runtime-focused `__all__` budget remains frozen, and any new root-level export requires an explicit inventory review. |
| `src/bioetl/application/services/__init__.py` | Retained application-services package-root facade that preserves legacy flat service imports while first-party runtime code uses owner submodules. | `bioetl.application.services.execution, bioetl.application.services.control_plane, bioetl.application.services.lineage, or the concrete service/model owner module` | `retained-entrypoint` | `bioetl.application.services` | `2026-04 issue-3474 caller-zero gate` | `src`: no direct imports from `bioetl.application.services`; use semantic subpackages or concrete service/model owner modules; `tests`: temporary compatibility coverage may import the package root while migrating patch seams, but new production callers are blocked by architecture gates | `2026-09-30` | Keep `bioetl.application.services` as an external compatibility seam only. New first-party runtime code must import from canonical owner modules such as `bioetl.application.services.execution.pipeline_runner_models`, `bioetl.application.services.execution.pipeline_runner_service`, `export_models`, `export_service`, `vacuum_service`, or focused control-plane/lineage submodules. | First-party `src/` imports of `bioetl.application.services` stay at zero, remaining test-only package-root imports are migrated or intentionally retained as compatibility coverage, and any new root export requires inventory review before merge. |
...
| `src/bioetl/domain/value_objects/publication_field_groups.py` | Permanent public entrypoint for publication field-group definitions that shields private split config/type modules. | `bioetl.domain.value_objects.publication_field_groups` | `public-entrypoint` | `bioetl.domain.value_objects` | `legacy-pre-2026-03` | `src`: canonical entrypoint usage allowed; private split modules stay inside `src/bioetl/domain/value_objects/`; `tests`: direct split-internal coverage stays confined to `tests/unit/domain/value_objects/test_value_object_facade_reexports.py`, while ordinary tests keep importing the public entrypoint | `2026-09-30` | Use `bioetl.domain.value_objects.publication_field_groups` as the sanctioned public seam; do not import split value-object internals outside the owning package or the dedicated facade-coverage test. | The facade remains the sanctioned public path and direct imports of private publication-field-group internals stay confined to the owning package plus the dedicated facade-coverage test. |
| `src/bioetl/application/composite/merger.py` | Permanent public composite merge module that requires `MergeCollaboratorGroup` bundle; legacy per-collaborator keyword wiring removed in RF-009.2. | `bioetl.application.composite.merger` | `public-entrypoint` | `bioetl.application.composite` | `2026-03 merge collaborator migration` | `src`: all wiring passes `collaborators=MergeCollaboratorGroup(...)`; `tests`: `tests/unit/application/composite/test_merger.py`, `tests/unit/application/composite/merge_test_support.py`, `tests/unit/composition/bootstrap/runtime/test_composite_support_services_factory.py` | `2026-09-30` | Use `bioetl.application.composite.merger` as the sanctioned public seam with `collaborators=MergeCollaboratorGroup(...)`. | The merge module remains the sanctioned public seam and all composition paths use collaborator bundles with legacy keyword wiring fully removed. |
| `src/bioetl/infrastructure/adapters/pubmed/client.py` | Permanent public client entrypoint that shields older `pubmed_client` imports while exporting the current adapter surface and public `create_pubmed_adapter` factory alias. | `bioetl.infrastructure.adapters.pubmed.client` | `public-entrypoint` | `bioetl.infrastructure.adapters.pubmed` | `2026-03 pubmed entrypoint hardening` | `src`: direct `bioetl.infrastructure.adapters.pubmed.client` imports stay confined to `src/bioetl/infrastructure/adapters/pubmed/__init__.py`; first-party code imports the provider package root; `tests`: `tests/unit/infrastructure/adapters/test_provider_entrypoints.py`, `tests/architecture/test_adapter_contracts.py`, `tests/architecture/test_retained_adapter_entrypoint_policy.py` | `2026-09-30` | Use the provider package root `bioetl.infrastructure.adapters.pubmed` in new first-party code; keep `bioetl.infrastructure.adapters.pubmed.client` as the sanctioned public adapter seam and do not import `bioetl.infrastructure.adapters.pubmed.pubmed_client` directly. | The public `client.py` seam remains sanctioned, direct `client.py` imports stay confined to the provider package root plus dedicated compatibility coverage, and private `_create_pubmed_adapter` remains unexported from the public entrypoint. |
| `src/bioetl/infrastructure/adapters/semanticscholar/client.py` | Permanent public client entrypoint that shields older `adapter` imports. | `bioetl.infrastructure.adapters.semanticscholar.client` | `public-entrypoint` | `bioetl.infrastructure.adapters.semanticscholar` | `legacy-pre-2026-03` | `src`: direct `bioetl.infrastructure.adapters.semanticscholar.client` imports stay confined to `src/bioetl/infrastructure/adapters/semanticscholar/__init__.py`; first-party code imports the provider package root; `tests`: `tests/unit/infrastructure/adapters/test_provider_entrypoints.py`, `tests/architecture/test_adapter_contracts.py`, `tests/architecture/test_retained_adapter_entrypoint_policy.py` | `2026-09-30` | Use the provider package root `bioetl.infrastructure.adapters.semanticscholar` in new first-party code; keep `bioetl.infrastructure.adapters.semanticscholar.client` as the sanctioned public adapter seam and do not import `bioetl.infrastructure.adapters.semanticscholar.adapter` directly. | The public `client.py` seam remains sanctioned, direct `client.py` imports stay confined to the provider package root plus dedicated compatibility coverage, and legacy-path references remain reduced to that public seam. |
...
- For provider adapters, first-party code should prefer provider package roots when those roots are
  the documented canonical path; sanctioned `client.py` modules remain stable public seams.
