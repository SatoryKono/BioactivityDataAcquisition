...
| `src/bioetl/application/composite/merger.py` | Mixed composite merge module that exposes the canonical `MergeCollaboratorGroup` bundle while still accepting legacy per-collaborator keyword wiring. | `bioetl.application.composite.merger` | `mixed-module` | `bioetl.application.composite` | `2026-03 merge collaborator migration` | `src`: new composition/runtime wiring should pass `collaborators=MergeCollaboratorGroup(...)`; legacy per-collaborator kwargs remain transitional and must not expand to new call sites; `tests`: `tests/unit/application/composite/test_merger.py`, `tests/unit/application/composite/merge_test_support.py`, `tests/unit/composition/bootstrap/runtime/test_composite_support_services_factory.py` | `2026-09-30` | Use `bioetl.application.composite.merger` in new bootstrap/runtime seams and phase out legacy keyword wiring once compatibility coverage is no longer needed. | Production composition paths use collaborator bundles by default, and legacy keyword-only construction survives only in dedicated compatibility tests or explicitly grandfathered seams. |
| `src/bioetl/application/composite/merger_compatibility_mixin.py` | Compatibility helper mixin that retains historical `MergeService` helper methods while forwarding behavior to canonical join/conflict/coalesce collaborators. | `bioetl.application.composite.merger.MergeService` | `compat-shim` | `bioetl.application.composite` | `2026-03 RF-005 merge facade split` | `src`: import stays confined to `src/bioetl/application/composite/merger.py`; new application/runtime code must call `MergeService.merge()` or the injected collaborator services directly instead of importing the mixin; `tests`: `tests/unit/application/composite/test_merger.py`, `tests/unit/application/composite/test_merger_golden_snapshots.py` | `2026-09-30` | Keep the mixin internal to `bioetl.application.composite.merger`; phase compatibility callers toward `MergeService.merge()` plus direct `JoinPlannerService`, `ConflictResolverService`, and `CoalescePolicyService` coverage. | No first-party module imports the mixin directly outside `merger.py`, and compatibility tests stop calling the helper wrappers once dedicated collaborator coverage is sufficient. |
...
- `src/bioetl/application/composite/merger_compatibility_mixin.py`
