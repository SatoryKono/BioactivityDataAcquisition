...
| `src/bioetl/composition/maintenance_api.py` | Permanent logic-free public maintenance lazy re-export facade that shields maintenance/resource owners behind a reviewed export budget. | `bioetl.composition.maintenance_api` | `public-entrypoint` | `bioetl.composition` | `2026-05 public export review` | `src`: do not add first-party imports of `bioetl.composition.maintenance_api`; the maintenance CLI owner seam routes through `bioetl.composition.entrypoints` and the reviewed `_services` owner. `tests`: public facade imports may appear in composition coverage only when validating the external seam itself. | `2026-12-31` | External consumers use `bioetl.composition.maintenance_api` as the single supported maintenance import path; first-party runtime callers use the reviewed owner seam and executable behavior remains in owner modules. | The public maintenance facade contains no executable wrapper bodies, remains the sanctioned external import seam, and its reviewed export budget stays bounded. |
