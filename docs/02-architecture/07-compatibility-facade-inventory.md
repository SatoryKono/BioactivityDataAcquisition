...
| `src/bioetl/application/core/base_transformer/dependencies.py` | Typing-only compatibility facade that preserves legacy `TransformerDependencyContext` imports while concrete default collaborator assembly stays composition-owned. | `bioetl.application.core.base_transformer.types.TransformerDependencyContext` | `compat-shim` | First-party imports migrate to the package root or `types.py` directly and only compatibility coverage continues exercising the facade. |
