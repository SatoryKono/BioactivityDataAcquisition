...
| `src/bioetl/composition/registry.py` | Mixed registry seam whose instance API is canonical only through the package-root composition surface, while `get_default_registry()` is a narrow compatibility wrapper over dedicated shared state. | `bioetl.composition` | `mixed-module` | `bioetl.composition` | `2025-12 instance-registry migration` | `src`: canonical `PipelineRegistry`, `PipelineDefinition`, and `create_registry()` imports go through `bioetl.composition`; direct `bioetl.composition.registry` imports stay confined to `src/bioetl/composition/__init__.py` and `src/bioetl/composition/registry_default.py`; direct imports of `get_default_registry()` from `bioetl.composition.registry` stay at zero in first-party `src`; `tests`: direct module imports stay confined to `tests/architecture/test_registry_contracts.py`, `tests/unit/composition/test_types.py`, and `tests/unit/composition/factories/pipeline/test_registry.py`; ordinary tests import instance APIs from `bioetl.composition` | `2026-09-30` | Use `bioetl.composition` for canonical instance-registry imports and prefer `create_registry()` plus explicit registration/wiring for new code; treat `get_default_registry()` as transitional shared-state compatibility only, with explicit compatibility-only re-exports documented in `types.py`. | Shared default-registry behavior stays implemented outside the canonical instance API, direct `bioetl.composition.registry` imports remain confined to dedicated compatibility seams, and ordinary tests/runtime code use `bioetl.composition` for instance APIs. |
| `src/bioetl/infrastructure/config_loader.py` | Compatibility shim that preserves the historical cached loader and source-section helper surface while delegating staged pipeline-config flow to dedicated config modules. | `bioetl.infrastructure.config` | `compat-shim` | `bioetl.infrastructure.config` | `2026 ADR-039 transition wave` | `src`: no direct first-party imports are expected; new function-based loader imports use the public `bioetl.infrastructure.config` package seam and only drop to `bioetl.infrastructure.config.pipeline_config_api` for leaf staged-loader flows instead of `config_loader.py`; `tests`: direct module/symbol imports stay confined to `tests/unit/infrastructure/test_config_dynamic.py` and `tests/unit/infrastructure/test_config_loader_schema_path.py`; ordinary contract/architecture tests load configs through `bioetl.infrastructure.config` | `2026-09-30` | Import `load_pipeline_config` and `load_source_config` from `bioetl.infrastructure.config` in first-party code, use `bioetl.infrastructure.config.pipeline_config_api` for staged pipeline-loader seams, and keep `bioetl.infrastructure.config_loader` only for legacy compatibility coverage of the preserved cached/source-section helpers. | First-party `src/` imports of `bioetl.infrastructure.config_loader` stay at zero, ordinary tests stop using the compatibility wrapper as the normal load path, dedicated compatibility tests remain limited to the preserved cached/source-section helpers, and the canonical staged flow lives in `bioetl.infrastructure.config` / `pipeline_config_api` rather than inside `config_loader.py`. |
...
- `src/` direct imports of `get_default_registry` from `bioetl.composition.registry`: `0`
- `src/` direct imports of `bioetl.infrastructure.config_loader`: `0`
...
- `tests/` direct imports of `get_default_registry` from `bioetl.composition.registry` outside dedicated registry compatibility tests: `0`
...
- `tests/` direct imports of `bioetl.infrastructure.storage.metadata_builder_composite_helpers` outside `tests/unit/infrastructure/storage/test_metadata_builder_composite_helpers.py`: `0`
