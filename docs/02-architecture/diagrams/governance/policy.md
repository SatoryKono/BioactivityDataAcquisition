...
5. Рендер-артефакты: `svg/png` под sibling-каталогами `diagrams/**/svg` и `diagrams/**/png` считаются поддерживаемыми documentation outputs и регенерируются через `render.sh` после изменения источников.

## 3. Форматы и именование

1. Обязательный формат для новых диаграмм: Mermaid (`.mmd`).
2. Расширение `.mermaid` допускается для decomposed views (`diagrams/views/`) и legacy-набора.
...
2. Есть соответствующий обновлённый рендер: `svg` как обязательный publication artifact и `png` там, где он ещё нужен как compatibility/export surface.
