...
| ------------------------- | ------------ | ------------------------------------------------------------------------------------------------------------------------------------------ |
| Lock TTL                  | 90s          | `RuntimeConfig` (`src/bioetl/domain/config/runtime.py`)                                                                                    |
| Heartbeat Interval        | 30s          | `LockManager` (`src/bioetl/application/core/lock_manager.py`)                                                                              |
| Circuit Breaker Threshold | 5 failures   | `CircuitBreaker` (`src/bioetl/infrastructure/adapters/http/circuit_breaker.py`)                                                            |
| Recovery Timeout          | 300s (5 min) | `CircuitBreaker` (`src/bioetl/infrastructure/adapters/http/circuit_breaker.py`)                                                            |
| DQ Soft Threshold         | 5%           | `DQConfig` (`src/bioetl/domain/config/dq.py`)                                                                                              |
| DQ Hard Threshold         | 20%          | `DQConfig` (`src/bioetl/domain/config/dq.py`)                                                                                              |
| Bronze Retention          | 90 days      | `BronzeCleanupService` (`src/bioetl/application/services/bronze_cleanup_service.py`)                                                       |
| Quarantine Retention      | 30 days      | `QuarantinePort` + `UnifiedQuarantineService` (`src/bioetl/domain/ports/quarantine.py`, `src/bioetl/infrastructure/quarantine/unified.py`) |
| Silver/Gold Retention     | Permanent    | `StoragePort` (`src/bioetl/domain/ports/storage.py`)                                                                                       |
...
├─────────────────────────────────────────────────────────────┤
│                    APPLICATION LAYER                        │
│  PipelineRunner, BatchExecutor (`src/bioetl/application/core/│
│  batch_executor.py`), RecordProcessor,                      │
