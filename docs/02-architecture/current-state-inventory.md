...
| --- | ---: | --- |
| Architecture quality score | `8.56` (`good_targeted_improvements`) | `reports/quality/debt-governance-gates.json`, `reports/quality/architecture-quality-scorecard.json` |
...
| Full-app duplication hotspot baseline | `49` clusters | `reports/quality/full-app-duplication-baseline.json` |
...
| Current-state architecture evidence table lagged live quality reports | `reports/quality/debt-governance-gates.json` reports score `8.56`, `38` passing gates, and no warnings; `reports/quality/module-coverage-inventory.json` reports `2213` source modules with zero unmeasured/uncovered modules; `reports/quality/full-app-duplication-baseline.json` reports `49` clusters. | `reports/quality/*.json` generated on 2026-07-01. | Refreshed the current-state table and verification date from live report artifacts. |
