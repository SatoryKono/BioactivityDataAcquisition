...
`module_coverage_unmeasured_modules=1`; generated artifact drift is currently
...
- Module coverage currently has one unmeasured source module and no uncovered
  source modules in `reports/quality/module-coverage-inventory.json`; keep this
  as a regression gate through `report-module-coverage --check` and
