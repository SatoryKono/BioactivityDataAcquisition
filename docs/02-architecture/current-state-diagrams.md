...
| Optional health/ops process | `src/bioetl/interfaces/cli/commands/domains/health/observability_backend_runtime.py` is the CLI façade; `src/bioetl/application/services/ops/observability_backend_process.py` launches `python -m bioetl health server`; `docker-compose.yml` packages the same command |
