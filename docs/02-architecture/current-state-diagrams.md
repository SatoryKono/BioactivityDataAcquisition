...
| Optional health/ops process | `src/bioetl/interfaces/cli/commands/domains/health/observability_backend_runtime.py` is the CLI façade; `src/bioetl/composition/observability_backend.py` wires `src/bioetl/infrastructure/observability/observability_backend_process.py` which launches `python -m bioetl health server`; `docker-compose.yml` packages the same command |
