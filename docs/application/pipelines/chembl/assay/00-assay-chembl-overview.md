...
- **flatten_chembl_payload** — утилита для детерминированного сплющивания сырых JSON-ответов, с исключениями для вложенных полей (classification/parameters).
- **AssayTableSchema** — описывает обязательность `assay_chembl_id`, типы полей и порядок колонок (нормализованный слой).
