...
2) `transform` применяет `ChemblTransformerImpl` + post-transform цепочку по умолчанию (`HashColumnsTransformerImpl`, `IndexColumnTransformerImpl`, `DatabaseVersionTransformer`, `FulldateTransformerImpl`).
