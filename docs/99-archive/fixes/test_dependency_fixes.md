...
1. **Environment:** `/home/fedor/.venvs/bioetl` (created by setup script)
...
1. **Async Tests:** Use `--asyncio-mode=auto` flag for pytest when running async tests

1. **Dependency Management:** Use the project's setup scripts rather than manual pip installs

1. **Missing Files:** Check git history when encountering missing module errors - files may have been removed in recent commits
