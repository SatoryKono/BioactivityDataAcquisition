...
1. Все команды/скрипты и тесты, использующие `memory`, работают через `src`-layout.
1. CI зелёный на архитектурных, unit, integration и docs lane.
1. Документация и onboarding отражают новый контракт запуска.
...
1. Собрать список всех запусков, где используется `import memory`:
   - `scripts/memory/*`
   - `testing_support/*`
   - `tests/unit/memory/*`
   - локальные команды из README/SKILL.
1. Заморозить параллельные изменения в `src/memory`, `scripts/memory`, `pyproject.toml` на время миграции.
...
1. Проверить, что импорты `from memory...` резолвятся из `src/memory`, а не из корня.
1. Обновить smoke-команды в `scripts/memory/README.md` и `docs/00-project/ai/memory/README.md`.
...
1. В warning указать дату удаления и требуемый контракт запуска.
1. Добавить тест, проверяющий наличие предупреждения и корректный fallback в `src/memory`.

### 5. Удаление shim

1. Удалить `memory/__init__.py` и пустую директорию `memory/`.
1. Удалить связанные исключения/обходы в тестах, если были.
1. Обновить docs и внутренние инструкции на “только `src/memory`”.
...
1. Полный шардовый прогон CI lanes, где раньше были падения по импорту/форматированию.
1. Ручной smoke:
...
1. Хранить rollback-коммит готовым до закрытия релиза.
...
1. PR-2: удаление shim + cleanup + тесты.
1. PR-3 (опционально): ужесточение проверок (например, тест “root `memory/` отсутствует”).
