...
Зависимости этого RF не изменились: `RF-FS-002` всё ещё не стоит запускать до стабилизации `RF-FS-001` и `RF-FS-004`. При этом недавние ownership waves уже сняли один соседний structural pain point в `application/services`, так что будущий package-split здесь должен концентрироваться именно на четырёх hotspot-пакетах из scope, а не расползаться на уже закрытый service-tail.
