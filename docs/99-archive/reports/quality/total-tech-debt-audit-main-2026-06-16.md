...
| Composition | Compatibility debt | `src/bioetl/composition/entrypoints.py`, `health_api.py`, `maintenance_api.py` | 4 retained public export facades and 0 lazy-export twin pairs. | Public API protection is explicit, but lazy indirection increases import-surface complexity. |
...
| `src/bioetl/domain/value_objects/activity_values.py` | Removed value-object facade that previously shielded split activity value modules. | No first-party source importers in refreshed compatibility census; activity symbols remain available through `bioetl.domain.value_objects`. | Already removed. | Low; keep tests/docs importing the package root instead of restoring the facade. |
...
| `src/bioetl/composition/lazy_exports.py` and `_lazy_exports.py` | Retired public/private lazy-export twin pair. | Refreshed compatibility census reports `twin_pair_count=0`. | Already removed from tracked twin burden. | Low; fail-fast census should prevent reintroducing public/private twin pairs. |
