...
Run ID: [run-492157 ▼]  ← выбрать конкретный run

Действия:
1. Кликнуть на Pipeline → выбрать "uniprot"
2. График обновится и покажет ТОЛЬКО uniprot данные
3. Кликнуть на Run ID → выбрать "run-492157"
4. График обновится и покажет ТОЛЬКО этот run
...
**Диагностика:**
```
1. Видны ли фильтры (Pipeline, Run ID)?
   ✅ Да   → перейти к шагу 2
