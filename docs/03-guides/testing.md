...
Для обеспечения стабильной работы Quality Gates (особенно расчёта покрытия) в CI-окружении, проект использует строгую проверку зависимостей.

### 7.1. Smoke-check зависимостей

Перед запуском основного набора тестов необходимо убедиться, что все критические runtime-зависимости (`pandas`, `pandera`, `polars` и др.) установлены и доступны для импорта.
...
Эта команда запускает специализированный тест `TestRuntimeDependencies` из `tests/smoke/test_smoke.py`, который проверяет доступность всех пакетов, указанных в `pyproject.toml`.

### 7.2. Решение проблем с Coverage

Если `pytest-cov` не может собрать данные или падает с ошибкой импорта:
1. Убедитесь, что выполнена команда `make install`.
2. Запустите `make test-deps` для выявления отсутствующего пакета.
3. Проверьте версию Python (минимум 3.11).

В CI-конвейере шаг `make test` автоматически включает `test-deps` как пререквизит, что гарантирует достоверность отчётов о покрытии.
