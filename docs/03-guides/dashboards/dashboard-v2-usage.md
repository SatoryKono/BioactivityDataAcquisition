...
- `overview.id=215 (Review First Action)`: table shows up to **four** routes via
  ...
  escalating. The RFA-00 migration was applied via a retired one-off mutator
  (historical only; removed after verification). Do not reintroduce dashboard
  mutators under `scripts/ops/observability/grafana/` — edit shipped
