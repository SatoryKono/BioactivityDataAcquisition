...
is `0. Run Explorer` and alert triage lives in Incident Workspace.
...
- Navigation panels use the same link-only layout at `h=3` on all seven
  dashboards, without a visible heading. `options.bioetlDisplayTitle` preserves
  `Navigate Dashboards` for inventories and render tooling. Links retain their
  visible names and sanitizer-safe `title` tooltips; Grafana strips the authored
  group ARIA label. Link text stays at `16px`. Current and inactive items both use `2px` borders so changing
  dashboards does not move the buttons.
- Use `rerender-grafana --navigation-only --no-expand-collapsed-rows` for a
  ...
  exposes all seven named items, keeps link text at `16px`, contains all
...
- Pipeline-scoped handoffs to `4. Provider Health` keep `pipeline_context=$pipeline`
...
стабильный `id=1`). Эталон — визуальная семантика `5. Data Quality`: orange accent,
...
frozen `2. Overview v3` layout as its baseline. It answers one question:
...
| --- | --- | --- |
| Status | `2. Overview` | trusted derivatives in `2. Runtime`, `0. Control Plane` |
| First Action | `2. Overview` | `2. Runtime`, `4. Provider Health` |
| Inputs | `2. Overview` | `2. Runtime`, `5. Data Quality`, `0. Control Plane` |
| Data Validation | `2. Overview` | `2. Runtime`, `0. Control Plane` |
| Provider | `2. Overview` | `4. Provider Health` |
| Workflow evidence | `2. Overview` | `3. Pipeline Diagnostics` (workflow band) |
| Replay Safety State | `0. Control Plane` | `2. Overview`, `2. Runtime` |
| Checkpoint Freshness Lag | `0. Control Plane` | `2. Runtime` |
| Ledger/Manifest Consistency | `0. Control Plane` | `2. Runtime` |
| Provider Health (aggregated) | `4. Provider Health` | `2. Overview`, `2. Runtime` |
| DQ Status (Silver Reject / quality posture) | `5. Data Quality` | `2. Overview`, `2. Runtime` |
