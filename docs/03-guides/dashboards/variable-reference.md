...
| `$pipeline` | `bioetl-overview-v2`, `bioetl-control-plane-v1`, `bioetl-runtime`, `bioetl-provider-health-v2`, `bioetl-dq-v2`, `bioetl-workflow-overview`, `bioetl-silver-reject-explorer`, `bioetl-alerts-slo` | Prometheus label query from each dashboard's bounded universe: Overview/DQ/Provider/Workflow/Alerts use `bioetl_overview_pipeline_run_type_universe`; Runtime uses its exact alias source `bioetl_runtime_pipeline_run_type_universe`; Control Plane uses the documented control-plane subset `bioetl_control_plane_run_type_universe`; Explorer uses the documented concrete processed-row subset for Quarantine API | Single-select | `All` on Overview; otherwise fail-closed `unknown` | Canonical pipeline context. The normative exact/shared and subset relations live in `contracts/selector-contracts.yaml#pipeline_universe_contract`. Explorer requires one concrete pipeline. Provider/Workflow/Alerts expose `$pipeline` as context shell, not as their primary business selector. |
