...
| `$run_id` | `bioetl-overview-v2`, `bioetl-control-plane-v1`, `bioetl-runtime`, `bioetl-provider-health-v2`, `bioetl-dq-v2`, `bioetl-workflow-overview` | Quarantine Explorer HTTP `/ops/control-plane/filter-options?dimension=run_id&response_shape=list&workflow=${workflow}&pipeline=${pipeline}&run_type=${run_type:csv}` | Single-select, no Include All | `-` | Preserved identity context for shared HTTP `ID`/details panels and primary-dashboard handoffs; not a Prometheus label. Explicit inbound links to Silver Reject Explorer may map this value into `$quarantine_run_id`; outbound explorer/alert links do not export primary `$run_id`. |
