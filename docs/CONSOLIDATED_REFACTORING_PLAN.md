...
*Версия: 1.0 | Дата: 2025-12-27 | Статус: Финальный*
...
#### P1.1: Включить require_lock=True по умолчанию

**Статус**: ⚠️ ТРЕБУЕТСЯ
**Проблема**: Валидация реализована, но отключена по умолчанию (`require_lock=False`).

| Параметр | Значение |
|----------|----------|
| Текущее состояние | `require_lock=False` в `bronze_writer.py:63`, `delta_writer.py:80`, `gold_writer.py:66` |
| Целевое состояние | `require_lock=True` по умолчанию |
| Риски | Ломающие изменения для существующих вызовов |
| Митигация | Фаза 1: warning при `require_lock=False`; Фаза 2: default `True` |
| Трудозатраты | S (0.5-1 день) |

**Критерий готовности**:
```bash
grep "require_lock: bool = True" src/bioetl/infrastructure/storage/*.py
# Должен найти 3 вхождения
```
