...
Проведен точечный фикс (fix_failures) для архитектурных тестов. Все падающие тесты (3 штуки) были исправлены.
...
3. `tests/architecture/test_architecture_dependency_docs_drift.py::test_dependency_map_drift_check_passes_current_repo` - исправлен перегенерацией map через `scripts/generate_architecture_dependency_map.py --update`.
...
| 3 | test_dependency_map_drift_check_passes_current_repo | Contract | Architecture Map Drift | `python scripts/generate_architecture_dependency_map.py --update` | `tests/architecture/test_architecture_dependency_docs_drift.py` |
