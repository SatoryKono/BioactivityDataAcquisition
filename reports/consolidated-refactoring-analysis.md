...
Анализ четырёх планов рефакторинга выявил **критическую проблему**: ~60% предложенных задач основаны на **ложных утверждениях** о состоянии кодовой базы. Это произошло из-за несоблюдения протокола верификации кодом и проведения аудитов в **сломанном окружении** (без orjson и других зависимостей).

**Реальный общий балл после коррекции: ~8.5/10** (vs 6.87-7.42 в планах)

### Источники (анализируемые планы из PR)

| # | Документ | Заявленный балл | Ложных утверждений |
|---|----------|-----------------|-------------------|
| 1 | `reports/architecture_audit_bioetl.md` (v1) | 6.87 | 5/9 (55%) |
| 2 | Сводные метрики (inline) | 7.27 | 4/9 (44%) |
| 3 | `docs/09-architecture-audit-2025-12-29.md` (Jules) | 7.42 | 5/8 (62%) |
| 4 | `reports/architecture-audit-20250311.md` | 7.40 | 3/7 (43%) |

### Общие проблемы всех 4 планов

| Проблема | Частота | Причина |
|----------|---------|---------|
| Аудит в сломанном окружении | 4/4 | Отсутствие orjson, pytest не запускался |
| Утверждения без grep/Read | 4/4 | Нет ссылок `файл:строка` |
| "MemoryLock требует Redis" | 4/4 | Непонимание архитектуры локальных пайплайнов |
| "DQ метрики не реализованы" | 3/4 | Не проверен `postrun_service.py` |
| "VACUUM не автоматизирован" | 4/4 | Не проверен `runner.py:136` |
...
## Часть 4. Сравнительная таблица утверждений по 4 планам (из PR)

### 4.1 Сравнение оценок по категориям

| Категория | План 1 (v1) | План 2 (Сводные) | План 3 (Jules) | План 4 (20250311) | Верифицированная |
|-----------|-------------|------------------|----------------|-------------------|------------------|
| Слоистая архитектура | 9 | 9 | 9 | 9 | **9** |
| Контракты и Ports | 8 | 9 | 9 | 8 | **9** |
| Medallion Architecture | 8 | 8 | 8 | 7 | **9** |
| Ошибки и Circuit Breaker | 7 | 7 | 8 | 7 | **8** |
| Блокировки | **6** | **5** | **5** | **6** | **8** ← занижено |
| Валидация и DQ | 6 | 6 | 8 | 7 | **9** ← занижено |
| Логирование | 7 | 8 | 8 | 6 | **8** |
| Тестирование | **3** | **3** | **3** | 9 | **9** ← сломанное окружение |
| Безопасность | 6 | 8 | 7 | 7 | **8** |
| Документация | 6 | 8 | 7 | 7 | **8** |
| **Итого** | **6.87** | **7.27** | **7.42** | **7.40** | **8.5** |

### 4.2 Ключевые ложные задачи P1/P2 из планов

| Задача из плана | Приоритет | Какой план | Почему ЛОЖНО | Верификация |
|-----------------|-----------|------------|--------------|-------------|
| "Усилить блокировки (heartbeat + fencing)" | P1 | 1, 2, 3 | УЖЕ реализовано | `memory_lock.py:176-238` |
| "Восстановить тестовый контур" | P1 | 1, 2, 3 | Проблема окружения, не кода | 2895 passed, 89% cov |
| "Добавить Redis SETNX" | P1 | 1, 3 | MemoryLock достаточен by design | `CLAUDE.md` §5 |
| "DQ пороги и метрики" | P2 | 1, 2 | УЖЕ реализовано | `postrun_service.py:122-207` |
| "Автоматизировать VACUUM" | P2 | 1, 2, 4 | УЖЕ реализовано | `runner.py:136` |
| "Ужесточить Pandera strict" | P2 | 2, 3 | strict=False by design | `pandera_validator.py:34` |
| "Обязательная Pandera-валидация" | P2 | 2, 3 | NoOp — Null Object Pattern | По дизайну |

### 4.3 Сравнение утверждений

| Утверждение | План 1 | План 2 | План 3 | План 4 | Статус |
|-------------|--------|--------|--------|--------|--------|
| MemoryLock без TTL/heartbeat | ✓ P1 | ✓ P1 | ✓ P1 | ✓ P1 | ❌ ЛОЖНО |
| Требуется Redis для locks | ✓ P1 | ✓ P1 | ✓ P1 | — | ❌ ЛОЖНО |
| Pytest падает (orjson) | ✓ P1 | ✓ P1 | ✓ P1 | — | ❌ Проблема окружения |
| VACUUM не автоматизирован | ✓ P2 | ✓ P2 | ✓ P2 | ✓ P2 | ❌ ЛОЖНО |
| DQ метрики не в Prometheus | ✓ P2 | — | — | — | ❌ ЛОЖНО |
| NoOp validators — нарушение DI | ✓ P2 | ✓ P2 | ✓ P2 | — | ⚠️ ПО ДИЗАЙНУ |
| Pandera strict=False баг | — | ✓ P2 | ✓ P2 | — | ❌ ЛОЖНО (by design) |
| CLI click.echo нарушает logging | — | — | — | ✓ P2 | ⚠️ КОРРЕКТНО для CLI |
| API ключи хардкод | ✓ P3 | ✓ P3 | ✓ P3 | ✓ P3 | ❌ ЛОЖНО (env vars) |

**Легенда:**
- ❌ ЛОЖНО — утверждение не соответствует коду
- ⚠️ ПО ДИЗАЙНУ — преднамеренное архитектурное решение
- ⚠️ КОРРЕКТНО — валидный паттерн для interfaces слоя
