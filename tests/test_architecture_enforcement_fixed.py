...
def test_infrastructure_no_application_imports(src_dir: Path):
    ...
    ИСКЛЮЧЕНИЕ: infrastructure.orchestration - это глюевой слой оркестрации,
    который запускает Application pipelines. Он находится на границе системы.
    """
    ...
    for py_file in infrastructure_path.rglob("*.py"):
        ...
        # ИСКЛЮЧЕНИЕ: orchestration слой может импортировать Application
        # Это "глюевой" слой, который запускает pipelines через Prefect/Airflow
    ...
    assert not violations, (
        ...
        "Exception: infrastructure.orchestration (glue layer for running pipelines)."
    )
