...
def test_coordinator_exists_and_is_always_on() -> None:
    ...
    assert "pull_request" in on, "coordinator must trigger on pull_request"
    pr = on["pull_request"]
    assert pr is None or not {"paths", "paths-ignore"}.intersection(pr if isinstance(pr, dict) else {}), "coordinator pull_request must be unfiltered"
