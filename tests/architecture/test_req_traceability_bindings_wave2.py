...
Each REQ maps to a tuple of surface paths that MUST exist in the repo
checkout. The single guard test below asserts existence — semantic content
checks (e.g. SilverWriteMode enum, httpx import, pandera nullable) are
covered by dedicated architecture tests; this file provides the
requirements <-> surface binding for traceability.
"""
...
# REQ_ID -> tuple of surface paths (relative to repo root) that prove the
# requirement is implemented. All paths listed here MUST exist.
WAVE2_CONCRETE_SURFACES: dict[str, tuple[str, ...]] = {
    # REQ-DATA-007 — absence of bare .parquet Silver tables without _delta_log/
    "REQ-DATA-007": (
    ...
    # REQ-DATA-008 — only supported SilverWriteMode + no semantic bucket keys in silver_filters
    "REQ-DATA-008": (
        "src/bioetl/infrastructure/storage/silver_writer.py",
        "src/bioetl/infrastructure/storage/__init__.py",
        "pyproject.toml",
    ),
    # REQ-DELTA-001 — import deltalake (delta-rs)
    "REQ-DELTA-001": (
        "pyproject.toml",
        "src/bioetl/infrastructure/storage/delta_reader.py",
    ...
    # REQ-CLEAR-001 — test-rebuild-lifecycle-order (clear is called on rebuild)
    "REQ-CLEAR-001": (
        "src/bioetl/infrastructure/storage/silver_writer.py",
        "src/bioetl/infrastructure/storage/bronze_writer.py",
        "tests/architecture/test_medallion_invariants.py",
    ),
    # REQ-CLEAR-003 — test-incremental-skips-clear (clear NOT called on incremental)
    "REQ-CLEAR-003": (
        "src/bioetl/infrastructure/storage/silver_writer.py",
        "tests/architecture/test_medallion_policy.py",
    ),
    # REQ-CLEAR-004 — async def / await (async clear path)
    "REQ-CLEAR-004": (
        "src/bioetl/infrastructure/storage/silver_writer.py",
    ...
    "REQ-LOAD-001": (
        ...
        "src/bioetl/infrastructure/checkpoint/__init__.py",
    ...
    # REQ-STACK-003 — __init__ package markers
    "REQ-STACK-003": (
    ...
    # REQ-PYTHON-002 — List/Dict/Set/Tuple (modern generics, no typing.List)
    "REQ-PYTHON-002": (
        "pyproject.toml",
        "src/bioetl/infrastructure/storage/silver_writer.py",
    ),
    # REQ-PYTHON-003 — Optional (X | None / Optional)
    "REQ-PYTHON-003": (
        "pyproject.toml",
        "src/bioetl/infrastructure/storage/atomic.py",
    ),
    # REQ-TEST-003 — tests/fixtures/vcr/
    "REQ-TEST-003": (
        "tests/fixtures/vcr",
    ...
    # REQ-GOV-002 — golden; os.replace (atomic golden writes)
    "REQ-GOV-002": (
        "src/bioetl/infrastructure/storage/atomic.py",
    ...
    # REQ-GOV-009 — Delta; Pandera (governance of storage validation stack)
    "REQ-GOV-009": (
        "pyproject.toml",
        "src/bioetl/infrastructure/storage/delta_reader.py",
        "uv.lock",
    ...
    # REQ-CLEANUP-001 — async def aclose (resource cleanup)
    "REQ-CLEANUP-001": (
        "src/bioetl/infrastructure/adapters",
        "src/bioetl/infrastructure/storage/silver_writer.py",
    ),
    ...
    # REQ-DEP-001 — uv.lock; == (pinned dependencies)
    "REQ-DEP-001": (
        "uv.lock",
        "pyproject.toml",
    ),
...
def test_concrete_req_surfaces_exist() -> None:
    ...
    assert len(WAVE2_CONCRETE_SURFACES) == 26, (
        f"Wave2 must bind exactly 26 REQs, got {len(WAVE2_CONCRETE_SURFACES)}"
    )

    missing: list[str] = []
    for req_id, surfaces in sorted(WAVE2_CONCRETE_SURFACES.items()):
        assert surfaces, f"{req_id} has empty surface tuple"
        for rel in surfaces:
            abs_path = _ROOT / rel
            if not abs_path.exists():
                missing.append(f"{req_id}: {rel} -> not found at {abs_path}")

    assert not missing, (
        "Wave2 concrete REQ surfaces missing (bind to existing paths only):\n"
