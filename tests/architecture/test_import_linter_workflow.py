...
def test_import_linter_workflow_runs_for_all_pr_and_push_changes() -> None:
    ...
    workflow = Path(".github/workflows/import-linter.yml").read_text(
        encoding="utf-8"
    )
...
def test_import_linter_workflow_requires_full_capabilities_for_arch_tests() -> None:
    ...
    assert 'uv run pytest tests/architecture/ -m "not slow and not benchmark and not memory"' in workflow
