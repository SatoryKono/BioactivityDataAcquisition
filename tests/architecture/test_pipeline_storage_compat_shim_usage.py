...
@pytest.mark.architecture
def test_removed_pipeline_storage_compat_shims_are_not_used_in_src() -> None:
    """First-party source code must use canonical pipeline/storage modules directly."""
    violations = _iter_removed_compat_import_violations(SRC_ROOT)
...
@pytest.mark.architecture
def test_removed_pipeline_storage_compat_shims_are_not_used_in_tests() -> None:
    """Tests must not keep importing removed pipeline/storage compatibility modules."""
    violations = _iter_removed_compat_import_violations(TESTS_ROOT)
    assert not violations, (
