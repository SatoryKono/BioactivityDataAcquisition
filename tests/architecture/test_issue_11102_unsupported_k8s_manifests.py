...
def test_deployment_guide_does_not_apply_unsupported_manifests() -> None:
    ...
    deployment = (DEPLOYMENT / "k8s-deployment.yaml").read_text(encoding="utf-8")
    assert "change_me_to_random" not in deployment
    assert "BIOETL_DQ_HARD_THRESHOLD" not in deployment
    assert "BIOETL_DQ_SOFT_THRESHOLD" not in deployment
