...
def test_shell_wrapper_keeps_local_coverage_opt_in() -> None:
    ...
    assert 'DEFAULT_FLAGS=(--cov=src/bioetl --cov-report=term "${DEFAULT_FLAGS[@]}")' in content
...
def test_powershell_wrapper_keeps_local_coverage_opt_in() -> None:
    ...
    assert '$PytestArgs = @("--cov=src/bioetl", "--cov-report=term") + $PytestArgs' in content


def test_wrapper_autopreflight_scope_is_limited_to_full_repo_and_config_heavy_runs() -> None:
    shell = RUN_PYTEST_SH.read_text(encoding="utf-8")
    powershell = RUN_PYTEST_PS1.read_text(encoding="utf-8")
    shell_scope_block = shell[
        shell.index("should_run_preflight()") : shell.index("determine_preflight_scope()")
    ]
    powershell_scope_block = powershell[
        powershell.index("function Test-PreflightScope") : powershell.index("foreach ($Arg in $PytestArgs)")
    ]
