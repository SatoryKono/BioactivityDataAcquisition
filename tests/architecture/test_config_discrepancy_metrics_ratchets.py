...
@pytest.mark.parametrize(
    ...
    [
        ...
        "configs/quality/entity_filter_metadata_registry.yaml",
    ],
)
def test_config_discrepancy_fingerprint_covers_effective_loader_inputs(
    monkeypatch: pytest.MonkeyPatch,
    changed_path: str,
) -> None:
    original_read_bytes = Path.read_bytes
    baseline = generator._config_evidence_fingerprint()

    def read_with_change(path: Path) -> bytes:
        content = original_read_bytes(path)
        if path.as_posix() == changed_path:
            return content + b"\n# fingerprint invalidation probe\n"
        return content

    monkeypatch.setattr(Path, "read_bytes", read_with_change)

    assert generator._config_evidence_fingerprint() != baseline
