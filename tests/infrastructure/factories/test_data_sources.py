...
def test_create_pubchem_adapter(mock_http_client):
    ...
    with patch("bioetl.infrastructure.adapters.pubchem.client.PubChemClient") as MockPubChem:
        adapter_mock = Mock(spec=PubChemClient)
        MockPubChem.return_value = adapter_mock

        adapter = DataSourceFactory.create("pubchem", http_client=mock_http_client, rate=1.0)
...
def test_create_uniprot_adapter(mock_http_client):
    ...
    with patch("bioetl.infrastructure.adapters.uniprot.client.UniProtClient") as MockUniProt:
        adapter_mock = Mock(spec=UniProtClient)
        MockUniProt.return_value = adapter_mock

        adapter = DataSourceFactory.create("uniprot", http_client=mock_http_client, api_key="test_key")
