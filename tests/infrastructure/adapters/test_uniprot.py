...
async def test_health_check_degraded(uniprot_client):
    """Test health check returns UNHEALTHY on non-200.

    Note: UnifiedHTTPClient calls raise_for_status() for all responses,
    so non-2xx codes result in HTTPStatusError exceptions.
    This causes health_check to catch the exception and return UNHEALTHY.
    The DEGRADED state is determined by circuit breaker failure count,
    not by HTTP status codes directly.
    """
    respx.get("https://rest.uniprot.org/rest/beta/health").mock(
    ...
    async with uniprot_client:
        status = await uniprot_client.health_check()
    # 404 causes raise_for_status() -> exception -> UNHEALTHY
