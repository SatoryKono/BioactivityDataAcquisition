...
_MODULE_TIMEOUT_SECONDS = 180 if _WINDOWS_PLATFORM else 60

# Benchmark baselines can take materially longer on Windows-backed checkouts even
# when the measured operation medians remain in budget. Keep an explicit timeout
# for this module so perf assertions fail on budget drift instead of a generic
# 60-second pytest-timeout.
pytestmark = [
    pytest.mark.benchmark,
    pytest.mark.timeout(_MODULE_TIMEOUT_SECONDS),
]
...
class TestPolarsBaselines:
    ...
    GROUP_AGG_WARMUP_ITERATIONS = 3
    WINDOWS_GROUP_AGG_WARMUP_ITERATIONS = 2
    GROUP_AGG_MEASURE_ITERATIONS = 10
    WINDOWS_GROUP_AGG_MEASURE_ITERATIONS = 5
    ...
    def test_group_aggregate_baseline(
        ...
        warmup_iterations = (
            self.WINDOWS_GROUP_AGG_WARMUP_ITERATIONS
            if _WINDOWS_PLATFORM
            else self.GROUP_AGG_WARMUP_ITERATIONS
        )
        measure_iterations = (
            self.WINDOWS_GROUP_AGG_MEASURE_ITERATIONS
            if _WINDOWS_PLATFORM
            else self.GROUP_AGG_MEASURE_ITERATIONS
        )
        ...
        # Measure enough rounds for a stable median without hitting Windows timeout noise.
        times = []
