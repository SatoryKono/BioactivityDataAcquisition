...
Enforced: DASH-LAYOUT-003/004, DASH-FIT-002/003, DASH-COPY-004/005/006/007,
DASH-PERF-003. DASH-FIT-001 stays blocked until VIEWPORT_ROWS is calibrated.
DASH-COPY-003 stays advisory: new verbs must be canonical or pending.
"""
...
# --- DASH-COPY-003 advisory: verbs stay in the known sets -------------------


def test_content_panel_verbs_stay_in_known_sets() -> None:
    """Does not enforce the canonical set; blocks unknown new verbs."""
    allowed = CANONICAL_ACTION_VERBS | PENDING_ACTION_VERBS
    violations: list[str] = []
    for dashboard_path in get_dashboard_files():
        dashboard = load_dashboard(dashboard_path)
        for panel in get_dashboard_panels(dashboard):
            ...
            if verb not in allowed:
                violations.append(
                    f"{dashboard_path.name}: {title!r} (id={panel.get('id')}) "
                    f"starts with unknown verb {verb!r}"
                )
    assert not violations, "unknown action-first verbs:\n" + "\n".join(violations)
...
def test_fold_constants_are_distinct_and_named() -> None:
    ...
    assert VIEWPORT_ROWS is None or 1 <= VIEWPORT_ROWS <= FIRST_LOAD_Y_MAX
