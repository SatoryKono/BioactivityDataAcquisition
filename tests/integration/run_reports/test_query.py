...
def _write_simple(tmp_path: Path, *, run_id: str, silver: int) -> None:
    ...
    write_pipeline_run_report(report, root=tmp_path, store=FileRunReportStoreAdapter())
...
def test_load_helpers_handle_absent_non_mapping_and_invalid_json(
    ...
    assert list_pipeline_reports(root=tmp_path, store=FileRunReportStoreAdapter()) == []
    assert (
        load_latest_pointer(
            kind="pipeline",
            owner="missing",
    ...
    assert (
        load_pipeline_report(
            pipeline_name="missing", root=tmp_path, store=FileRunReportStoreAdapter()
        )
        is None
    )
...
def test_workflow_report_loads_direct_and_latest_targets(tmp_path: Path) -> None:
    ...
    implicit_latest = load_workflow_report(
        workflow_name="nightly", root=tmp_path, store=FileRunReportStoreAdapter()
    )
...
def test_listing_filters_entries_sorts_by_mtime_and_honors_limits(
    ...
    entries = list_pipeline_reports(
        root=tmp_path, limit=2, store=FileRunReportStoreAdapter()
    )
    ...
    assert (
        list_pipeline_reports(
            root=tmp_path, limit=-1, store=FileRunReportStoreAdapter()
        )
        == []
    )
    assert (
...
def test_prune_rejects_invalid_options(
    ...
    with pytest.raises(ValueError, match=message):
        prune_reports(root=tmp_path, **kwargs, store=FileRunReportStoreAdapter())
...
def test_workflow_prune_removes_recursive_duplicate_candidate_once(
    ...
    assert [
        entry.run_id
        for entry in list_workflow_reports(
...
def test_reports_for_prune_is_unbounded(tmp_path: Path, monkeypatch) -> None:
    ...
    entries = report_query._reports_for_prune(
        "pipeline", None, tmp_path, store=FileRunReportStoreAdapter()
    )
...
def test_identity_metadata_handles_os_error_and_non_mapping_payloads(
    ...
    assert read_identity_preview(tmp_path, store=FileRunReportStoreAdapter()) == empty

    payload_path = tmp_path / "report.json"
    payload_path.write_text("[]", encoding="utf-8")
    assert (
        read_identity_preview(payload_path, store=FileRunReportStoreAdapter()) == empty
    )
...
def test_identity_metadata_reads_started_at_and_workflow_run_id(
    ...
    assert read_identity_preview(
        payload_path, store=FileRunReportStoreAdapter()
    ) == IdentityIndexPreview(
        "success",
