...
Note: OpenAlex and SemanticScholar have their own vcr_config fixtures
...
CROSSREF_CASSETTE_DIR = FIXTURES_DIR / "crossref"
...
def vcr_config(request):
    ...
    test file name (e.g., test_chembl.py uses chembl/ directory).

    Note: OpenAlex and SemanticScholar adapters define their own vcr_config
    fixtures which take precedence over this one.
    """
    # Get test file name
    test_file = Path(request.fspath).name

    if "chembl" in test_file:
        return _get_vcr_config(CHEMBL_CASSETTE_DIR)
    elif "uniprot" in test_file:
        return _get_vcr_config(UNIPROT_CASSETTE_DIR)
    elif "pubmed" in test_file:
        return _get_vcr_config(PUBMED_CASSETTE_DIR)
    elif "pubchem" in test_file:
        return _get_vcr_config(PUBCHEM_CASSETTE_DIR)
    elif "crossref" in test_file:
        return _get_vcr_config(CROSSREF_CASSETTE_DIR)
    else:
