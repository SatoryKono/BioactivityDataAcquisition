...
CrossRef provides dates in various formats:
- Full: [[2024, 3, 15]] -> "2024-03-15"
- Partial month: [[2024, 3]] -> "2024-03-01"
- Partial year: [[2024]] -> "2024-01-01"
...
class TestCrossRefDateNormalization:
    ...
    async def test_partial_date_year_only_normalization(
        ...
        """Year-only dates ([[2024]]) should be normalized to YYYY-01-01."""
        record = make_crossref_record(published_print=[[2024]])

        result = await transformer.transform(pipeline_context, record, 0)

        pub_date = result.get("publication_date")
        assert pub_date == "2024-01-01", (
            f"Year-only date should normalize to YYYY-01-01, got {pub_date}"
        )
    ...
    async def test_partial_date_year_month_normalization(
        ...
        """Year-month dates ([[2024, 3]]) should be normalized to YYYY-MM-01."""
        ...
        assert pub_date == "2024-03-01", (
            f"Year-month date should normalize to YYYY-MM-01, got {pub_date}"
        )
...
class TestCrossRefDateEdgeCases:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # Partial print date (YYYY-MM)
            ("2024-03", None, "2024-03-01"),
            # Partial print date (YYYY)
            ("2024", None, "2024-01-01"),
            # Print is None, use online
            (None, "2024-03-15", "2024-03-15"),
            # Both partial
            ("2024-06", "2024-03", "2024-06-01"),
            # Both None
            (None, None, None),
        ],
    )
    def test_date_normalization_combinations(
        self,
        transformer: CrossRefPublicationTransformer,
        ...
        """Test date normalization with various input combinations."""
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # Partial YYYY
            ("2024", "2024-01-01"),
            # Weird format (should return as-is)
            ("2024-1", "2024-1"),
        ],
    )
    def test_partial_date_formats(
        self,
        transformer: CrossRefPublicationTransformer,
        date_str: str,
        expected: str,
    ) -> None:
        """Test various partial date format handling."""
...
class TestCrossRefVCRIntegration:
    ...
    async def test_real_api_partial_date_patterns(
        ...
        - [[2024, 3]] for year-month only (common!)
        - [[2024]] for year only
        """
        ...
        # Should be normalized to YYYY-MM-01
        pub_date = result.get("publication_date")
