...
def test_dq_current_status_splits_hard_failures_from_degraded_warnings() -> None:
    ...
    assert (
        ...
        == 'label_replace((bioetl_dq_monitor_disabled_current), "action_scope", '
        '"var-pipeline=$1&var-stage=$$__all", "pipeline", "([A-Za-z0-9_]+)")'
    )
    assert (
        quarantined_reason_rules[0].get("expr", "")
        == "label_replace((max by (pipeline) "
        "(max_over_time(bioetl_dq_records_quarantined_total[15m])) > bool 0), "
        '"action_scope", "var-pipeline=$1&var-stage=$$__all", '
        '"pipeline", "([A-Za-z0-9_]+)")'
    )
...
def test_dq_first_window_reason_record_keeps_status_gap_fallback() -> None:
    ...
    payload = _load_rules()
    expr = str(
        _build_record_map(payload)["bioetl_dq_first_window_reason"].get("expr", "")
    )
