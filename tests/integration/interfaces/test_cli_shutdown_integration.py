...
class TestRunnerShutdownIntegration:
    ...
    def test_runner_logs_graceful_shutdown(
        ...
        """Test that CLI shows graceful shutdown message."""
        from bioetl.application.services import RunResult, RunStatus
        ...
        # Should have shown shutdown message
        assert "shut down" in result.output.lower() or result.exit_code == 130
    ...
    def test_runner_shutdown_signal_passed_to_setup(
        ...
        """Test that service-based CLI correctly handles shutdown status."""
        from bioetl.application.services import RunResult, RunStatus
        ...
        # Service-based CLI handles shutdown via status, not signal handlers
        assert result.exit_code == 130
...
class TestLockReleaseOnShutdown:
    ...
    def test_lock_released_after_shutdown(
        ...
        """Test that CLI completes correctly on shutdown status."""
        from bioetl.application.services import RunResult, RunStatus
