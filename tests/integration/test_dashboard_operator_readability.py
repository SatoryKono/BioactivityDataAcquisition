...
def test_status_scope_tokens_keep_canonical_caps_in_authored_html() -> None:
    """All shipped HTML must not render status/scope tokens in mixed case."""
    violations: list[str] = []
    for path in get_dashboard_files():
        ...
        for panel in get_dashboard_panels(dashboard):
            ...
            if not html:
                continue
            visible = _STRIP_CODE_RE.sub("", html)
            visible = _STRIP_HREF_RE.sub("", visible)
            for token in STATUS_SCOPE_TOKENS:
                for match in re.finditer(re.escape(token), visible, flags=re.I):
                    if match.group() != token:
                        violations.append(
                            f"{path.name}:id={panel.get('id')} {match.group()!r} "
                            f"must be written {token!r}"
                        )
    ...
    for path in get_dashboard_files():
        ...
        for panel in first_window:
            ...
            for match in _OVERFLOW_RE.finditer(html):
                ...
                window = html[max(0, match.start() - 90) : match.end() + 40]
                compact = re.sub(r"\s+", "", window)
                spacer = "height:0" in compact and "aria-hidden" in window
                if kind == "hidden" and spacer:
                    continue
                violations.append(
