...
class TestValidatePipelineName:
    ...
    def test_valid_pipeline_ignores_retired_default_registry_fallback(
        self, mock_registry: MagicMock
    ) -> None:
        """Validator should build a fresh explicit registry, not default compat state."""
        click_context = MagicMock(spec=click.Context)
        click_context.obj = None
        ...
        assert result == "chembl_activity"
        assert click_context.obj is mock_registry
        mock_default_registry.assert_not_called()
