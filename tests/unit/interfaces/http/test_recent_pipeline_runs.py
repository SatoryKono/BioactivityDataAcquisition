...
def _list(root, **kwargs):
    ...
    catalog = kwargs.get("manifest_port", empty_catalog)
    if isinstance(catalog, Mock):
        catalog.get_by_run_id.side_effect = lambda run_id: next(
            (item for item in catalog.list_all() if item.run_id == run_id), None
        )
