...
class TestApplyJoinsSmartColumnRenaming:
    """Tests for _apply_joins with qualified column renaming."""

    @pytest.mark.asyncio
    async def test_always_uses_qualified_name(self, merge_service):
        """Test enricher columns are always renamed to {provider}.{entity}.{field}."""
        import polars as pl
        ...
        assert "crossref.title" not in result.columns
    ...
    async def test_conflict_after_prefixing_gets_suffixes(self, merge_service):
        ...
        # Seed already has qualified column
    ...
    async def test_secondary_join_keys_are_prefixed(self, merge_service):
        ...
        # Enricher also has doi and title
        enricher_df = pl.DataFrame(
            {
                "doi": ["10.1/a"],
        ...
        # Secondary key (title) should be qualified
        ...
        # Regular columns should also be qualified
    ...
    async def test_multiple_enrichers_secondary_keys_prefixed(self, merge_service):
        """Test multiple enrichers with secondary join keys all get qualified."""
        import polars as pl
        ...
        assert "title" in result.columns
        # Each enricher's title is qualified
        assert "crossref.publication.title" in result.columns
        assert "openalex.publication.title" in result.columns

    @pytest.mark.asyncio
    async def test_renames_even_if_no_seed_pipeline(self, merge_service):
        """Test qualified renaming even when seed_pipeline not provided."""
        import polars as pl
        ...
        # Should be qualified regardless of seed
        assert "crossref.publication.title" in result.columns
...
class TestGetEnricherPrefix:
    ...
    def test_qualified_prefix(self, merge_service):
        """Test prefix is always qualified."""
        prefix = merge_service._get_enricher_prefix(
            "crossref_publication", "chembl_publication"
        )
        assert prefix == "crossref.publication."

    def test_legacy_prefix_fallback(self, merge_service):
        """Test legacy prefix on invalid pipeline name."""
        prefix = merge_service._get_enricher_prefix("invalidname", None)
...
class TestExtractBaseColumn:
    ...
    def test_extracts_base_from_dot_prefix(self, merge_service):
        ...
        base = merge_service._extract_base_column(
            "crossref.publication.title", "crossref.publication."
        )
        assert base == "title"
