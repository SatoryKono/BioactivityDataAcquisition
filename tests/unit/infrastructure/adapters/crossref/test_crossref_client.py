...
async def test_fetch_by_doi_success(adapter, mock_http_client):
    """Test successful fetch_by_doi."""
    mock_response = MagicMock()
    mock_response.status_code = 200
    mock_response.json.return_value = {"message": {"title": ["Test Title"]}}
    mock_http_client.get.return_value = mock_response

    result = await adapter.fetch_by_doi("10.1234/test")

    assert result == {"title": ["Test Title"]}
    mock_http_client.get.assert_called_once_with(
        "https://api.crossref.org/works/10.1234/test",
        headers={"User-Agent": "BioETL/1.0 (mailto:test@example.com)"},
    )


@pytest.mark.asyncio
async def test_fetch_by_doi_not_found(adapter, mock_http_client, mock_logger):
    """Test fetch_by_doi when DOI is not found (404)."""
    mock_response = MagicMock()
    mock_response.status_code = 404
    mock_http_client.get.return_value = mock_response

    result = await adapter.fetch_by_doi("10.1234/notfound")
...
async def test_fetch_by_doi_http_error(adapter, mock_http_client, mock_logger):
    """Test fetch_by_doi with HTTP error (e.g., 500)."""
    mock_response = MagicMock()
    mock_response.status_code = 500
    mock_http_client.get.return_value = mock_response

    result = await adapter.fetch_by_doi("10.1234/error")
...
async def test_fetch_by_doi_request_error(adapter, mock_http_client, mock_logger):
    """Test fetch_by_doi with a request error (e.g., network issue)."""
    mock_http_client.get.side_effect = RequestError("Network error")

    result = await adapter.fetch_by_doi("10.1234/network-error")

    assert result is None
    mock_logger.error.assert_called_with(
        "crossref_request_failed",
        doi="10.1234/network-error",
        error="Network error",
    )
