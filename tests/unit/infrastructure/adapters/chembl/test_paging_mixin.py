...
class TestPageIteratorRefactoring:
    ...
    async def test_page_iterator_limit_exhausted(self):
        ...
        results = []
        async for page in self.mixin._page_iterator("activity", limit=1, start_offset=0):
            results.extend(page)
    ...
    async def test_page_iterator_no_limit(self):
        ...
        results = []
        async for page in self.mixin._page_iterator("activity", limit=None, start_offset=0):
            results.extend(page)
    ...
    async def test_page_iterator_empty_response(self):
        ...
        results = []
        async for page in self.mixin._page_iterator("activity", limit=10, start_offset=0):
            results.extend(page)
    ...
    async def test_page_iterator_preserves_params(self):
        ...
        async for _ in self.mixin._page_iterator("activity", limit=None, start_offset=0):
            break
