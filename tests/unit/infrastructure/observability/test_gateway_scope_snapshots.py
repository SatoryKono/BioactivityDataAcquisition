...
def _registry(run_type: str, *, workflow: bool = False) -> CollectorRegistry:
    ...
    Counter(
        "records", "Observed records", ["pipeline", "run_type"], registry=registry
    ).labels("chembl_assay", run_type).inc(1000)
    ...
    if workflow:
        Gauge(
            "workflow_status", "Workflow status", ["workflow"], registry=registry
        ).labels("chembl_baseline").set(0)
    return registry
...
def test_planned_workflow_snapshot_does_not_replace_pipeline_group() -> None:
    ...
    Gauge(
        ...
        registry=registry,
    ).labels("chembl_baseline", "chembl_assay", "backfill").set(1)
    snapshots = partition_snapshots(registry, job="bioetl")
...
def test_integrity_refresh_cannot_erase_an_unexecuted_scope() -> None:
    registry = _registry("incremental")
    Gauge(
        "bioetl_manifest_ledger_integrity_ratio",
        "Persisted integrity",
        ["pipeline", "run_type", "integrity_type"],
        registry=registry,
    ).labels("chembl_assay", "backfill", "consistent").set(1)
    snapshots = partition_snapshots(registry, job="bioetl")
    assert not any(
        job == "bioetl" and group.get("run_type") == "backfill"
