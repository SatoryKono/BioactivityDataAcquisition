...
def test_empirical_payload_persists_seeds_and_reconciles_stable_runs(
            ...
            json.dumps(
                {
                    ...
                    "execution_order": [
                        ...
                        f"tests.test_order::test_{seed}",
                    ],
                    "artifact_hashes": {
                        "replay_tree_before_sha256": "replay",
                        "replay_tree_after_sha256": "replay",
                        "generated_artifacts_before_sha256": "generated",
                        "generated_artifacts_after_sha256": "generated",
                    },
                }
            ),
            encoding="utf-8",
    ...
    assert payload["comparison"]["generated_artifact_fingerprint_stable"] is True
    assert payload["comparison"]["execution_order_changed"] is True
    assert payload["curated_inventory_reconciliation"]["untriaged_count"] == 0


def test_empirical_payload_detects_per_run_replay_mutation(tmp_path: Path) -> None:
    _write_inputs(tmp_path)
    run_dir = tmp_path / "runs"
    run_dir.mkdir()
    for index, seed in enumerate((17, 73, 113), start=1):
            ...
            json.dumps(
                {
                    ...
                    "seed": seed,
                    "source_sha": "abc123",
                    "execution_order": [f"tests.test_x::test_{seed}"],
                    "artifact_hashes": {
                        "replay_tree_before_sha256": "before",
                        "replay_tree_after_sha256": (
                            "mutated" if seed == 73 else "before"
                        ),
                        "generated_artifacts_before_sha256": "generated",
                        "generated_artifacts_after_sha256": "generated",
                    },
