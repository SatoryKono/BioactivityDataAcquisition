...
def test_fallback_rows_include_unprofiled_entity_fields() -> None:
    ...
    assert any(
        ...
        and row["normalization_source"] == "fallback"
        for row in rows
    )
...
def test_build_payload_summarizes_fallback_rows() -> None:
    ...
    assert payload["pipelines_with_fallback_count"] == 2
...
def test_render_markdown_mentions_top_fallback_entries() -> None:
    markdown = _render_markdown(
        {
            ...
            "normalizers": [{"normalizer": "normalize_title", "field_count": 2}],
            "entries": [
                {
                    "pipeline_name": "openalex_publication",
                    "field_name": "title",
    ...
    assert "`openalex_publication.title` -> `normalize_title`" in markdown
