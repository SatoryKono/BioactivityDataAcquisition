...
@pytest.mark.unit
def test_initialize_protein_class_target_type_mapping_caches_loader_per_configs_root() -> (
    None
):
...
def test_runtime_data_caches_are_scoped_by_configs_root() -> None:
    ...
    with (
        ...
        initialize_protein_class_target_type_mapping(Path("configs"))
        initialize_protein_class_target_type_mapping(Path("/tmp/other-configs"))

    assert mock_chembl_load.call_count == 2
    assert mock_classification_load.call_count == 2
    assert mock_protein_class_load.call_count == 2
