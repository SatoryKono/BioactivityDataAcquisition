...
def test_post_lock_finalization_uses_launch_context(tmp_path, resume):
    ...
    launch = PipelineRunContext(
        pipeline_name="chembl_assay", run_id=run_id, run_type=RunType.BACKFILL,
        manifest_id=manifest_id, contract_ref="chembl.assay",
        contract_schema_hash="a" * 64, resume=resume,
    )
    runner = object.__new__(PipelineRunner)
    runner._context = PipelineContext(
        run_id=run_id, run_type=RunType.BACKFILL, logger=NoOpLogger(),
    ...
    payload = json.loads(
        (tmp_path / "run_manifest" / f"{manifest_id}.contract-evidence.json").read_text()
    )
