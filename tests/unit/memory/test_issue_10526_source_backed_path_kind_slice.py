"""Ratchet coverage for AUD-002 source_backed_path_kind extract (#10526)."""
...
from memory.graph.sync_pkg import source_backed_path_kind as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "source_backed_path_kind.py"
_CORE_LOC_BEFORE = 9378
_NAME = "_source_backed_path_kind"
_OWNED = re.compile(
...
def test_source_backed_path_kind_owns_symbols_and_shrinks_core() -> None:
