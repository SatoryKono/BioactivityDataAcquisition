"""Ratchet coverage for AUD-002 policy_governance_targets extract (#10526)."""
...
from memory.graph.sync_pkg import policy_governance_targets as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "policy_governance_targets.py"
_CORE_LOC_BEFORE = 4688
_NAME = "_policy_governance_targets"
_OWNED = re.compile(
...
def test_policy_governance_targets_owns_symbols_and_shrinks_core() -> None:
