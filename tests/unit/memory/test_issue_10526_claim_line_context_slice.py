"""Ratchet coverage for AUD-002 claim_line_context extract (#10526)."""
...
from memory.graph.sync_pkg import claim_line_context as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "claim_line_context.py"
_CORE_LOC_BEFORE = 5075
_NAME = "_claim_line_context"
_OWNED = re.compile(
...
def test_claim_line_context_owns_symbols_and_shrinks_core() -> None:
