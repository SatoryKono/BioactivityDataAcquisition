"""Ratchet coverage for AUD-002 storage_surface_state extract (#10526)."""
...
from memory.graph.sync_pkg import storage_surface_state as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "storage_surface_state.py"
_CORE_LOC_BEFORE = 12024
_NAME = "_storage_surface_state"
_OWNED = re.compile(
...
def test_storage_surface_state_owns_symbols_and_shrinks_core() -> None:
