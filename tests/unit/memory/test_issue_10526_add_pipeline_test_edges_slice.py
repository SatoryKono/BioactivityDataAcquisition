"""Ratchet coverage for AUD-002 add_pipeline_test_edges extract (#10526)."""
...
from memory.graph.sync_pkg import add_pipeline_test_edges as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "add_pipeline_test_edges.py"
_CORE_LOC_BEFORE = 2915
_NAME = "_add_pipeline_test_edges"
_OWNED = re.compile(
...
def test_add_pipeline_test_edges_owns_symbols_and_shrinks_core() -> None:
