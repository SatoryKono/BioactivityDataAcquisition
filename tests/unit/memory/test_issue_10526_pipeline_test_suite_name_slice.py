"""Ratchet coverage for AUD-002 pipeline_test_suite_name extract (#10526)."""
...
from memory.graph.sync_pkg import pipeline_test_suite_name as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "pipeline_test_suite_name.py"
_CORE_LOC_BEFORE = 3125
_NAME = "_pipeline_test_suite_name"
_OWNED = re.compile(
...
def test_pipeline_test_suite_name_owns_symbols_and_shrinks_core() -> None:
