"""Ratchet coverage for AUD-002 adr_constraint_candidates extract (#10526)."""
...
from memory.graph.sync_pkg import adr_constraint_candidates as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "adr_constraint_candidates.py"
_CORE_LOC_BEFORE = 7369
_NAME = "_adr_constraint_candidates"
_OWNED = re.compile(
...
def test_adr_constraint_candidates_owns_symbols_and_shrinks_core() -> None:
