"""Ratchet coverage for AUD-002 complexity_surface_scores extract (#10526)."""
...
from memory.graph.sync_pkg import complexity_surface_scores as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "complexity_surface_scores.py"
_CORE_LOC_BEFORE = 4406
_NAME = "_complexity_surface_scores"
_OWNED = re.compile(
...
def test_complexity_surface_scores_owns_symbols_and_shrinks_core() -> None:
