"""Ratchet coverage for AUD-002 provider_config_properties extract (#10526)."""
...
from memory.graph.sync_pkg import provider_config_properties as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "provider_config_properties.py"
_CORE_LOC_BEFORE = 5907
_NAME = "_provider_config_properties"
_OWNED = re.compile(
...
def test_provider_config_properties_owns_symbols_and_shrinks_core() -> None:
