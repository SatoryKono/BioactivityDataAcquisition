"""Ratchet coverage for AUD-002 merge_field_validation_item extract (#10526)."""
...
from memory.graph.sync_pkg import merge_field_validation_item as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "merge_field_validation_item.py"
_CORE_LOC_BEFORE = 11924
_NAME = "_merge_field_validation_item"
_OWNED = re.compile(
...
def test_merge_field_validation_item_owns_symbols_and_shrinks_core() -> None:
