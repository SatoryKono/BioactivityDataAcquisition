"""Ratchet coverage for AUD-002 workflow_output_expression extract (#10526)."""
...
from memory.graph.sync_pkg import workflow_output_expression as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "workflow_output_expression.py"
_CORE_LOC_BEFORE = 12901
_NAME = "_workflow_output_expression"
_OWNED = re.compile(
...
def test_workflow_output_expression_owns_symbols_and_shrinks_core() -> None:
