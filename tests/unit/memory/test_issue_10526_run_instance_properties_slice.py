"""Ratchet coverage for AUD-002 run_instance_properties extract (#10526)."""
...
from memory.graph.sync_pkg import run_instance_properties as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "run_instance_properties.py"
_CORE_LOC_BEFORE = 9322
_NAME = "_run_instance_properties"
_OWNED = re.compile(
...
def test_run_instance_properties_owns_symbols_and_shrinks_core() -> None:
