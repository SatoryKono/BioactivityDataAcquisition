"""Ratchet coverage for AUD-002 process_workflow_job extract (#10526)."""
...
from memory.graph.sync_pkg import process_workflow_job as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "process_workflow_job.py"
_CORE_LOC_BEFORE = 3499
_NAME = "_process_workflow_job"
_OWNED = re.compile(
...
def test_process_workflow_job_owns_symbols_and_shrinks_core() -> None:
