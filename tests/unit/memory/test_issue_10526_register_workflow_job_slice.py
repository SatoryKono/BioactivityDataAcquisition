"""Ratchet coverage for AUD-002 register_workflow_job extract (#10526)."""
...
from memory.graph.sync_pkg import register_workflow_job as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "register_workflow_job.py"
_CORE_LOC_BEFORE = 4058
_NAME = "_register_workflow_job"
_OWNED = re.compile(
...
def test_register_workflow_job_owns_symbols_and_shrinks_core() -> None:
