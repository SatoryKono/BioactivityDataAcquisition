"""Ratchet coverage for AUD-002 add_secret_requirements extract (#10526)."""
...
from memory.graph.sync_pkg import add_secret_requirements as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "add_secret_requirements.py"
_CORE_LOC_BEFORE = 10742
_NAME = "_add_secret_requirements"
_OWNED = re.compile(
...
def test_add_secret_requirements_owns_symbols_and_shrinks_core() -> None:
