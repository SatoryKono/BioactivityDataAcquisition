"""Ratchet coverage for AUD-002 resolved_base_classes extract (#10526)."""
...
from memory.graph.sync_pkg import resolved_base_classes as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "resolved_base_classes.py"
_CORE_LOC_BEFORE = 7297
_NAME = "_resolved_base_classes"
_OWNED = re.compile(
...
def test_resolved_base_classes_owns_symbols_and_shrinks_core() -> None:
