"""Ratchet coverage for AUD-002 add_curated_doc_source extract (#10526)."""
...
from memory.graph.sync_pkg import add_curated_doc_source as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "add_curated_doc_source.py"
_CORE_LOC_BEFORE = 6608
_NAME = "_add_curated_doc_source"
_OWNED = re.compile(
...
def test_add_curated_doc_source_owns_symbols_and_shrinks_core() -> None:
