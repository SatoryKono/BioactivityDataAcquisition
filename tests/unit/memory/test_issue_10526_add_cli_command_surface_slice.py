"""Ratchet coverage for AUD-002 add_cli_command_surface extract (#10526)."""
...
from memory.graph.sync_pkg import add_cli_command_surface as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "add_cli_command_surface.py"
_CORE_LOC_BEFORE = 12661
_NAME = "_add_cli_command_surface"
_OWNED = re.compile(
...
def test_add_cli_command_surface_owns_symbols_and_shrinks_core() -> None:
