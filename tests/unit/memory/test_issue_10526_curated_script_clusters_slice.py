"""Ratchet coverage for AUD-002 curated_script_clusters extract (#10526)."""
...
from memory.graph.sync_pkg import curated_script_clusters as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "curated_script_clusters.py"
_CORE_LOC_BEFORE = 13460
_NAME = "CURATED_SCRIPT_CLUSTERS"
_OWNED = re.compile(
...
def test_curated_script_clusters_owns_symbols_and_shrinks_core() -> None:
