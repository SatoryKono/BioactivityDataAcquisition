"""Ratchet coverage for AUD-002 pipelinetestcontext extract (#10526)."""
...
from memory.graph.sync_pkg import pipelinetestcontext as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "pipelinetestcontext.py"
_CORE_LOC_BEFORE = 3450
_NAME = "PipelineTestContext"
_OWNED = re.compile(
...
def test_pipelinetestcontext_owns_symbols_and_shrinks_core() -> None:
