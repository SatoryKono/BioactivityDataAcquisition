"""Ratchet coverage for AUD-002 alert_rule_file_payload extract (#10526)."""
...
from memory.graph.sync_pkg import alert_rule_file_payload as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "alert_rule_file_payload.py"
_CORE_LOC_BEFORE = 9495
_NAME = "_alert_rule_file_payload"
_OWNED = re.compile(
...
def test_alert_rule_file_payload_owns_symbols_and_shrinks_core() -> None:
