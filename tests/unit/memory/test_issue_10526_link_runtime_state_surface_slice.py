"""Ratchet coverage for AUD-002 link_runtime_state_surface extract (#10526)."""
...
from memory.graph.sync_pkg import link_runtime_state_surface as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "link_runtime_state_surface.py"
_CORE_LOC_BEFORE = 3593
_NAME = "_link_runtime_state_surface"
_OWNED = re.compile(
...
def test_link_runtime_state_surface_owns_symbols_and_shrinks_core() -> None:
