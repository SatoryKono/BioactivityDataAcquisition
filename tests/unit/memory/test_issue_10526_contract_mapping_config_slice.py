"""Ratchet coverage for AUD-002 contract_mapping_config extract (#10526)."""
...
from memory.graph.sync_pkg import contract_mapping_config as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "contract_mapping_config.py"
_CORE_LOC_BEFORE = 4616
_NAME = "_contract_mapping_config"
_OWNED = re.compile(
...
def test_contract_mapping_config_owns_symbols_and_shrinks_core() -> None:
