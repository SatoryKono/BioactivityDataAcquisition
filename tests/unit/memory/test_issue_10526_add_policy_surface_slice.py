"""Ratchet coverage for AUD-002 add_policy_surface extract (#10526)."""
...
from memory.graph.sync_pkg import add_policy_surface as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "add_policy_surface.py"
_CORE_LOC_BEFORE = 6869
_NAME = "_add_policy_surface"
_OWNED = re.compile(
...
def test_add_policy_surface_owns_symbols_and_shrinks_core() -> None:
