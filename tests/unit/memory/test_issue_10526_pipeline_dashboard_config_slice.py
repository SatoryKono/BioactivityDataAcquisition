"""Ratchet coverage for AUD-002 pipeline_dashboard_config extract (#10526)."""
...
from memory.graph.sync_pkg import pipeline_dashboard_config as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "pipeline_dashboard_config.py"
_CORE_LOC_BEFORE = 9978
_NAME = "_pipeline_dashboard_config"
_OWNED = re.compile(
...
def test_pipeline_dashboard_config_owns_symbols_and_shrinks_core() -> None:
