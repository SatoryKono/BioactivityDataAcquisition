"""Ratchet coverage for AUD-002 add_file_structure_surfaces extract (#10526)."""
...
from memory.graph.sync_pkg import add_file_structure_surfaces as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "add_file_structure_surfaces.py"
_CORE_LOC_BEFORE = 3791
_NAME = "_add_file_structure_surfaces"
_OWNED = re.compile(
...
def test_add_file_structure_surfaces_owns_symbols_and_shrinks_core() -> None:
