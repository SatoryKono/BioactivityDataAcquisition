"""Ratchet coverage for AUD-002 snapshotrelationindex extract (#10526)."""
...
from memory.graph.sync_pkg import snapshotrelationindex as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "snapshotrelationindex.py"
_CORE_LOC_BEFORE = 11196
_NAME = "_SnapshotRelationIndex"
_OWNED = re.compile(
...
def test_snapshotrelationindex_owns_symbols_and_shrinks_core() -> None:
