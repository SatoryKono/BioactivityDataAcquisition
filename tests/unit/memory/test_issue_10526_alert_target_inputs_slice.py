"""Ratchet coverage for AUD-002 alert_target_inputs extract (#10526)."""
...
from memory.graph.sync_pkg import alert_target_inputs as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "alert_target_inputs.py"
_CORE_LOC_BEFORE = 3442
_NAME = "_alert_target_inputs"
_OWNED = re.compile(
...
def test_alert_target_inputs_owns_symbols_and_shrinks_core() -> None:
