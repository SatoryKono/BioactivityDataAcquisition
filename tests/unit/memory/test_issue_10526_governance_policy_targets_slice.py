"""Ratchet coverage for AUD-002 governance_policy_targets extract (#10526)."""
...
from memory.graph.sync_pkg import governance_policy_targets as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "governance_policy_targets.py"
_CORE_LOC_BEFORE = 5633
_NAME = "_governance_policy_targets"
_OWNED = re.compile(
...
def test_governance_policy_targets_owns_symbols_and_shrinks_core() -> None:
