"""Ratchet coverage for AUD-002 retirement_analysis_context extract (#10526)."""
...
from memory.graph.sync_pkg import retirement_analysis_context as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "retirement_analysis_context.py"
_CORE_LOC_BEFORE = 8631
_NAME = "_retirement_analysis_context"
_OWNED = re.compile(
...
def test_retirement_analysis_context_owns_symbols_and_shrinks_core() -> None:
