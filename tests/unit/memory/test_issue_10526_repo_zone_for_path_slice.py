"""Ratchet coverage for AUD-002 repo_zone_for_path extract (#10526)."""
...
from memory.graph.sync_pkg import repo_zone_for_path as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "repo_zone_for_path.py"
_CORE_LOC_BEFORE = 5486
_NAME = "_repo_zone_for_path"
_OWNED = re.compile(
...
def test_repo_zone_for_path_owns_symbols_and_shrinks_core() -> None:
