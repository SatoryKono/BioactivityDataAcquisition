"""Ratchet coverage for AUD-002 curated_policy_surfaces extract (#10526)."""
...
from memory.graph.sync_pkg import curated_policy_surfaces as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "curated_policy_surfaces.py"
_CORE_LOC_BEFORE = 13574
_NAME = "CURATED_POLICY_SURFACES"
_OWNED = re.compile(
...
def test_curated_policy_surfaces_owns_symbols_and_shrinks_core() -> None:
