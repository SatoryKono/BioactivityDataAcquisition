"""Ratchet coverage for AUD-002 link_alert_targets extract (#10526)."""
...
from memory.graph.sync_pkg import link_alert_targets as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "link_alert_targets.py"
_CORE_LOC_BEFORE = 3435
_NAME = "_link_alert_targets"
_OWNED = re.compile(
...
def test_link_alert_targets_owns_symbols_and_shrinks_core() -> None:
