"""Ratchet coverage for AUD-002 walk_repo_zone_root extract (#10526)."""
...
from memory.graph.sync_pkg import walk_repo_zone_root as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "walk_repo_zone_root.py"
_CORE_LOC_BEFORE = 5510
_NAME = "_walk_repo_zone_root"
_OWNED = re.compile(
...
def test_walk_repo_zone_root_owns_symbols_and_shrinks_core() -> None:
