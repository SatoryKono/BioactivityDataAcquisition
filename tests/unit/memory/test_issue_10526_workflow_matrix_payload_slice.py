"""Ratchet coverage for AUD-002 workflow_matrix_payload extract (#10526)."""
...
from memory.graph.sync_pkg import workflow_matrix_payload as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "workflow_matrix_payload.py"
_CORE_LOC_BEFORE = 6391
_NAME = "_workflow_matrix_payload"
_OWNED = re.compile(
...
def test_workflow_matrix_payload_owns_symbols_and_shrinks_core() -> None:
