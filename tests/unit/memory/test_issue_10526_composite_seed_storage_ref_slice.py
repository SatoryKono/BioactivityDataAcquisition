"""Ratchet coverage for AUD-002 composite_seed_storage_ref extract (#10526)."""
...
from memory.graph.sync_pkg import composite_seed_storage_ref as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "composite_seed_storage_ref.py"
_CORE_LOC_BEFORE = 8495
_NAME = "_composite_seed_storage_ref"
_OWNED = re.compile(
...
def test_composite_seed_storage_ref_owns_symbols_and_shrinks_core() -> None:
