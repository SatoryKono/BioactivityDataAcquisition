"""Ratchet coverage for AUD-002 process_adapter_module extract (#10526)."""
...
from memory.graph.sync_pkg import process_adapter_module as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "process_adapter_module.py"
_CORE_LOC_BEFORE = 10587
_NAME = "_process_adapter_module"
_OWNED = re.compile(
...
def test_process_adapter_module_owns_symbols_and_shrinks_core() -> None:
