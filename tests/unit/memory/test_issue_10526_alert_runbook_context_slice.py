"""Ratchet coverage for AUD-002 alert_runbook_context extract (#10526)."""
...
from memory.graph.sync_pkg import alert_runbook_context as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "alert_runbook_context.py"
_CORE_LOC_BEFORE = 4706
_NAME = "_alert_runbook_context"
_OWNED = re.compile(
...
def test_alert_runbook_context_owns_symbols_and_shrinks_core() -> None:
