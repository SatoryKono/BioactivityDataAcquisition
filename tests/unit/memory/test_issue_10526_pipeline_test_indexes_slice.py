"""Ratchet coverage for AUD-002 pipeline_test_indexes extract (#10526)."""
...
from memory.graph.sync_pkg import pipeline_test_indexes as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "pipeline_test_indexes.py"
_CORE_LOC_BEFORE = 2939
_NAME = "_pipeline_test_indexes"
_OWNED = re.compile(
...
def test_pipeline_test_indexes_owns_symbols_and_shrinks_core() -> None:
