"""Ratchet coverage for AUD-002 file_structure_zone_roots extract (#10526)."""
...
from memory.graph.sync_pkg import file_structure_zone_roots as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "file_structure_zone_roots.py"
_CORE_LOC_BEFORE = 5275
_NAME = "_file_structure_zone_roots"
_OWNED = re.compile(
...
def test_file_structure_zone_roots_owns_symbols_and_shrinks_core() -> None:
