"""Ratchet coverage for AUD-002 evaluate_complexity_surface extract (#10526)."""
...
from memory.graph.sync_pkg import evaluate_complexity_surface as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "evaluate_complexity_surface.py"
_CORE_LOC_BEFORE = 4325
_NAME = "_evaluate_complexity_surface"
_OWNED = re.compile(
...
def test_evaluate_complexity_surface_owns_symbols_and_shrinks_core() -> None:
