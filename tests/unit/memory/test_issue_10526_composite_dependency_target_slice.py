"""Ratchet coverage for AUD-002 composite_dependency_target extract (#10526)."""
...
from memory.graph.sync_pkg import composite_dependency_target as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "composite_dependency_target.py"
_CORE_LOC_BEFORE = 8539
_NAME = "_composite_dependency_target"
_OWNED = re.compile(
...
def test_composite_dependency_target_owns_symbols_and_shrinks_core() -> None:
