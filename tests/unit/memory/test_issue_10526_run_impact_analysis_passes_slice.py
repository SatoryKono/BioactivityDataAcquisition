"""Ratchet coverage for AUD-002 run_impact_analysis_passes extract (#10526)."""
...
from memory.graph.sync_pkg import run_impact_analysis_passes as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "run_impact_analysis_passes.py"
_CORE_LOC_BEFORE = 2893
_NAME = "_run_impact_analysis_passes"
_OWNED = re.compile(
...
def test_run_impact_analysis_passes_owns_symbols_and_shrinks_core() -> None:
