"""Ratchet coverage for AUD-002 policysurfacecontext extract (#10526)."""
...
from memory.graph.sync_pkg import policysurfacecontext as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "policysurfacecontext.py"
_CORE_LOC_BEFORE = 4668
_NAME = "PolicySurfaceContext"
_OWNED = re.compile(
...
def test_policysurfacecontext_owns_symbols_and_shrinks_core() -> None:
