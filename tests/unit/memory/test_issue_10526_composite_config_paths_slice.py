"""Ratchet coverage for AUD-002 composite_config_paths extract (#10526)."""
...
from memory.graph.sync_pkg import composite_config_paths as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "composite_config_paths.py"
_CORE_LOC_BEFORE = 6037
_NAME = "_composite_config_paths"
_OWNED = re.compile(
...
def test_composite_config_paths_owns_symbols_and_shrinks_core() -> None:
