"""Ratchet coverage for AUD-002 contains_any extract (#10526)."""
...
from memory.graph.sync_pkg import contains_any as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "contains_any.py"
_CORE_LOC_BEFORE = 6322
_NAME = "_contains_any"
_OWNED = re.compile(
...
def test_contains_any_owns_symbols_and_shrinks_core() -> None:
