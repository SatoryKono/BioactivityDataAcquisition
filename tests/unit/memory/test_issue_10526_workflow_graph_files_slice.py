"""Ratchet coverage for AUD-002 workflow_graph_files extract (#10526)."""
...
from memory.graph.sync_pkg import workflow_graph_files as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "workflow_graph_files.py"
_CORE_LOC_BEFORE = 2996
_NAME = "_workflow_graph_files"
_OWNED = re.compile(
...
def test_workflow_graph_files_owns_symbols_and_shrinks_core() -> None:
