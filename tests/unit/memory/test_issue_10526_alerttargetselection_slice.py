"""Ratchet coverage for AUD-002 alerttargetselection extract (#10526)."""
...
from memory.graph.sync_pkg import alerttargetselection as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "alerttargetselection.py"
_CORE_LOC_BEFORE = 4253
_NAME = "AlertTargetSelection"
_OWNED = re.compile(
...
def test_alerttargetselection_owns_symbols_and_shrinks_core() -> None:
