"""Ratchet coverage for AUD-002 add_adr_decision_node extract (#10526)."""
...
from memory.graph.sync_pkg import add_adr_decision_node as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "add_adr_decision_node.py"
_CORE_LOC_BEFORE = 4935
_NAME = "_add_adr_decision_node"
_OWNED = re.compile(
...
def test_add_adr_decision_node_owns_symbols_and_shrinks_core() -> None:
