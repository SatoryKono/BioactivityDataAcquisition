"""Ratchet coverage for AUD-002 add_ci_workflow_graph extract (#10526)."""
...
from memory.graph.sync_pkg import add_ci_workflow_graph as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "add_ci_workflow_graph.py"
_CORE_LOC_BEFORE = 2963
_NAME = "_add_ci_workflow_graph"
_OWNED = re.compile(
...
def test_add_ci_workflow_graph_owns_symbols_and_shrinks_core() -> None:
