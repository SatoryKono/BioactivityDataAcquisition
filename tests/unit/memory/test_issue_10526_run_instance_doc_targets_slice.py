"""Ratchet coverage for AUD-002 run_instance_doc_targets extract (#10526)."""
...
from memory.graph.sync_pkg import run_instance_doc_targets as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "run_instance_doc_targets.py"
_CORE_LOC_BEFORE = 9266
_NAME = "_run_instance_doc_targets"
_OWNED = re.compile(
...
def test_run_instance_doc_targets_owns_symbols_and_shrinks_core() -> None:
