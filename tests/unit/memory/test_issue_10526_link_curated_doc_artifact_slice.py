"""Ratchet coverage for AUD-002 link_curated_doc_artifact extract (#10526)."""
...
from memory.graph.sync_pkg import link_curated_doc_artifact as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "link_curated_doc_artifact.py"
_CORE_LOC_BEFORE = 12125
_NAME = "_link_curated_doc_artifact"
_OWNED = re.compile(
...
def test_link_curated_doc_artifact_owns_symbols_and_shrinks_core() -> None:
