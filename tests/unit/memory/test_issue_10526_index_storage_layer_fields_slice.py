"""Ratchet coverage for AUD-002 index_storage_layer_fields extract (#10526)."""
...
from memory.graph.sync_pkg import index_storage_layer_fields as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "index_storage_layer_fields.py"
_CORE_LOC_BEFORE = 7583
_NAME = "_index_storage_layer_fields"
_OWNED = re.compile(
...
def test_index_storage_layer_fields_owns_symbols_and_shrinks_core() -> None:
