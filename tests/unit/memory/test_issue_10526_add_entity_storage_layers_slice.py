"""Ratchet coverage for AUD-002 add_entity_storage_layers extract (#10526)."""
...
from memory.graph.sync_pkg import add_entity_storage_layers as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "add_entity_storage_layers.py"
_CORE_LOC_BEFORE = 3709
_NAME = "_add_entity_storage_layers"
_OWNED = re.compile(
...
def test_add_entity_storage_layers_owns_symbols_and_shrinks_core() -> None:
