"""Ratchet coverage for AUD-002 existing_snapshot_nodes extract (#10526)."""
...
from memory.graph.sync_pkg import existing_snapshot_nodes as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "existing_snapshot_nodes.py"
_CORE_LOC_BEFORE = 5139
_NAME = "_existing_snapshot_nodes"
_OWNED = re.compile(
...
def test_existing_snapshot_nodes_owns_symbols_and_shrinks_core() -> None:
