"""Ratchet coverage for AUD-002 composite_pipeline_name extract (#10526)."""
...
from memory.graph.sync_pkg import composite_pipeline_name as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "composite_pipeline_name.py"
_CORE_LOC_BEFORE = 6997
_NAME = "_composite_pipeline_name"
_OWNED = re.compile(
...
def test_composite_pipeline_name_owns_symbols_and_shrinks_core() -> None:
