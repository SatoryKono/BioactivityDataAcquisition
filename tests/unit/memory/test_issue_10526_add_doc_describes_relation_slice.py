"""Ratchet coverage for AUD-002 add_doc_describes_relation extract (#10526)."""
...
from memory.graph.sync_pkg import add_doc_describes_relation as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "add_doc_describes_relation.py"
_CORE_LOC_BEFORE = 4958
_NAME = "_add_doc_describes_relation"
_OWNED = re.compile(
...
def test_add_doc_describes_relation_owns_symbols_and_shrinks_core() -> None:
