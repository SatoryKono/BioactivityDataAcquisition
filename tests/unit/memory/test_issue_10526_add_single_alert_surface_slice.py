"""Ratchet coverage for AUD-002 add_single_alert_surface extract (#10526)."""
...
from memory.graph.sync_pkg import add_single_alert_surface as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "add_single_alert_surface.py"
_CORE_LOC_BEFORE = 3407
_NAME = "_add_single_alert_surface"
_OWNED = re.compile(
...
def test_add_single_alert_surface_owns_symbols_and_shrinks_core() -> None:
