"""Ratchet coverage for AUD-002 batch_pipeline_names extract (#10526)."""
...
from memory.graph.sync_pkg import batch_pipeline_names as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "batch_pipeline_names.py"
_CORE_LOC_BEFORE = 7233
_NAME = "_batch_pipeline_names"
_OWNED = re.compile(
...
def test_batch_pipeline_names_owns_symbols_and_shrinks_core() -> None:
