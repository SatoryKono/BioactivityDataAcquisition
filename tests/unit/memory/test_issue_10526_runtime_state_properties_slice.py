"""Ratchet coverage for AUD-002 runtime_state_properties extract (#10526)."""
...
from memory.graph.sync_pkg import runtime_state_properties as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "runtime_state_properties.py"
_CORE_LOC_BEFORE = 8019
_NAME = "_runtime_state_properties"
_OWNED = re.compile(
...
def test_runtime_state_properties_owns_symbols_and_shrinks_core() -> None:
