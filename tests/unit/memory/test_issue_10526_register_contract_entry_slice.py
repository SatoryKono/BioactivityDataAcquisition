"""Ratchet coverage for AUD-002 register_contract_entry extract (#10526)."""
...
from memory.graph.sync_pkg import register_contract_entry as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "register_contract_entry.py"
_CORE_LOC_BEFORE = 4496
_NAME = "_register_contract_entry"
_OWNED = re.compile(
...
def test_register_contract_entry_owns_symbols_and_shrinks_core() -> None:
