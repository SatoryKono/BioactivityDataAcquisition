"""Ratchet coverage for AUD-002 normalization_statement extract (#10526)."""
...
from memory.graph.sync_pkg import normalization_statement as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "normalization_statement.py"
_CORE_LOC_BEFORE = 7203
_NAME = "_normalization_statement"
_OWNED = re.compile(
...
def test_normalization_statement_owns_symbols_and_shrinks_core() -> None:
