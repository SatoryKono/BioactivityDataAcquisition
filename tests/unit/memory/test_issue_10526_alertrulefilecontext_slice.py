"""Ratchet coverage for AUD-002 alertrulefilecontext extract (#10526)."""
...
from memory.graph.sync_pkg import alertrulefilecontext as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "alertrulefilecontext.py"
_CORE_LOC_BEFORE = 4042
_NAME = "AlertRuleFileContext"
_OWNED = re.compile(
...
def test_alertrulefilecontext_owns_symbols_and_shrinks_core() -> None:
