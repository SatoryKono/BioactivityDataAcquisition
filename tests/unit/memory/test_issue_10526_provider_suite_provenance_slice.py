"""Ratchet coverage for AUD-002 provider_suite_provenance extract (#10526)."""
...
from memory.graph.sync_pkg import provider_suite_provenance as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "provider_suite_provenance.py"
_CORE_LOC_BEFORE = 3192
_NAME = "_provider_suite_provenance"
_OWNED = re.compile(
...
def test_provider_suite_provenance_owns_symbols_and_shrinks_core() -> None:
