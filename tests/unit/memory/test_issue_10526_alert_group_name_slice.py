"""Ratchet coverage for AUD-002 alert_group_name extract (#10526)."""
...
from memory.graph.sync_pkg import alert_group_name as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "alert_group_name.py"
_CORE_LOC_BEFORE = 4526
_NAME = "_alert_group_name"
_OWNED = re.compile(
...
def test_alert_group_name_owns_symbols_and_shrinks_core() -> None:
