"""Ratchet coverage for AUD-002 snapshot_support_specs extract (#10526)."""
...
from memory.graph.sync_pkg import snapshot_support_specs as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "snapshot_support_specs.py"
_CORE_LOC_BEFORE = 10502
_NAME = "_snapshot_support_specs"
_OWNED = re.compile(
...
def test_snapshot_support_specs_owns_symbols_and_shrinks_core() -> None:
