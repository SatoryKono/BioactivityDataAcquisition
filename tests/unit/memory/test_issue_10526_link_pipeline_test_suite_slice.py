"""Ratchet coverage for AUD-002 link_pipeline_test_suite extract (#10526)."""
...
from memory.graph.sync_pkg import link_pipeline_test_suite as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "link_pipeline_test_suite.py"
_CORE_LOC_BEFORE = 3110
_NAME = "_link_pipeline_test_suite"
_OWNED = re.compile(
...
def test_link_pipeline_test_suite_owns_symbols_and_shrinks_core() -> None:
