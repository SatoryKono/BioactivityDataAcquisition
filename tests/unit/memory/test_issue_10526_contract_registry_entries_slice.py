"""Ratchet coverage for AUD-002 contract_registry_entries extract (#10526)."""
...
from memory.graph.sync_pkg import contract_registry_entries as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "contract_registry_entries.py"
_CORE_LOC_BEFORE = 4474
_NAME = "_contract_registry_entries"
_OWNED = re.compile(
...
def test_contract_registry_entries_owns_symbols_and_shrinks_core() -> None:
