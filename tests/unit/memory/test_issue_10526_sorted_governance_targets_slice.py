"""Ratchet coverage for AUD-002 sorted_governance_targets extract (#10526)."""
...
from memory.graph.sync_pkg import sorted_governance_targets as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "sorted_governance_targets.py"
_CORE_LOC_BEFORE = 5686
_NAME = "_sorted_governance_targets"
_OWNED = re.compile(
...
def test_sorted_governance_targets_owns_symbols_and_shrinks_core() -> None:
