"""Ratchet coverage for AUD-002 contract_source_prefixes extract (#10526)."""
...
from memory.graph.sync_pkg import contract_source_prefixes as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "contract_source_prefixes.py"
_CORE_LOC_BEFORE = 4632
_NAME = "_contract_source_prefixes"
_OWNED = re.compile(
...
def test_contract_source_prefixes_owns_symbols_and_shrinks_core() -> None:
