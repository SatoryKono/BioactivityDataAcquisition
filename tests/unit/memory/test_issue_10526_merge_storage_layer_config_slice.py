"""Ratchet coverage for AUD-002 merge_storage_layer_config extract (#10526)."""
...
from memory.graph.sync_pkg import merge_storage_layer_config as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "merge_storage_layer_config.py"
_CORE_LOC_BEFORE = 12548
_NAME = "_merge_storage_layer_config"
_OWNED = re.compile(
...
def test_merge_storage_layer_config_owns_symbols_and_shrinks_core() -> None:
