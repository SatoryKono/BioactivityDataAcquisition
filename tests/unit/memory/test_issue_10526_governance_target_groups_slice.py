"""Ratchet coverage for AUD-002 governance_target_groups extract (#10526)."""
...
from memory.graph.sync_pkg import governance_target_groups as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "governance_target_groups.py"
_CORE_LOC_BEFORE = 8933
_NAME = "_governance_target_groups"
_OWNED = re.compile(
...
def test_governance_target_groups_owns_symbols_and_shrinks_core() -> None:
