"""Ratchet coverage for AUD-002 pipeline_test_context extract (#10526)."""
...
from memory.graph.sync_pkg import pipeline_test_context as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "pipeline_test_context.py"
_CORE_LOC_BEFORE = 2934
_NAME = "_pipeline_test_context"
_OWNED = re.compile(
...
def test_pipeline_test_context_owns_symbols_and_shrinks_core() -> None:
