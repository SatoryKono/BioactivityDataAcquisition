"""Ratchet coverage for AUD-002 workflow_family_rules extract (#10526)."""
...
from memory.graph.sync_pkg import workflow_family_rules as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "workflow_family_rules.py"
_CORE_LOC_BEFORE = 6342
_NAME = "_WORKFLOW_FAMILY_RULES"
_OWNED = re.compile(
...
def test_workflow_family_rules_owns_symbols_and_shrinks_core() -> None:
