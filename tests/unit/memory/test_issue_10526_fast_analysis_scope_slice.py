"""Ratchet coverage for AUD-002 fast_analysis_scope extract (#10526)."""
...
from memory.graph.sync_pkg import fast_analysis_scope as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "fast_analysis_scope.py"
_CORE_LOC_BEFORE = 4777
_NAME = "_fast_analysis_scope"
_OWNED = re.compile(
...
def test_fast_analysis_scope_owns_symbols_and_shrinks_core() -> None:
