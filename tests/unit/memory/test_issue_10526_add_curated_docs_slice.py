"""Ratchet coverage for AUD-002 add_curated_docs extract (#10526)."""
...
from memory.graph.sync_pkg import add_curated_docs as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "add_curated_docs.py"
_CORE_LOC_BEFORE = 4554
_NAME = "_add_curated_docs"
_OWNED = re.compile(
...
def test_add_curated_docs_owns_symbols_and_shrinks_core() -> None:
