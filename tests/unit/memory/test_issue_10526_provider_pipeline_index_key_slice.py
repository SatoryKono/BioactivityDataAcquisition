"""Ratchet coverage for AUD-002 provider_pipeline_index_key extract (#10526)."""
...
from memory.graph.sync_pkg import provider_pipeline_index_key as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "provider_pipeline_index_key.py"
_CORE_LOC_BEFORE = 3095
_NAME = "_provider_pipeline_index_key"
_OWNED = re.compile(
...
def test_provider_pipeline_index_key_owns_symbols_and_shrinks_core() -> None:
