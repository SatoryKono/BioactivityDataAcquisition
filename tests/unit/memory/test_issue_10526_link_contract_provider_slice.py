"""Ratchet coverage for AUD-002 link_contract_provider extract (#10526)."""
...
from memory.graph.sync_pkg import link_contract_provider as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "link_contract_provider.py"
_CORE_LOC_BEFORE = 6778
_NAME = "_link_contract_provider"
_OWNED = re.compile(
...
def test_link_contract_provider_owns_symbols_and_shrinks_core() -> None:
