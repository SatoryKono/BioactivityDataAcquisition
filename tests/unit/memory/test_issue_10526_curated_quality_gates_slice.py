"""Ratchet coverage for AUD-002 curated_quality_gates extract (#10526)."""
...
from memory.graph.sync_pkg import curated_quality_gates as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "curated_quality_gates.py"
_CORE_LOC_BEFORE = 13173
_NAME = "CURATED_QUALITY_GATES"
_OWNED = re.compile(
...
def test_curated_quality_gates_owns_symbols_and_shrinks_core() -> None:
