"""Ratchet coverage for AUD-002 entity_storage_context extract (#10526)."""
...
from memory.graph.sync_pkg import entity_storage_context as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "entity_storage_context.py"
_CORE_LOC_BEFORE = 5312
_NAME = "_entity_storage_context"
_OWNED = re.compile(
...
def test_entity_storage_context_owns_symbols_and_shrinks_core() -> None:
