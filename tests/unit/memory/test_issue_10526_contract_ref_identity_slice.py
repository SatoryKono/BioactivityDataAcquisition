"""Ratchet coverage for AUD-002 contract_ref_identity extract (#10526)."""
...
from memory.graph.sync_pkg import contract_ref_identity as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "contract_ref_identity.py"
_CORE_LOC_BEFORE = 3171
_NAME = "_contract_ref_identity"
_OWNED = re.compile(
...
def test_contract_ref_identity_owns_symbols_and_shrinks_core() -> None:
