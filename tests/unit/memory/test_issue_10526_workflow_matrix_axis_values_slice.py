"""Ratchet coverage for AUD-002 workflow_matrix_axis_values extract (#10526)."""
...
from memory.graph.sync_pkg import workflow_matrix_axis_values as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "workflow_matrix_axis_values.py"
_CORE_LOC_BEFORE = 12338
_NAME = "_workflow_matrix_axis_values"
_OWNED = re.compile(
...
def test_workflow_matrix_axis_values_owns_symbols_and_shrinks_core() -> None:
