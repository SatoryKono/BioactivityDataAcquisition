"""Ratchet coverage for AUD-002 contract_policy_config_path extract (#10526)."""
...
from memory.graph.sync_pkg import contract_policy_config_path as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "contract_policy_config_path.py"
_CORE_LOC_BEFORE = 7487
_NAME = "_contract_policy_config_path"
_OWNED = re.compile(
...
def test_contract_policy_config_path_owns_symbols_and_shrinks_core() -> None:
