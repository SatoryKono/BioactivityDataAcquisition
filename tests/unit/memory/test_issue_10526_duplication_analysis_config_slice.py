"""Ratchet coverage for AUD-002 duplication_analysis_config extract (#10526)."""
...
from memory.graph.sync_pkg import duplication_analysis_config as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "duplication_analysis_config.py"
_CORE_LOC_BEFORE = 3998
_NAME = "_duplication_analysis_config"
_OWNED = re.compile(
...
def test_duplication_analysis_config_owns_symbols_and_shrinks_core() -> None:
