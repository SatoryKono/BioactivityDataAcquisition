"""Ratchet coverage for AUD-002 complexity_analysis_context extract (#10526)."""
...
from memory.graph.sync_pkg import complexity_analysis_context as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "complexity_analysis_context.py"
_CORE_LOC_BEFORE = 4388
_NAME = "_complexity_analysis_context"
_OWNED = re.compile(
...
def test_complexity_analysis_context_owns_symbols_and_shrinks_core() -> None:
