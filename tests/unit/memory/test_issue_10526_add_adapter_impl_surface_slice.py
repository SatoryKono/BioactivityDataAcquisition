"""Ratchet coverage for AUD-002 add_adapter_impl_surface extract (#10526)."""
...
from memory.graph.sync_pkg import add_adapter_impl_surface as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "add_adapter_impl_surface.py"
_CORE_LOC_BEFORE = 10663
_NAME = "_add_adapter_impl_surface"
_OWNED = re.compile(
...
def test_add_adapter_impl_surface_owns_symbols_and_shrinks_core() -> None:
