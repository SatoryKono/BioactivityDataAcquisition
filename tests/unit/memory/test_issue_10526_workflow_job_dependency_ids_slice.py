"""Ratchet coverage for AUD-002 workflow_job_dependency_ids extract (#10526)."""
...
from memory.graph.sync_pkg import workflow_job_dependency_ids as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "workflow_job_dependency_ids.py"
_CORE_LOC_BEFORE = 3047
_NAME = "_workflow_job_dependency_ids"
_OWNED = re.compile(
...
def test_workflow_job_dependency_ids_owns_symbols_and_shrinks_core() -> None:
