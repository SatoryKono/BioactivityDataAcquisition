"""Ratchet coverage for AUD-002 relation_requirement_keys extract (#10526)."""
...
from memory.graph.sync_pkg import relation_requirement_keys as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "relation_requirement_keys.py"
_CORE_LOC_BEFORE = 10423
_NAME = "_relation_requirement_keys"
_OWNED = re.compile(
...
def test_relation_requirement_keys_owns_symbols_and_shrinks_core() -> None:
