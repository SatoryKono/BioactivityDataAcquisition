"""Ratchet coverage for AUD-002 alert_runbook_path extract (#10526)."""
...
from memory.graph.sync_pkg import alert_runbook_path as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "alert_runbook_path.py"
_CORE_LOC_BEFORE = 5609
_NAME = "_alert_runbook_path"
_OWNED = re.compile(
...
def test_alert_runbook_path_owns_symbols_and_shrinks_core() -> None:
