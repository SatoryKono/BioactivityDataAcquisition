"""Ratchet coverage for AUD-002 storage_surface_format extract (#10526)."""
...
from memory.graph.sync_pkg import storage_surface_format as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "storage_surface_format.py"
_CORE_LOC_BEFORE = 13029
_NAME = "_storage_surface_format"
_OWNED = re.compile(
...
def test_storage_surface_format_owns_symbols_and_shrinks_core() -> None:
