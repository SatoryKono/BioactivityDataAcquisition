"""Ratchet coverage for AUD-002 link_provider_suite_targets extract (#10526)."""
...
from memory.graph.sync_pkg import link_provider_suite_targets as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "link_provider_suite_targets.py"
_CORE_LOC_BEFORE = 3181
_NAME = "_link_provider_suite_targets"
_OWNED = re.compile(
...
def test_link_provider_suite_targets_owns_symbols_and_shrinks_core() -> None:
