"""Ratchet coverage for AUD-002 support_storage_surface extract (#10526)."""
...
from memory.graph.sync_pkg import support_storage_surface as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "support_storage_surface.py"
_CORE_LOC_BEFORE = 10997
_NAME = "_support_storage_surface"
_OWNED = re.compile(
...
def test_support_storage_surface_owns_symbols_and_shrinks_core() -> None:
