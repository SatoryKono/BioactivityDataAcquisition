"""Ratchet coverage for AUD-002 entity_pipeline_identity extract (#10526)."""
...
from memory.graph.sync_pkg import entity_pipeline_identity as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "entity_pipeline_identity.py"
_CORE_LOC_BEFORE = 8153
_NAME = "_entity_pipeline_identity"
_OWNED = re.compile(
...
def test_entity_pipeline_identity_owns_symbols_and_shrinks_core() -> None:
