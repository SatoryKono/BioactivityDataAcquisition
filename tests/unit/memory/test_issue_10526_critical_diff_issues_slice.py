"""Ratchet coverage for AUD-002 critical_diff_issues extract (#10526)."""
...
from memory.graph.sync_pkg import critical_diff_issues as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "critical_diff_issues.py"
_CORE_LOC_BEFORE = 4796
_NAME = "_critical_diff_issues"
_OWNED = re.compile(
...
def test_critical_diff_issues_owns_symbols_and_shrinks_core() -> None:
