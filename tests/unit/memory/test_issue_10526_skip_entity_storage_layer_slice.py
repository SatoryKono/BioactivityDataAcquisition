"""Ratchet coverage for AUD-002 skip_entity_storage_layer extract (#10526)."""
...
from memory.graph.sync_pkg import skip_entity_storage_layer as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "skip_entity_storage_layer.py"
_CORE_LOC_BEFORE = 9780
_NAME = "_skip_entity_storage_layer"
_OWNED = re.compile(
...
def test_skip_entity_storage_layer_owns_symbols_and_shrinks_core() -> None:
