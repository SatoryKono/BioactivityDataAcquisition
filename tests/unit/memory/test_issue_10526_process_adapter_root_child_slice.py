"""Ratchet coverage for AUD-002 process_adapter_root_child extract (#10526)."""
...
from memory.graph.sync_pkg import process_adapter_root_child as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "process_adapter_root_child.py"
_CORE_LOC_BEFORE = 7897
_NAME = "_process_adapter_root_child"
_OWNED = re.compile(
...
def test_process_adapter_root_child_owns_symbols_and_shrinks_core() -> None:
