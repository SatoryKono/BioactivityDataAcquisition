"""Ratchet coverage for AUD-002 add_file_structure_zone extract (#10526)."""
...
from memory.graph.sync_pkg import add_file_structure_zone as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "add_file_structure_zone.py"
_CORE_LOC_BEFORE = 5340
_NAME = "_add_file_structure_zone"
_OWNED = re.compile(
...
def test_add_file_structure_zone_owns_symbols_and_shrinks_core() -> None:
