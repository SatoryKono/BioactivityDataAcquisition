"""Ratchet coverage for AUD-002 add_claim_token_targets extract (#10526)."""
...
from memory.graph.sync_pkg import add_claim_token_targets as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "add_claim_token_targets.py"
_CORE_LOC_BEFORE = 5733
_NAME = "_add_claim_token_targets"
_OWNED = re.compile(
...
def test_add_claim_token_targets_owns_symbols_and_shrinks_core() -> None:
