"""Ratchet coverage for AUD-002 link_alert_target_group extract (#10526)."""
...
from memory.graph.sync_pkg import link_alert_target_group as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "link_alert_target_group.py"
_CORE_LOC_BEFORE = 5120
_NAME = "_link_alert_target_group"
_OWNED = re.compile(
...
def test_link_alert_target_group_owns_symbols_and_shrinks_core() -> None:
