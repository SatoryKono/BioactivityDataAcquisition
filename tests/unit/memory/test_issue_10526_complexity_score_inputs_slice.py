"""Ratchet coverage for AUD-002 complexity_score_inputs extract (#10526)."""
...
from memory.graph.sync_pkg import complexity_score_inputs as extracted

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[3]
CORE = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "_core.py"
MOD = ROOT / "src" / "memory" / "graph" / "sync_pkg" / "complexity_score_inputs.py"
_CORE_LOC_BEFORE = 4423
_NAME = "_complexity_score_inputs"
_OWNED = re.compile(
...
def test_complexity_score_inputs_owns_symbols_and_shrinks_core() -> None:
