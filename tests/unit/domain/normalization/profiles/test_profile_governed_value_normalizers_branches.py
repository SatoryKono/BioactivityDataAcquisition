...
def test_standard_unit_enum_branches() -> None:
    ...
    assert (
        normalizers.normalize_profile_standard_unit_enum(None, allowed_values=allowed)
        is None
    )
    assert (
        normalizers.normalize_profile_standard_unit_enum(1, allowed_values=allowed)
        is None
    )
    # non-string path already covered; string outside enum after normalize may return None
    assert (
        normalizers.normalize_profile_standard_unit_enum(
            "not-a-unit", allowed_values=allowed
        )
        is None
    )
...
def test_assay_parameter_and_target_component_normalizers() -> None:
    ...
    out = normalizers.normalize_profile_assay_parameter_type(
        "ic50", allowed_values=allowed
    )
