...
class TestChemblAssayEnumFields:
    ...
    def test_enum_fields_configured(self) -> None:
        ...
        enum_fields = ['assay_type', 'assay_test_type', 'assay_category', 'relationship_type']
        
        for field_name in enum_fields:
            assert field_name in CHEMBL_ASSAY_PROFILE.field_rules, f"{field_name} not found in field rules"
    ...
    def test_assay_type_enum_normalization(self) -> None:
        ...
        for valid_value in ['B', 'F', 'A', 'T', 'P', 'U']:
            ...
            assert result == valid_value.upper(), f"Expected {valid_value.upper()}, got {result}"
    ...
    def test_profile_integration(self) -> None:
        ...
        for field_name in enum_fields:
            ...
            assert callable(rule.normalizer), f"{field_name} should have a callable normalizer"
            # Should have appropriate notes
            assert 'normalize' in rule.notes.lower(), f"{field_name} should have normalization notes"
...
class TestEnumValidation:
    ...
    def test_enum_validation_function(self) -> None:
        ...
        from bioetl.domain.normalization.profiles.profile_normalizers import normalize_profile_enum
