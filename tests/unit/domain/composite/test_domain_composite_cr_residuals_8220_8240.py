...
import pytest
from bioetl.domain.composite.config_composite_serialization import composite_from_dict, composite_to_dict
from bioetl.domain.composite.config_composite_validation import validate_composite_config
...
from bioetl.domain.composite.config_models import CompositeConfig, DependencyConfig, EnricherConfig, SeedConfig
...
from bioetl.domain.composite.config_validators import require_non_empty, validate_positive
from bioetl.domain.composite.field_groups_models import FieldGroupDefinition, FieldGroupId, FieldMapping
...
def test_lineage_config_freezes_nested_mappings():
    ...
    cfg = LineageConfig(provider_lookup_fields=nested, track_source_for_fields=["title", "doi"])
...
def test_data_schema_empty_groups_exclude_all():
    schema = DataSchemaConfig(column_groups=(), gold=LayerColumnConfig(include_groups=()))
...
def test_dq_effective_override_threshold_order():
    with pytest.raises(ValueError, match="effective soft"):
        CompositeDQConfig(soft_fail_threshold=0.1, hard_fail_threshold=0.5, enricher_overrides={"e1": DQOverrideConfig(soft_fail_threshold=0.6)})
    ok = CompositeDQConfig(soft_fail_threshold=0.1, hard_fail_threshold=0.5, enricher_overrides={"e1": DQOverrideConfig(soft_fail_threshold=0.2)})
...
def test_merge_result_freezes_mappings_and_payloads():
    ...
    lineage = {"ok": 1}
    payload = {"id": "q1"}
    result = MergeResult(records_merged=1, field_coverage=coverage, lineage_summary=lineage, quarantine_payloads=(payload,))
    coverage["title"] = 0.1
    lineage["ok"] = 9
    payload["id"] = "mut"
...
def test_composite_result_nominal_and_failure_paths():
    ...
    good = CompositeResult(composite_name="c", composite_run_id="r1", seed_result=_seed(), dependency_results={"dep": dep_ok}, enrichment_results={"enr": enr_ok}, merge_result=_merge(), _required_dependencies=frozenset({"dep"}), _required_enrichers=frozenset({"enr"}))
    assert good.is_success is True
    missing = CompositeResult(composite_name="c", composite_run_id="r2", seed_result=_seed(), dependency_results={}, enrichment_results={"enr": enr_ok}, merge_result=_merge(), _required_dependencies=frozenset({"dep"}), _required_enrichers=frozenset({"enr"}))
...
def test_composite_result_maps_are_immutable():
    ...
    result = CompositeResult(composite_name="c", composite_run_id="r", seed_result=_seed(), dependency_results=source, merge_result=_merge())
...
def test_registry_uses_enclosing_group_id():
    mapping = FieldMapping(base_name="title", provider_columns=("chembl.publication.title",), group=FieldGroupId.TRASH)
    group = FieldGroupDefinition(group_id=FieldGroupId.BIBLIOGRAPHY, display_name="Bib", fields=(mapping,))
...
def test_coerce_and_validate_composite_config():
    ...
    merge = MergeConfig(strategy=MergeStrategy.LEFT_OUTER, conflict_resolution=ConflictResolution.SEED_PRIORITY, output_silver_path="silver/x", output_gold_path="gold/x")
    config = CompositeConfig(name="c", version="1", seed=seed, enrichers=[EnricherConfig(pipeline="e", join_keys=("k",))], merge=merge, dependencies=[])
    ...
    with pytest.raises(ValueError, match="Duplicate enricher"):
        CompositeConfig(name="c", version="1", seed=seed, enrichers=(EnricherConfig(pipeline="e", join_keys=("k",)), EnricherConfig(pipeline="e", join_keys=("k",))), merge=merge)
...
def test_composite_to_dict_from_dict_roundtrip_preserves_options():
    config = CompositeConfig(
        ...
        dependencies=(DependencyConfig(pipeline="dep", join_keys=("k",), required=True, timeout_seconds=120, key_source="seed"),),
        enrichers=(EnricherConfig(pipeline="e", join_keys=("k",), required=True, timeout_seconds=90, limit=3),),
        merge=MergeConfig(strategy=MergeStrategy.LEFT_OUTER, conflict_resolution=ConflictResolution.SEED_PRIORITY, output_silver_path="silver/x", output_gold_path="gold/x", field_priorities={"title": ("chembl", "crossref")}, preserve_all_sources=True),
    ...
    restored = composite_from_dict(payload, composite_cls=CompositeConfig, seed_cls=SeedConfig, dependency_cls=DependencyConfig, enricher_cls=EnricherConfig, merge_cls=MergeConfig)
...
def test_enrichment_result_factories_and_rates():
    ok = EnrichmentResult.success("e", records_input=10, records_enriched=8, records_not_found=2)
