...
class TestDQResultExtended:
    ...
    def test_rule_outcomes_filtering(self):
        ...
        schema_violations = result.get_outcomes_by_violation_kind(DQViolationKind.SCHEMA_VIOLATION)
        assert len(schema_violations) == 1
        assert schema_violations[0].rule_id == "schema.not_null"
        
        threshold_breaches = result.get_outcomes_by_violation_kind(DQViolationKind.THRESHOLD_BREACH)
    ...
    def test_dq_result_serialization(self):
        ...
        outcome = DQRuleOutcome("schema.not_null", DQViolationKind.SCHEMA_VIOLATION, "high", DQDisposition.FAIL)
...
class TestDQResultProperties:
    ...
    def test_rule_outcomes_count_property(self):
        ...
        outcomes = [
            DQRuleOutcome("rule1", DQViolationKind.SCHEMA_VIOLATION, "high", DQDisposition.FAIL),
            DQRuleOutcome("rule2", DQViolationKind.THRESHOLD_BREACH, "medium", DQDisposition.WARN),
        ]
    ...
    def test_has_rule_violations_property(self):
        ...
        outcomes_without_violations = [
            DQRuleOutcome("rule1", DQViolationKind.SCHEMA_VIOLATION, "low", DQDisposition.PASS),
            DQRuleOutcome("rule2", DQViolationKind.THRESHOLD_BREACH, "low", DQDisposition.PASS),
        ]
    ...
    def test_has_quarantine_decisions_property(self):
        ...
        outcomes = [
            DQRuleOutcome("rule1", DQViolationKind.SCHEMA_VIOLATION, "high", DQDisposition.QUARANTINE),
            DQRuleOutcome("rule2", DQViolationKind.THRESHOLD_BREACH, "medium", DQDisposition.WARN),
        ]
