...
def test_not_proven_blocked_or_no_issue() -> None:
    ...
    assert decide_issue_state(status="NOT_PROVEN", allow_issue_write=True, no_actionable=True) == "no_issue"


def test_matching_fingerprint_reuses() -> None:
    assert (
        decide_issue_state(status="PROVEN", allow_issue_write=True, fingerprint_match_open=True)
        == "reuse"
    )
    # reuse takes precedence even when NOT_PROVEN would otherwise block
    assert (
        decide_issue_state(status="NOT_PROVEN", allow_issue_write=False, fingerprint_match_open=True)
        == "reuse"
    )


def test_target_branch_close_gate_requires_all() -> None:
    assert can_close_issue(allow_close=True, proven_on_origin_base=True, required_checks_green=True) is True
    assert can_close_issue(allow_close=False, proven_on_origin_base=True, required_checks_green=True) is False
    assert can_close_issue(allow_close=True, proven_on_origin_base=False, required_checks_green=True) is False
    assert can_close_issue(allow_close=True, proven_on_origin_base=True, required_checks_green=False) is False


def test_defer_and_blocked_states() -> None:
    assert decide_issue_state(status="PROVEN", allow_issue_write=True, deferred=True) == "defer"
