...
2. ✅ Add numeric_id_fields exclusions (Phase 2.1) — 2 minutes
...
6. 📝 Document date vs datetime usage guideline
7. 📝 Create list of ChEMBL standard measurement types
...
2. **ID field types:** Нарушают convention "IDs должны быть строками", но есть valid exceptions для internal numeric IDs
3. **Date fields:** Семантически правильно использовать `date` для calendar dates без времени
4. **Enum validation:** Legitimate missing validation для enum field
