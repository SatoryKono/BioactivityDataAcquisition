...
@pytest.fixture
def valid_publication_df() -> pd.DataFrame:
    schema = PublicationTableSchema.to_schema()
...
def test_publication_schema_accepts_valid_frame(
    valid_publication_df: pd.DataFrame,
) -> None:
    validated = PublicationTableSchema.validate(valid_publication_df)
    pd.testing.assert_frame_equal(
        validated,
        valid_publication_df,
        check_dtype=False,
    )


def test_publication_schema_rejects_bad_doc_type(
    valid_publication_df: pd.DataFrame,
) -> None:
