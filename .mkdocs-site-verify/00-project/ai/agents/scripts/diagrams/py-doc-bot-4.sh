...
usage() {
  cat <<'EOF'
Usage: docs/00-project/ai/agents/scripts/diagrams/py-doc-bot-4.sh [options]
...
if [[ "$SKIP_CHECKS" -eq 0 ]]; then
  CHECK_CMD=("bash" "$REPO_ROOT/docs/00-project/ai/agents/scripts/diagrams/py-doc-bot-1.sh" "--profile" "$PROFILE" "--text-layer" "$TEXT_LAYER")
...
if [[ "$SKIP_DOCX" -eq 0 ]]; then
  DOCX_CMD=("$PYTHON_CMD" "$REPO_ROOT/docs/00-project/ai/agents/scripts/diagrams/py-doc-bot-2.py")
  DOCX_CMD+=("${MD_ARGS[@]}")
...
if [[ "$SKIP_PDF" -eq 0 ]]; then
  PDF_CMD=("$PYTHON_CMD" "$REPO_ROOT/docs/00-project/ai/agents/scripts/diagrams/py-doc-bot-3.py")
  PDF_CMD+=("${MD_ARGS[@]}")
