...
def scaffold(source, entity):
    ...
    (adapter_dir / "client.py").write_text(CLIENT_TEMPLATE.format(source_title=source_title))
    (adapter_dir / "__init__.py").write_text(f"from .client import {source_title}Adapter")
    ...
    pipeline_file.write_text(PIPELINE_TEMPLATE.format(source_title=source_title, entity_title=entity_title))
