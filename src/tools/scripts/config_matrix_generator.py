...
def main() -> None:
    ...
    all_keys = sorted({k for v in configs.values() for k in v.keys()}, key=_sort_key)
    ...
    print("=" * 80)
    if configs:
        common = set.intersection(*(set(v.keys()) for v in configs.values()))
    ...
    with report_path.open("w", encoding="utf-8") as f:
        ...
        f.write(f"Total unique parameters: {len(all_keys)}\n\n")
        f.write("## Inconsistent Parameters\n\n")
        for key in partial:
