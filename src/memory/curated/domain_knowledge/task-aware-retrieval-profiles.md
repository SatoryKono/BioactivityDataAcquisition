...
- The memory layer exposes retrieval profiles such as `architecture`, `implementation`,
  `operations`, and `audit` to bias ranking toward the most useful evidence for the
  current task.
- The same query string can surface different artifacts depending on whether the agent
  is tracing architecture, editing runtime code, or investigating an operational event.
- Use `implementation` when changing code or configs, `operations` when investigating
  failures and runbooks, and `architecture` when reasoning about ADRs or structural
  boundaries.
