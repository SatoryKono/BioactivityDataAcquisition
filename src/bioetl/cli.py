...
from bioetl.clients.chembl.factories import default_chembl_extraction_service
from bioetl.config.resolver import ConfigResolver
from bioetl.logging.factories import default_logger
from bioetl.output.factories import default_metadata_writer, default_writer
from bioetl.output.unified_writer import UnifiedOutputWriter
from bioetl.pipelines.registry import PIPELINE_REGISTRY, get_pipeline_class
...
def run(
    ...
    """
    # Import schemas here to ensure registration happens but avoid circular imports or init issues
    import bioetl.schemas  # noqa
    ...
    try:
        ...
        # Extraction Service
        # Currently we assume ChEMBL based on provider or pipeline class
        if config.provider == "chembl":
            extraction_service = default_chembl_extraction_service()
        else:
            # TODO: Add factories for other providers when needed
            console.print(f"[red]Unknown provider: {config.provider}[/red]")
            sys.exit(1)

        # 4. Run Pipeline
        # Note: We assume ChemblPipelineBase signature which requires extraction_service
        # If we add non-Chembl pipelines, we might need a factory for pipeline instantiation
        # or standardize __init__ args further.
