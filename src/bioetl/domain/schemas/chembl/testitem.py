...
from bioetl.domain.transform.impl.normalize import CHEMBL_ID_REGEX
...
class TestitemSchema(pa.DataFrameModel):
    ...
    molecule_type: Series[str] = pa.Field(
        nullable=True, description="Тип молекулы (Small molecule, Antibody, etc.)"
    )
    ...
    molecule_structures: Series[str] = pa.Field(
        nullable=True, description="Структурные данные (SMILES, InChI, молфайл)"
    )
    molecule_hierarchy: Series[str] = pa.Field(
        nullable=True, description="Иерархия молекулы (родительская форма и пр.)"
    )
