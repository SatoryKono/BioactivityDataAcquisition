...
class AssaySchema(pa.DataFrameModel):
    ...
    assay_cell_type: Series[str] = pa.Field(
        nullable=True, description="Тип клеточной линии (если применимо)"
    )
    ...
    assay_classifications: Series[str] = pa.Field(
        nullable=True, description="Классификации ассая (BAO и др.)"
    )
    assay_group: Series[str] = pa.Field(
        nullable=True, description="Группа/серия, к которой относится тест"
    )
    assay_organism: Series[str] = pa.Field(
        nullable=True, description="Организм системы тестирования"
    )
    assay_parameters: Series[str] = pa.Field(
        nullable=True, description="Параметры теста"
    )
    assay_strain: Series[str] = pa.Field(nullable=True, description="Штамм организма")
    assay_subcellular_fraction: Series[str] = pa.Field(
        nullable=True, description="Субклеточная фракция"
    )
    assay_tax_id: Series[float] = pa.Field(
        nullable=True, description="NCBI Taxonomy ID организма"
    )
    assay_test_type: Series[str] = pa.Field(
        nullable=True, description="Тип теста (in vitro, in vivo, ex vivo и т.п.)"
    )
    assay_tissue: Series[str] = pa.Field(
        nullable=True, description="Ткань, на которой проведён ассай"
    )
    ...
    assay_type_description: Series[str] = pa.Field(
        nullable=True, description="Расшифровка типа ассая"
    )
    ...
    bao_label: Series[str] = pa.Field(
        nullable=True, description="Читабельная метка BAO-формата"
    )
    ...
    confidence_description: Series[str] = pa.Field(
        nullable=True, description="Текстовое описание уровня уверенности"
    )
    confidence_score: Series[int] = pa.Field(
        nullable=True,
        ge=0,
        le=9,
        description="Уровень уверенности (0–9) в маппинге таргета",
    )
    ...
    relationship_description: Series[str] = pa.Field(
        nullable=True, description="Расшифровка relationship_type"
    )
    ...
    score: Series[float] = pa.Field(
        nullable=True, description="Score для ранжирования ассая при поиске"
    )
    src_assay_id: Series[str] = pa.Field(
        nullable=True, description="Идентификатор ассая в исходной БД"
    )
