...
class ActivitySchema(pa.DataFrameModel):
    ...
    activity_comment: Series[str] = pa.Field(
        nullable=True, description="Комментарий к активности"
    )
    activity_id: Series[int] = pa.Field(ge=1, description="Внутренний ID активности")
    activity_properties: Series[str] = pa.Field(
        nullable=True, description="Список дополнительных свойств активности"
    )
    ...
    assay_variant_mutation: Series[str] = pa.Field(
        nullable=True, description="Описание мутаций варианта белка"
    )
    ...
    pchembl_value: Series[float] = pa.Field(
        nullable=True, ge=0, le=15, description="Нормализованная активность (-log10)"
    )
    potential_duplicate: Series[bool] = pa.Field(
        nullable=True, description="Флаг потенциального дубликата"
    )
    qudt_units: Series[str] = pa.Field(nullable=True, description="URI единиц (QUDT)")
    record_id: Series[float] = pa.Field(
        nullable=True, ge=1, description="ID связанной записи (compound_record)"
    )
    ...
    standard_flag: Series[bool] = pa.Field(
        description="Признак нормализации типа/значения"
    )
    ...
    standard_text_value: Series[str] = pa.Field(
        nullable=True, description="Нормализованный текст для качественных значений"
    )
    standard_type: Series[str] = pa.Field(
        nullable=True, description="Нормализованный тип активности"
    )
    standard_units: Series[str] = pa.Field(
        nullable=True, description="Нормализованные единицы измерения"
    )
    standard_upper_value: Series[float] = pa.Field(
        nullable=True, description="Верхняя граница нормализованного интервала"
    )
    standard_value: Series[float] = pa.Field(
        nullable=True, description="Нормализованное числовое значение"
    )
    ...
    target_organism: Series[str] = pa.Field(
        nullable=True, description="Организм таргета"
    )
    target_pref_name: Series[str] = pa.Field(
        nullable=True, description="Название таргета"
    )
    target_tax_id: Series[float] = pa.Field(
        nullable=True, description="NCBI TaxID таргета"
    )
    text_value: Series[str] = pa.Field(
        nullable=True, description="Исходное текстовое значение"
    )
    toid: Series[str] = pa.Field(nullable=True, description="ID из Target Ontology")
    type: Series[str] = pa.Field(
        nullable=True, description="Исходный тип активности (как в источнике)"
    )
    units: Series[str] = pa.Field(
        nullable=True, description="Исходные единицы измерения"
    )
    uo_units: Series[str] = pa.Field(
        nullable=True, description="ID единиц (Unit Ontology)"
    )
    upper_value: Series[float] = pa.Field(
        nullable=True, description="Верхняя граница исходного интервала"
    )
