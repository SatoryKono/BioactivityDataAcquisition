...
class PubchemMolecule(BaseEntity):
    """Lineage-aware PubChem domain entity using ``pubchem_cid`` as canonical ID."""
    ...
    def __init__(
        ...
        *,
        ...
        if (
            pubchem_cid is not None
            and molecule_id is not None
            and pubchem_cid != molecule_id
        ):
            raise ValueError(
    ...
    def _validate_invariants(self) -> None:
        if not self.pubchem_cid:
            raise ValueError("PubchemMolecule pubchem_cid is required")
    ...
    def molecule_id(self) -> str:
        """Backward-compatible alias for the canonical PubChem CID field."""
