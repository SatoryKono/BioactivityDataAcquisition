...
def _resolve_helpers(kwargs: dict[str, Any], logger: LoggerPort, metrics: Any) -> Any:
...
def create_crossref_adapter(
    ...
    **kwargs: Any,  # Any: forward arbitrary adapter config kwargs
    ...
    helpers = _resolve_helpers(kwargs, logger, metrics)
    adapter_metrics = helpers["adapter_metrics"]
    request_collector = helpers["request_collector"]
    ...
    batch_fetcher = kwargs.get("batch_fetcher") or _create_default_crossref_batch_fetcher(
        http=http_client, logger=logger, metrics=adapter_metrics,
    ...
    title_fallback_handler = kwargs.get("title_fallback_handler") or _create_default_crossref_title_fallback_handler(
    ...
    return CrossRefAdapter(
        ...
        error_handler=helpers["error_handler"],
        adapter_metrics=adapter_metrics,
        request_collector=request_collector,
        fallback_fetch_service=helpers["fallback_fetch_service"],
