...
def create_crossref_adapter(
    ...
    **kwargs: Any,  # Any: forward arbitrary adapter config kwargs
    ...
    """
    mailto = kwargs.get(
        "mailto", getattr(settings, "default_email", None) if settings else None
    )
    if not mailto or http_client is None or logger is None:
        raise ValueError(
            "CrossRef adapter requires mailto, http_client, and logger. "
            "Provide mailto via 'mailto' kwarg or settings.default_email"
        )
