...
def _resolve_crossref_mailto(mailto: Any, settings: Settings | None) -> str:
    resolved = mailto or (
        getattr(settings, "default_email", None) if settings else None
    )
    if not resolved:
        raise ValueError(
...
def create_crossref_adapter(
    ...
    **kwargs: Any,  # Any: forward arbitrary adapter config kwargs
    ...
    mailto = _resolve_crossref_mailto(kwargs.get("mailto"), settings)
    ...
    return CrossRefAdapter(
        ...
        or _create_default_crossref_response_mapper(),
