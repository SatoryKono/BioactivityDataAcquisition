...
def create_pipeline_config_loader(configs_root: Path) -> Callable[[str], object]:
    return lambda pipeline_name: load_pipeline_config_from_root(pipeline_name, configs_root=configs_root)
...
def bootstrap_config_service(*, registry: PipelineRegistry | None = None, configs_root: Path | None = None) -> ConfigService:
    ...
    return build_cli_config_service(
        ...
        pipeline_yaml_getter=partial(get_pipeline_yaml_for_dq, pipeline_config_loader=pipeline_config_loader),
        dq_config_loader=partial(load_dq_config_for_pipeline, configs_root=resolved_configs_root),
