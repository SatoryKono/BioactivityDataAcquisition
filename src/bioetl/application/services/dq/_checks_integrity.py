...
def _count_scd_overlaps(
    *,
    ...
    try:
        # Limit to 100 entities; maintain_order=False + sort keeps the overlap sample deterministic.
        entities = df[entity_key].unique(maintain_order=False).sort().head(100)
