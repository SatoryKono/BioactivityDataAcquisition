...
def _count_scd_overlaps(
    *,
    ...
    """Count overlapping SCD validity periods using vectorized window functions."""
    try:
        # Vectorized overlap check over all partitions without a python loop overhead
        return int(
            df.sort([entity_key, valid_from])
            .select(
                (
