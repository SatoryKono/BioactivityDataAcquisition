...
def resolve_merge_metadata_timestamp(
    ...
    *,
    ...
    """Return merge metadata timestamp for replay or live runs.

    Replay uses cached bronze date at UTC midnight. Live runs without a
    cached date use ``clock.now()`` when a ClockPort is provided.
    """
...
async def prepare_merge_execution_context(
    ...
    if request.metadata_timestamp is None:
        resolved_timestamp = resolve_merge_metadata_timestamp(
            None,
            clock=host._clock,
        )
