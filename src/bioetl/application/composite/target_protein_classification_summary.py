"""Pre-join summary for ChEMBL target protein-classification relations."""
...
def summarize_target_protein_classification_dependency(
    ...
    summary_rows = [
        _summarize_target(target_id, rows)
...
def _summarize_target(
    ...
    resolved_rows = _deduplicate_resolved_rows(rows)
    summary = _empty_summary_row(target_id)
...
def _empty_summary_row(target_id: str) -> dict[str, object]:
    row: dict[str, object] = {
