...
from bioetl.application.pipelines.chembl.assay_filter import AssayGoldFilter
...
class ChEMBLAssayPipeline(BasePipeline):
    ...
    def __init__(
        ...
        self._gold_filter = AssayGoldFilter(
            preferred_types=self.config.gold_filter_types,
            min_confidence=self.config.gold_min_confidence or 4,
        )
