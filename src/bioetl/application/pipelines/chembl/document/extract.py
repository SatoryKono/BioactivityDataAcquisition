"""
Extraction logic for Document pipeline.
"""
import logging
...
def extract_document(
    ...
    **kwargs: Any
    ...
    if path:
        ...
        if "document_chembl_id" not in header.columns:
             raise ValueError(f"Input file {path} must contain 'document_chembl_id'")
             
        is_id_only = len(header.columns) <= 2 and "document_chembl_id" in header.columns
        limit = kwargs.get("limit")
        
        if not is_id_only:
            logger.info("Detected full data in input CSV.")
            df = read_input_dataframe(config, id_col="document_chembl_id")
            if df is None:
                return pd.DataFrame()
        ...
        ids = read_input_ids(config, id_col="document_chembl_id")
        if not ids:
            return pd.DataFrame()
    ...
    return service.extract_all("document", **kwargs)
