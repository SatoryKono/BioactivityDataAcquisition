"""MQ job handlers for running pipelines from a message queue."""
...
@dataclass
class MQJob:
    """Description of a queued pipeline run request."""
...
def _to_pipeline_id(pipeline_name: str) -> str:
    """Convert human-friendly pipeline name to '<provider>.<entity>' id.

    Defaults provider to 'chembl' if suffix is missing.
    """
...
class MQJobHandler:
    """Handle MQ jobs by orchestrating pipeline execution."""
    ...
    def handle(self, job: MQJob) -> RunResult:
        ...
        ) = partial(create_provider_loader, config_path=providers_path)
