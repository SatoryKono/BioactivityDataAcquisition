...
async def _load_checkpoint_freshness_evidence_impl(
    ...
    if scope.selected_run_id is not None and scope.resolved_manifest is not None:
        ...
        checkpoint_tuple = await checkpoint_port.load_for_manifest_id(manifest_id)  # type: ignore[attr-defined]
        evidence_source = "immutable_manifest_history"
        if checkpoint_tuple is None:
            checkpoint_tuple = await checkpoint_port.load_for_run(  # type: ignore[attr-defined]
    ...
    if scope.selected_pipelines or not host._is_all_scope_token(
        ...
        checkpoint_tuple = await checkpoint_port.load(target_pipeline)  # type: ignore[attr-defined]
