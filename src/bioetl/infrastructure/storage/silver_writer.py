...
class SilverWriter(BaseDeltaWriter):
    ...
    def _prepare_arrow_data(
        ...
        # Optimization: Schema-driven iteration (Bolt)
        # Instead of iterating over all keys in records (which may have many unused fields),
        # we iterate over the schema fields. This avoids unnecessary processing and
        # creates a minimal dictionary for PyArrow.
        ...
        for rec in records:
            ...
            for k in schema_names:
                # Use rec.get(k) to safely handle missing fields (mapped to null by PyArrow)
                v = rec.get(k)
                if v is not None:
                    ...
                    if k in string_fields and isinstance(v, (dict, list)):
                        new_rec[k] = orjson.dumps(v, option=orjson.OPT_SORT_KEYS).decode(
                            "utf-8"
                        )
                    else:
                        new_rec[k] = v
