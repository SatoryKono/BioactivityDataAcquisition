...
def _parse_quarter_label(raw_value: object) -> tuple[int, int] | None:
    if not isinstance(raw_value, str):
        return None
    match = _QUARTER_RE.fullmatch(raw_value.strip())
    if match is None:
        return None
...
def _evaluate_program_done_criteria(
    *,
    ...
    deadline_tuple = _parse_quarter_label(deadline_quarter)
    if (
