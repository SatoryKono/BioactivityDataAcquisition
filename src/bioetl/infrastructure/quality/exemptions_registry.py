...
def validate_exemption_target_references(
    ...
    for registry_name, entries in sorted(registries.items()):
        ...
        if not is_class_reg and not is_func_reg:
            continue

        symbol_kind = "class" if is_class_reg else "function"
        symbols_map = classes_by_module if is_class_reg else functions_by_module
        counts_map = class_counts if is_class_reg else function_counts
        ...
        for key in sorted(entries):
            if isinstance(key, str):
                _validate_symbol_key_reference(
                    ...
                    symbols_by_module=symbols_map,
                    global_counts=counts_map,
                    errors=errors,
                )
