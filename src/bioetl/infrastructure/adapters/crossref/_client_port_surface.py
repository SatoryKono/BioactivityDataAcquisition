...
class _CrossRefPortSurfaceMixin:
    ...
    # Host attrs are provided by CrossRefAdapter. Keep them TYPE_CHECKING-only so
    # they are not collected as dataclass fields with defaults (which would put
    # defaulted http_client before required logger and raise TypeError).
    if TYPE_CHECKING:
