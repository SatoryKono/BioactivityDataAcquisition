#!/usr/bin/env python3
"""Publish DUX3 epic + children via GitHub CLI and write a publish record.
...
PACK = ROOT / ".github" / "ISSUES" / "DUX3-2026-07-29-DASHBOARD-UX-RESIDUAL-ISSUE-PACK.md"
PUBLISH = ROOT / "reports" / "quality" / "dux3-2026-07-29-issue-publish.json"
...
CHILD_ORDER = [c for c in META if c != "DUX3-00"]


def load_titles() -> dict[str, str]:
    titles: dict[str, str] = {}
    for line in TITLES_PATH.read_text(encoding="utf-8").splitlines():
        m = re.match(r"- `(DUX3-\d+)`: `(.*)`\s*$", line)
        if m:
...
def patch_parent(body: str, epic: int) -> str:
    return body.replace("_TBD_ (DUX3-00)", f"#{epic} (DUX3-00)")
...
def update_pack(created: list[dict[str, object]]) -> None:
    ...
    for row in created:
        ...
        url = str(row["url"])
        # Replace matrix cell: | DUX3-XX | _TBD_ |  -> | DUX3-XX | [#N](url) |
    ...
    if "dux3-2026-07-29-issue-publish.json" not in text:
        text = text.rstrip() + (
            "\n\n## Publish artifact\n\n"
            "- `reports/quality/dux3-2026-07-29-issue-publish.json`\n"
        )
...
def main() -> int:
    ...
    if not args.dry_run:
        ...
        comment = (
            "## Children (published)\n\n"
            f"{child_lines}\n\n"
            "Pack: `.github/ISSUES/DUX3-2026-07-29-DASHBOARD-UX-RESIDUAL-ISSUE-PACK.md`\n"
        )
        run_gh(
            gh,
        ...
        record = {
            ...
            "wave": "dux3-dashboard-ux-residual-2026-07-29",
            "issue_pack": ".github/ISSUES/DUX3-2026-07-29-DASHBOARD-UX-RESIDUAL-ISSUE-PACK.md",
