...
2. **Ontology ID Variations**: CLO, EFO, UBERON IDs not consistently formatted
3. **No Standard Rules**: Missing cross-pipeline case normalization strategy
4. **Content Hash Impact**: Case variations affect deterministic hashing
5. **DQ Rule Mismatches**: Filters expect specific case but normalization doesn't enforce it
...
3. **Create Case Strategy Configuration**
   ```yaml
...
2. **Add Ontology ID Normalization**
   ```python
