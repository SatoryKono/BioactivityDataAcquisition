...
||||||| 565fb33295
| 215 | Review First Action           | Table      | `topk(4, bioetl_l0_next_action_route{pipeline=~"$pipeline",run_type=~"$run_type"} or NO_ROUTE label_replace(vector(0)…))` | Up to four urgency-ordered routes: Priority (color-text), Action (row link via `action_dashboard_uid`), Why, Pipeline. Missing scope → `NO_ROUTE`. Priority: Runtime > Control Plane > Gold Lifecycle > DQ > Provider > Workflow > Monitor. Secondary panel links: Runtime / Control Plane / DQ / Provider (provider fail-closes `provider=unknown` + `pipeline_context`). |
=======
| 215 | Review First Action           | Table      | `topk(4, bioetl_l0_next_action_route{pipeline=~"$pipeline",run_type=~"$run_type"} or NO_ROUTE label_replace(vector(0)…))` | Up to four urgency-ordered routes: Priority (color-text), Action (row link via hidden `action_dashboard_uid`), Why, Pipeline. The panel fits all four bounded routes without internal scrolling. Missing scope → `NO_ROUTE`. Priority: Runtime > Control Plane > Gold Lifecycle > DQ > Provider > Workflow > Monitor. Secondary panel links: Runtime / Control Plane / DQ / Provider (provider fail-closes `provider=unknown` + `pipeline_context`). |
