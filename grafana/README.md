...
| 215 | Next Action                    | Table      | `topk(1, bioetl_l0_next_action_route{pipeline=~"$pipeline",run_type=~"$run_type"} > 0)`                                     | Shows `action_target`, `action_reason`, and `action_dashboard_uid`; routing priority remains Runtime > Control Plane > Gold Lifecycle > DQ > Provider > Workflow > Monitor.   |
