...
| 215 | Review First Action           | Table      | `bioetl_first_action{workflow,pipeline,run_type}>0 or on() bioetl_fa_gap` | Two urgency-ordered rows. Priority is `—` / VERIFY / REVIEW / HIGH / URGENT / CRIT. Standalone pipelines use pipeline series; a missing concrete workflow stays VERIFY and does not borrow another route. No action required has no diagnostic link. Run ID does not change CURRENT. |
...
| 9011 | Recent Non-success Terminal Runs | Table | `sum by (pipeline, status) (increase(bioetl_pipeline_runs_total{status!="success",...}[$__range]))`                          | Selected-range non-success terminal-run evidence only; empty table is not proof of current OK; handoffs `Open Control Plane` and `Open Runtime`.                            |
