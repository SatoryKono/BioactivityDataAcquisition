...
def test_null_patterns():
    ...
    from bioetl.domain.normalization.rules import normalize_null, NULL_PATTERNS
    ...
    for input_val, expected in null_test_cases:
        ...
        assert result is expected, f"Expected {expected}, got {result} for input {repr(input_val)}"
    ...
    for input_val, expected in non_null_test_cases:
        result = normalize_null(input_val)
        assert result == expected, f"Expected {expected}, got {result} for input {repr(input_val)}"
...
def test_profile_null_normalizer():
    ...
    from bioetl.domain.normalization.profiles.profile_normalizers import normalize_profile_null
    ...
    for null_val in null_cases:
        ...
        assert result is None, f"Expected None, got {result} for input {repr(null_val)}"
...
def test_null_patterns_comprehensive():
    ...
    for pattern in expected_patterns:
        assert pattern in NULL_PATTERNS, f"Missing null pattern: {repr(pattern)}"
...
def test_real_world_scenarios():
    ...
    for input_val, expected in chembl_scenarios:
        result = normalize_null(input_val)
        assert result == expected, f"Scenario failed: {repr(input_val)} -> expected {expected}, got {result}"
