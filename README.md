...
[![Version](https://img.shields.io/badge/version-6.1.4-blue)](CHANGELOG.md)
...
- Keeps Codex npm/uv runtime caches under the native Linux user cache (`~/.cache/bioetl-mcp`, or `$XDG_CACHE_HOME/bioetl-mcp`) so WSL package installs do not rely on atomic rename/cleanup operations on `/mnt/*`; tracked MCP manifests remain repo-relative.
