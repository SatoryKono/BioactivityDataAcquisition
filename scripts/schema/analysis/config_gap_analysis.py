...
Usage:
    ...
    python -m scripts.schema analyze-gaps -o docs/audits/config_gaps.md
"""
...
def _validate_contract_alignment(
    ...
    if contract_pk is None:
        ...
        and sorted(str(value) for value in business_keys)
        != sorted(str(value) for value in contract_pk)
    ):
        medium.append(
...
def _validate_inline_dq_thresholds(
    ...
    for dq_parent_name, dq_parent in (
        ...
        if "soft_fail_threshold" in dq or "hard_fail_threshold" in dq:
            medium.append(
                f"Inline dq_overrides thresholds in {dq_parent_name} "
                "(deprecated per ADR-027)"
            )
    return [], medium, []
...
def analyze_standard_config(
    ...
    validators = (
        ...
        lambda config: _validate_recommended_metadata(config, top),
        lambda config: _validate_contract_alignment(config, top),
        _validate_sink_path_suffixes,
        lambda config: _validate_inline_dq_thresholds(config, top),
        lambda _config: _validate_gold_filters(top),
    )
...
def analyze_config(config_path: Path) -> ConfigGaps:
    """Analyze one config for ADR compliance."""
...
def _append_gap_section(
    ...
    *,
    ...
    for gaps in gaps_list:
        relative_path = _relative_config_path(gaps.path)
        lines.append(f"### `{relative_path}`{_config_type_suffix(gaps)}")
...
def generate_report(all_gaps: list[ConfigGaps]) -> str:
    ...
    standard_gaps = [gaps for gaps in all_gaps if not gaps.is_composite]
    composite_gaps = [gaps for gaps in all_gaps if gaps.is_composite]
    lines = [
        ...
        f"| With critical issues | {sum(1 for gaps in all_gaps if gaps.critical)} |",
        f"| With medium issues | {sum(1 for gaps in all_gaps if gaps.medium)} |",
        f"| With low issues | {sum(1 for gaps in all_gaps if gaps.low)} |",
        f"| Clean (no issues) | {sum(1 for gaps in all_gaps if not gaps.total)} |",
        "",
        "### Issue Counts by Severity",
        "",
        "| Severity | Total Issues |",
        "|----------|-------------|",
        f"| Critical (MUST fix) | {sum(len(gaps.critical) for gaps in all_gaps)} |",
        f"| Medium (SHOULD fix) | {sum(len(gaps.medium) for gaps in all_gaps)} |",
        f"| Low (MAY fix) | {sum(len(gaps.low) for gaps in all_gaps)} |",
        "",
    ]
    _append_gap_section(
        lines,
        heading="## Critical Issues (MUST fix)",
        gaps_list=[gaps for gaps in all_gaps if gaps.critical],
        attribute="critical",
        marker="[CRIT]",
    ...
    _append_gap_section(
        ...
        gaps_list=[gaps for gaps in all_gaps if gaps.medium],
        attribute="medium",
        marker="[WARN]",
    )
    _append_gap_section(
        ...
        gaps_list=[gaps for gaps in all_gaps if gaps.low],
        attribute="low",
        marker="[INFO]",
    )
    lines.extend(
        [
            ...
            "2. Ensure `contracts.primary_key` exists and is aligned with "
            "`pipeline.business_primary_keys`",
            "3. Add missing unified sections (`pipeline`, `schema`, `quality`, "
            "`filters`, `contracts`)",
            "",
            "### Priority 1 (Medium - Should Fix)",
            "1. Add `pipeline.description` and semver `version` where missing",
            "2. Migrate inline `dq_overrides` thresholds into the unified `quality` "
            "hierarchy when possible (ADR-027)",
...
def _print_verbose_config_status(config_path: Path, gaps: ConfigGaps) -> None:
    """Print one verbose config status line."""
    relative_path = _relative_config_path(config_path)
    status = _status_for_gaps(gaps)
    print(
        f"{status} {relative_path}: {len(gaps.critical)} critical, "
...
def main(argv: list[str] | None = None) -> int:
    """Run config gap analysis and return non-zero for critical findings."""
    ...
    for config_path in _iter_config_files():
        if config_path.name.startswith("_"):
            continue
        gaps = analyze_config(config_path)
        all_gaps.append(gaps)
        if args.verbose:
            _print_verbose_config_status(config_path, gaps)
    ...
    critical_count = sum(len(gaps.critical) for gaps in all_gaps)
    medium_count = sum(len(gaps.medium) for gaps in all_gaps)
    low_count = sum(len(gaps.low) for gaps in all_gaps)
    print(f"\n{'=' * 60}")
    print(
        f"Summary: {critical_count} critical, {medium_count} medium, "
        f"{low_count} low issues"
    )
    print(f"{'=' * 60}")
