...
def main():
    ...
    parser.add_argument(
        "pattern", nargs="?", help="Grok pattern or predefined pattern name"
    )
    parser.add_argument("text", nargs="?", help="Text to parse")
    parser.add_argument(
        "--list-patterns", action="store_true", help="Show predefined patterns"
    )
    parser.add_argument("--custom-patterns", help="Path to custom patterns JSON file")
    parser.add_argument(
        "--output", choices=["json", "pretty"], default="pretty", help="Output format"
    )
