...
if [[ -n "$_WIN_HOST_IP" ]]; then
  export http_proxy="http://${_WIN_HOST_IP}:3128"   # NOSONAR - local WSL host proxy
  export https_proxy="http://${_WIN_HOST_IP}:3128"  # NOSONAR - local WSL host proxy
...
# NOSONAR - local WSL host proxy construction only
