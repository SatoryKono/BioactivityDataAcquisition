...
def _load_budget(policy_path: Path) -> dict[str, Any]:
    ...
    def walk(obj: Any) -> dict[str, Any] | None:
        if isinstance(obj, dict):
            if "corpus_size_budget" in obj and isinstance(
                obj["corpus_size_budget"], dict
            ):
...
def build_report(measure: dict[str, Any], budget: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "budget": {
            "soft_max_total_bytes": soft_bytes,
            "soft_max_file_count": soft_files,
            "gc_policy": budget.get("gc_policy", {}),
        },
        "measured": measure,
        "status": {
            "within_soft_byte_budget": total_bytes <= soft_bytes,
            "within_soft_file_budget": total_files <= soft_files,
            "byte_utilization": round(total_bytes / soft_bytes, 4)
            if soft_bytes
            else None,
        },
...
def render_md(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        + str(m["total_files"])
        + "** (soft max "
        + str(report["budget"]["soft_max_file_count"])
        + ")",
        "- Total bytes: **"
        + str(m["total_bytes"])
        + "** (soft max "
        + str(report["budget"]["soft_max_total_bytes"])
        + ")",
        "- Within soft budgets: bytes="
        + str(report["status"]["within_soft_byte_budget"])
        + " files="
        + str(report["status"]["within_soft_file_budget"]),
        "",
        "## Per provider",
        "",
