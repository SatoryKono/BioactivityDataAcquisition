...
class _AssertionReachabilityAnalyzer:
    ...
    def analyze(
        ...
        if False in final_states and not self.findings and not (
            self.continue_before_assertion_lines
        ):
            self.findings.append(
                (function.lineno, "conditional_path_without_assertion")
            )
    ...
    def _statement(
        ...
        *,
        ...
        if isinstance(statement, (ast.For, ast.AsyncFor, ast.While)):
            # Model one representative iteration. Empty-input semantics are a
            # fixture/data contract; this analyzer owns assertion bypass inside
            # entered loops (especially ``continue``), not collection cardinality.
            continues_before = set(self.continue_before_assertion_lines)
            body_states = self._block(statement.body, states, in_loop=True)
            has_continue_path = bool(
                self.continue_before_assertion_lines - continues_before
            )
            loop_exit = body_states or states
            if has_continue_path:
                loop_exit |= states
