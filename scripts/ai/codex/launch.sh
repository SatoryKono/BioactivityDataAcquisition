#!/usr/bin/env bash
# Canonical Codex launcher for WSL/Linux.
...
print_help() {
    cat <<'EOF'
Codex CLI Launcher for WSL/Linux

Usage: launch.sh [options] [prompt]
...
Examples:
  bash scripts/ai/codex/launch.sh
  bash scripts/ai/codex/launch.sh "analyze the pipeline structure"
  bash scripts/ai/codex/launch.sh --update "refactor the parser"
EOF
}
