...
def _github_server(root: Path) -> dict[str, object]:
    if os.name != "nt":
        wrapper_path = str(
            (root / "scripts" / "ai" / "mcp" / "github-mcp-wrapper.sh").resolve()
        )
        return {
            "command": "bash",
            "args": [wrapper_path],
    ...
    wrapper_path = str(
        (root / "scripts" / "ai" / "mcp" / "github-mcp-wrapper.ps1").resolve()
    )
    return {
        "command": "powershell",
        "args": [
...
def _core_servers(root: Path) -> dict[str, dict[str, object]]:
    ...
    return {
        ...
        "docker-docs": _wrapper_server(
            root, "scripts/ai/mcp/mcp_docker_docs_wrapper.sh"
        ),
        "context7": _wrapper_server(root, "scripts/ai/mcp/mcp_context7_wrapper.sh"),
        "paper-search": _wrapper_server(
            root, "scripts/ai/mcp/mcp_paper_search_wrapper.sh"
        ),
        "dockerhub": _wrapper_server(root, "scripts/ai/mcp/mcp_dockerhub_wrapper.sh"),
        "prometheus": _wrapper_server(
            root, "scripts/ai/mcp/mcp_prometheus_wrapper.sh"
        ),
        "grafana": _wrapper_server(root, "scripts/ai/mcp/mcp_grafana_wrapper.sh"),
        "brave-search": _wrapper_server(
            root, "scripts/ai/mcp/mcp_brave_search_wrapper.sh"
        ),
        "sonarqube": _wrapper_server(
            root, "scripts/ai/mcp/mcp_sonarqube_wrapper.sh"
        ),
        "neo4j-cypher": _wrapper_server(
            root, "scripts/ai/mcp/mcp_neo4j_cypher_wrapper.sh"
        ),
        "neo4j-memory": _wrapper_server(
