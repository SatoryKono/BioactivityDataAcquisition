...
def _check_registry_entry_identity(
    ...
    if entry.status not in STATUS_ENUM:
        report.add_error(
            "status_enum", f"invalid status {entry.status!r} for {entry.id}", entry.path
        )
    if entry.class_ not in CLASS_ENUM:
        report.add_error(
            "class_enum", f"invalid class {entry.class_!r} for {entry.id}", entry.path
        )


def _load_registered_card(
    report: CheckReport, entry: RegistryEntry
) -> PromptCard | None:
...
def _check_registered_card(
    ...
    for ssot in card.related_ssot:
        if not _ssot_exists(ssot):
            report.add_warning(
                "ssot_missing", f"related_ssot path not found: {ssot}", entry.path
            )
...
def _record_body_hash(body_hashes: dict[str, list[str]], card: PromptCard) -> None:
...
def _report_duplicate_bodies(
    report: CheckReport, body_hashes: dict[str, list[str]]
) -> None:
...
def check_hygiene(*, registry_path: Path | None = None) -> CheckReport:
    ...
    for entry in entries:
        ...
        if card is None:
            continue
        checked += 1
        _check_body_size(report, entry, card)
        _check_guardrails(report, entry, card)
        _check_hygiene_ssot(report, entry, card)
        _check_rules_dump(report, entry, card)
        _check_lifecycle(report, entry, card)
        _record_body_hash(body_hashes, card)
