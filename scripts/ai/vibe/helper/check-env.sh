...
if [[ -f "${ENV_FILE}" ]]; then
    if grep -qE "^(MISTRAL_API_KEY|VIBE_API_KEY)=" "${ENV_FILE}"; then
        if grep -q "your-api-key-here" "${ENV_FILE}"; then
            log_warn ".env.vibe exists but API key not configured"
            ALL_CHECKS=false
        else
            log_success ".env.vibe configured with API key"
        fi
    else
        log_warn ".env.vibe exists but missing MISTRAL_API_KEY"
        ALL_CHECKS=false
    fi
else
    log_warn ".env.vibe not found"
    ALL_CHECKS=false
fi
