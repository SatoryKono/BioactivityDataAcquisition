...
paths = [
    ...
    Path(
        "tests/fixtures/vcr/semanticscholar/TestSemanticScholarAdapterIntegration.test_fetch_with_query.yaml"
    ),
]
for p in paths:
    ...
    codes = Counter(
        i.get("response", {}).get("status", {}).get("code") for i in interactions
    )
