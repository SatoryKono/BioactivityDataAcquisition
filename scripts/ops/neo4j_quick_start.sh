...
if docker ps -a --format "table {{.Names}}" | grep -q "^bioetl-neo4j$"; then
  ...
  info "Launching: docker run -d --name bioetl-neo4j -p 7474:7474 -p 7687:7687 -e NEO4J_AUTH=neo4j/[REDACTED] neo4j:5.15-community"
...
printf "  ${BLUE}Password:${NC}  [REDACTED]\n"

printf "\n${GREEN}Next steps:${NC}\n"
printf "  1. Access Neo4j Browser at http://localhost:7474/browser/\n"
printf "  2. Run verification: ${BLUE}bash scripts/ops/check_neo4j_mcp.sh${NC}\n"
