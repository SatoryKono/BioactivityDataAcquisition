...
NEO4J_PASSWORD="${NEO4J_PASSWORD:-[REDACTED]}"
...
if [[ -n "${DOCKER_BIN}" ]] && "${DOCKER_BIN}" exec bioetl-neo4j cypher-shell -u neo4j -p [REDACTED] \
...
if [[ $status -eq 0 ]]; then
  ...
  printf "     (Username: neo4j | Password: [REDACTED])\n\n"
