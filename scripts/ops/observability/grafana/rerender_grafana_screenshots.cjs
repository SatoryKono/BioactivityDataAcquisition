    ...
    const config = {
        ...
        viewport: {
            ...
            height: Number.parseInt(
                process.env.GRAFANA_SCREENSHOT_HEIGHT || "2200",
                10,
            ),
        },
...
function classifyPanelTerminalEvidence(evidence) {
    ...
    const leadingError =
        /^(?:ERROR|QUERY ERROR|DATASOURCE ERROR|REQUEST ERROR)\b/i.test(bodyText);
    const leadingValidEmpty = /^(?:VALID EMPTY|EMPTY RESULT)\b/i.test(bodyText);
    const leadingNoMatch =
        /^(?:NO MATCHING(?: SCOPE| DATA| ROWS?)?|NOT APPLICABLE|N\/A)\b/i.test(
            bodyText,
        );
...
async function createAuthenticatedApiContext() {
    ...
    if (!response.ok()) {
        await api.dispose();
        throw new Error(
            `Grafana login failed: ${response.status()} ${response.statusText()}`,
        );
    }
    return api;
}
...
function listDashboardsFromRepo() {
        ...
        dashboards.push({
            ...
            collapsedRowTitles: CONFIG.expandCollapsedRows
                ? panels
                      .filter(
                          (panel) =>
                              panel && panel.type === "row" && panel.collapsed === true,
                      )
                      .map((panel) =>
    ...
    if (dashboards.length === 0) {
        throw new Error(
            "No local dashboard JSON files matched the current render selection",
        );
    }
...
async function tryExpandCollapsedRow(page, title, index, total, uid) {
    ...
    console.warn(
        `[${index}/${total}] could not expand collapsed row '${title}' in ${uid}`,
    );
    return false;
}
...
async function detectActualTheme(page) {
    return page.evaluate(() => {
        ...
        for (const element of [body, root]) {
            ...
            if (
                !match ||
                (match[4] !== undefined && Number.parseFloat(match[4]) === 0)
            ) {
                continue;
            }
    ...
    const states = await page.evaluate(
        ({ requiredPanels }) => {
                    ...
                    for (const headerSelector of headerSelectors) {
                        for (const header of document.querySelectorAll(
                            ...
                            if (container) {
                                return {
                                    element: container,
                                    selector: `${headerSelector} -> closest`,
                                };
                            }
                        }
                    }
            ...
            return requiredPanels.map((panel) => {
                ...
                const hasLoadingMarker = hasVisibleMarker(
                    surface,
                    loadingSelector,
                    true,
                );
...
async function validateDashboardTerminalStates(page, dashboard, index, total) {
    ...
    while (Date.now() <= deadline) {
        ...
        if (pending.length === 0) {
            const requiredTerminalStates = dashboard.requiredTerminalPanelIds.map(
                (panelId) => panelStates.find((state) => state.id === panelId),
            );
            if (
...
function describeTerminalStateFailure(dashboard) {
    ...
    const states = Array.isArray(validation.panelStates) ? validation.panelStates : [];
...
async function waitForDashboardContent(page, dashboard, index, total) {
    console.log(
        `[${index}/${total}] waiting for dashboard content ${dashboard.uid} ...`,
    );
    ...
    await page
        .waitForFunction(
            (readySelectors) => {
                ...
                return (
                    text.includes("Review Dashboard Navigation") ||
...
async function prepareDashboardForCapture(page, dashboard, index, total) {
    ...
    await page.waitForTimeout(
        Math.max(250, Math.min(1000, Math.floor(CONFIG.settleMs / 3))),
    );
...
async function materializeLazyPanels(page, dashboard, index, total) {
    const scrollDelayMs = Math.max(
        250,
        Math.min(1000, Math.floor(CONFIG.settleMs / 3)),
    );
    const step = Math.max(500, Math.floor(CONFIG.viewport.height * 0.75));
    let previousScrollHeight = 0;
    console.log(
        `[${index}/${total}] materializing lazy panels for ${dashboard.uid} ...`,
    );
    ...
    for (let pass = 1; pass <= 2; pass += 1) {
        ...
        const nextScrollHeight = Math.max(
            nextMetrics.scrollBottom,
            nextMetrics.panelBottom,
        );
        if (
    ...
    const payload = {
        ...
        actual: {
            viewports: Object.fromEntries(
                dashboards.map((dashboard) => [
                    dashboard.uid,
                    dashboard.actualViewport || null,
                ]),
            ),
            themes: Object.fromEntries(
                dashboards.map((dashboard) => [
                    dashboard.uid,
                    dashboard.actualTheme || "unknown",
                ]),
            ),
        },
        terminal_state_validation: {
            status:
                ...
                terminalStatuses.every((status) => status === "ok")
                    ? "ok"
                    : "error",
