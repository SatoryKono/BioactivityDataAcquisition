...
def apply_corrections(payload: dict) -> None:
    ...
    _correct_action_links(uid, panels)
    _correct_metric_panels(panels)

    _correct_scope_copy(uid, panels)

    _correct_incident(uid, panels)

    _correct_provider(uid, panels)

    _correct_retention(uid, panels)

    _correct_tables(uid, panels)
    _correct_trust(uid, panels)
    # A display-name alias must not reserve the same column's width twice.
    _deduplicate_widths(panels)
    _correct_runtime(uid, panels)
    _correct_dq(uid, panels)
    _correct_overview(uid, panels)
    _correct_run_explorer(uid, panels)

    _preserve_action_scope(uid, panels)


def _correct_action_links(uid: str | None, panels: dict) -> None:
...
def _correct_metric_panels(panels: dict) -> None:
    for panel in panels.values():
        _correct_metric_panel(panel)


def _correct_scope_copy(uid: str | None, panels: dict) -> None:
    scope_copy = {
        "bioetl-runtime": (
...
def _correct_incident(uid: str | None, panels: dict) -> None:
    if not (uid == "bioetl-incident-v1"):
        return
    for panel in panels.values():
...
def _correct_provider(uid: str | None, panels: dict) -> None:
    if not (uid == "bioetl-provider-health-v2"):
        return
    ...
    for panel_id in (9102, 9112):
        _correct_provider_fleet_panel(panels, panel_id, fleet, empty)
...
def _correct_retention(uid: str | None, panels: dict) -> None:
    if not (uid == "bioetl-control-plane-v1"):
        return
...
def _correct_tables(uid: str | None, panels: dict) -> None:
    targets = {
        "bioetl-dq-v2": (9102,),
    ...
    for panel_id in targets.get(uid, ()):
        _correct_table(uid, panels, panel_id)


def _correct_trust(uid: str | None, panels: dict) -> None:
    if not (uid == "bioetl-control-plane-v1" and 9418 in panels):
        return
    trust = panels[9418]
    ...
    _correct_trust_details(panels)
    _correct_trust_mapping_colors(trust)


def _deduplicate_widths(panels: dict) -> None:
    for panel in panels.values():
        _deduplicate_panel_widths(panel)


def _correct_runtime(uid: str | None, panels: dict) -> None:
    if not (uid == "bioetl-runtime" and 2460 in panels):
        return
...
def _correct_dq(uid: str | None, panels: dict) -> None:
    if not (uid == "bioetl-dq-v2" and 155 in panels):
        return
...
def _correct_overview(uid: str | None, panels: dict) -> None:
    if not (uid == "bioetl-overview-v2" and 215 in panels):
        return
    _override(panels[215], "Action", "custom.width", 150)


def _correct_run_explorer(uid: str | None, panels: dict) -> None:
    if not (uid == "bioetl-run-explorer-v1" and 3010 in panels):
        return
    _override(
        panels[3010],
        "Pipeline",
...
def _preserve_action_scope(uid: str | None, panels: dict) -> None:
    routed_panel_id = {
    ...
    if routed_panel_id in panels:
        ...
        for transform in routed_panel.get("transformations", []):
            _preserve_action_scope_transform(transform)
        _override(routed_panel, "action_scope", "custom.hidden", True)


def _correct_metric_panel(panel: dict) -> None:
    _correct_rolling_legend(panel)
    _correct_unknown_stat_color(panel)
    _correct_query_error_labels(panel)
    _correct_metric_targets(panel)


def _correct_provider_fleet_panel(
    panels: dict, panel_id: int, fleet: str, empty: str
) -> None:
    panel = panels[panel_id]
    ...
    _override(panel, "Severity" if panel_id == 9102 else "Status", "custom.width", 130)
    expr = f"{fleet} >= 1"
    if panel_id == 9102:
        expr = f"topk(4, {expr})"
...
def _correct_table(uid: str | None, panels: dict, panel_id: int) -> None:
    ...
    _correct_incident_table(uid, panel_id, panel)
    if uid == "bioetl-overview-v2" and panel_id == 9603:
        _override(panel, "Status", "custom.width", 130)
    _correct_run_index_table(uid, panel_id, panel, custom)
    _correct_overview_action_table(uid, panel_id, panel, panels)
    panel["options"]["cellHeight"] = "sm"


def _deduplicate_panel_widths(panel: dict) -> None:
    width_fields = set()
...
def _correct_rolling_legend(panel: dict) -> None:
...
def _correct_unknown_stat_color(panel: dict) -> None:
    if (
        panel.get("type") == "stat"
...
def _correct_query_error_labels(panel: dict) -> None:
    if (
        any(
...
def _correct_metric_targets(panel: dict) -> None:
    for target in panel.get("targets", []):
        _correct_metric_target(panel, target)


def _correct_incident_table(uid: str | None, panel_id: int, panel: dict) -> None:
    if not (uid == "bioetl-incident-v1" and panel_id in (2010, 22010)):
        return
    if panel_id == 2010:
...
def _correct_run_index_table(
    uid: str | None, panel_id: int, panel: dict, custom: dict
) -> None:
...
def _correct_overview_action_table(
    uid: str | None, panel_id: int, panel: dict, panels: dict
) -> None:
    if uid == "bioetl-overview-v2" and panel_id == 215:
        panel["gridPos"]["h"] = 6
...
def _correct_trust_details(panels: dict) -> None:
    details = panels[9451]
...
def _correct_trust_mapping_colors(trust: dict) -> None:
    for item in trust["fieldConfig"]["overrides"]:
        _correct_trust_override_colors(item)


def _correct_metric_target(panel: dict, target: dict) -> None:
...
def _preserve_action_scope_transform(transform: dict) -> None:
...
def _correct_trust_override_colors(item: dict) -> None:
    for prop in item["properties"]:
        _correct_trust_property_colors(prop)


def _correct_trust_property_colors(prop: dict) -> None:
    if prop["id"] == "mappings":
