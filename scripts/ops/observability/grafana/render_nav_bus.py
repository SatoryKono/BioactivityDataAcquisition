...
def nav_link_tooltip(*, source_uid: str, target: dict[str, str]) -> str:
    ...
    if not resets:
        if "pipeline context" in preserved:
            return f"{base}. {_PRESERVE_SCOPE_TOOLTIP} Preserves pipeline context."
            ...
            "transformations": [
                {
                    ...
                    "options": {
                        "include": {"names": ["verdict", "blockers", "unknown_checks"]}
                    },
                },
                {
                    "id": "organize",
...
def _stamp_trust_operator_surfaces(panels: list[object]) -> None:
    ...
    for panel_id in (9405, 9406, 9407, 9408, 9409, 9413, 9414, 9415, 9416, 9417):
        ...
        description = description.replace(
            "TIME RANGE · SELECTED RUN ", "SELECTED RUN · "
        )
...
def _retain_dq_selected_run_panels(payload: dict[str, object]) -> None:
    ...
    for panel_id in (9402, 9403):
        ...
        if not isinstance(footer, dict):
            raise SystemExit(f"bioetl-dq-v2: panel {panel_id} footer must be an object")
        footer["enablePagination"] = True
