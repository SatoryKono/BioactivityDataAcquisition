...
# Usage:
#   ./start-shared.sh              # daily servers only (daily!=false)
#   ./start-shared.sh --all        # full catalog including optional
#   ./start-shared.sh grafana brave-search
set -euo pipefail
...
# uv on WSL→Windows mounts often fails atomic rename; keep caches on native Linux FS.
...
python3 - <<'PY' "$CATALOG" "$ROOT" "$PID_DIR" "$PROXY_PKG" "$SETTLE" "$DOCKER_SETTLE" "$MAX_ATTEMPTS" "$NPM_CACHE" "$MODE" "${SELECTED[@]}"
...
DOCKER_HEAVY = frozenset(
    {"brave-search", "prometheus", "grafana", "docker", "mermaid", "dockerhub", "github", "fetch"}
)
...
if selected:
    ...
    items = [(n, e) for n, e in items if e.get("daily", True) is not False]
...
# Note optional (non-daily) servers that were not started
if mode == "daily" and not selected:
    for name, entry in catalog["servers"].items():
        if entry.get("daily", True) is False and name not in status["servers"]:
            status["servers"][name] = {
                "port": int(entry["port"]),
            ...
            skipped += 1
            print(f"skip optional (daily=false) {name}")

status["failed"] = failed
status["skipped_optional"] = skipped
status_path = root / "logs/mcp-shared/status.json"
status_path.write_text(json.dumps(status, indent=2) + "\n", encoding="utf-8")
print(f"Wrote {status_path} failed={failed} skipped_optional={skipped}")
