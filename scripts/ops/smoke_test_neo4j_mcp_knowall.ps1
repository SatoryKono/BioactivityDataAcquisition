...
$NEO4J_PASSWORD = $env:NEO4J_PASSWORD ? $env:NEO4J_PASSWORD : "[REDACTED]"
...
try {
    $result = docker exec bioetl-neo4j cypher-shell -u neo4j -p [REDACTED] `
...
if ($status -eq 0) {
    ...
    Write-Host "     (Username: neo4j | Password: [REDACTED])" -ForegroundColor Green
    Write-Host ""
