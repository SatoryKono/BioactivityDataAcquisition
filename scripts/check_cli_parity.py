        ...
        if stripped.startswith('_LAZY_COMMAND_SPECS:'):
            ...
            if '"' in line and ':' in line:
                ...
                if len(parts) >= 2:
                    ...
                    if (command_name.replace('-', '').isalpha() and 
                        command_name.islower() and 
                        not command_name.startswith('__') and
                        command_name not in ['name', 'Commands']):
...
def main() -> int:
    ...
    try:
        ...
        if missing_commands:
            print("💡 Recommendation: Update CLI documentation to include missing commands")
            return 1
        else:
