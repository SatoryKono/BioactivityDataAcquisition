...
def check_adapter_implements_port(filepath: Path) -> list[str]:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.ImportFrom) and node.module and ("domain.ports" in node.module or "domain.types" in node.module):
            has_port_import = True
            break

    # Adapters should reference domain types
    adapter_dirs = ["adapters", "storage", "locking", "checkpoint", "quarantine"]
    if any(d in str(filepath) for d in adapter_dirs) and (not has_port_import and "client.py" in filepath.name):
