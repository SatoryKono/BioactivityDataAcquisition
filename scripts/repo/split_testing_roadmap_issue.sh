...
if [[ "${1:-}" == "--help" || "${1:-}" == "-h" ]]; then
  cat <<'EOF'
Usage:
  bash scripts/repo/split_testing_roadmap_issue.sh
  bash scripts/repo/split_testing_roadmap_issue.sh --json
  GITHUB_PERSONAL_ACCESS_TOKEN=... bash scripts/repo/split_testing_roadmap_issue.sh --apply
...
Notes:
  ...
  - All arguments are forwarded to scripts/repo/split_testing_roadmap_issue.py.
EOF
  exit 0
fi
