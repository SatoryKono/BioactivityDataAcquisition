...
if [[ "${1:-}" == "--help" || "${1:-}" == "-h" ]]; then
  cat <<'EOF'
Usage:
  bash scripts/repo/sync_docs_issues.sh
  bash scripts/repo/sync_docs_issues.sh --json
  GITHUB_PERSONAL_ACCESS_TOKEN=... bash scripts/repo/sync_docs_issues.sh --apply --skip-milestone
  GITHUB_PERSONAL_ACCESS_TOKEN=... bash scripts/repo/sync_docs_issues.sh --apply --create-milestone
...
Notes:
  ...
  - All arguments are forwarded to scripts/repo/sync_docs_issues.py.
EOF
  exit 0
fi
