...
class GovernanceCheckResult:
    ...
    def get_score(self) -> float:
        ...
        return (
            (self.checks_passed / self.checks_run * 100)
...
class DocumentationGovernanceChecker:
    ...
    def __init__(self):
        ...
        self.governance_rules = {
            ...
            "terminology_consistency": {"threshold": 98.0, "weight": 0.2},
            "link_validity": {"threshold": 99.0, "weight": 0.2},
            "metadata_completeness": {"threshold": 95.0, "weight": 0.15},
            "structure_compliance": {"threshold": 98.0, "weight": 0.15},
        }
            ...
            if report_file.exists():
                ...
                if status == "pass":
                    checks_passed.append(
                        f"Documentation parity check passed ({parity_percentage:.1f}%)"
                    )
                    return (True, warnings, checks_passed)
                elif status == "warning":
                    warnings.append(
                        f"Documentation parity warning ({parity_percentage:.1f}%)"
                    )
                    return (True, warnings, checks_passed)
                else:
                    checks_failed.append(
                        f"Documentation parity check failed ({parity_percentage:.1f}%)"
                    )
                    return (False, warnings, checks_failed)
            else:
    ...
    def check_terminology_consistency(self) -> tuple[bool, list[str], list[str]]:
            ...
            if doc_file.exists():
                ...
                for legacy_term, canonical_term in canonical_terms.items():
                    if legacy_term in content.lower():
                        checks_failed.append(
                            f"Legacy term '{legacy_term}' found in {doc_file}"
                        )
        ...
        return (
            not legacy_term_found,
            warnings,
            checks_passed if not legacy_term_found else checks_failed,
        )
    ...
    def check_link_validity(self) -> tuple[bool, list[str], list[str]]:
        ...
        return (
            not broken_links_found,
            warnings,
            checks_passed if not broken_links_found else checks_failed,
        )
    ...
    def check_metadata_completeness(self) -> tuple[bool, list[str], list[str]]:
        ...
        return (
            ...
            warnings,
            checks_passed if not missing_metadata else checks_failed,
        )
    ...
    def generate_report(self, result: GovernanceCheckResult) -> str:
        ...
        report.append(
            f"**Overall Score**: {result.get_score():.1f}/100 ({result.get_grade()})"
        )
        report.append("")
