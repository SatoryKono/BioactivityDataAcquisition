...
| `check-drift`                            | After renaming classes, moving modules, changing ports, updating active runtime docs, or editing high-traffic narrative docs such as `README.md` and workflow guides; detects doc/code desync, bounded narrative regressions, runtime mirror drift, freshness conflicts, and AI-surface policy regressions | CI gate (`docs.yml` `python -m scripts.docs verify`; weekly `docs-kpi-weekly.yml` for freshness) |
