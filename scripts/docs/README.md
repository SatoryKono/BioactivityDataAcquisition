...
| ---------------------------------------- | --------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| `verify`                                 | `scripts/docs/checks/verify.py`                                 | Run the canonical docs verification chain (links, ports/classes/runtime-mirrors/freshness drift, docstrings, cleanup inventory, strict build) |
| `build-site`                             | `scripts/docs/build/mkdocs_build.py`                            | Build the MkDocs site through the packaged Python entrypoint                                       |
| `check-links`                            | `scripts/docs/checks/check_links.py`                            | Check documentation links, specs, configs, workflow/provider inventory parity, and mkdocs nav classification |
| `check-drift`                            | `scripts/docs/checks/check_drift.py`                            | Check documentation drift (ports, classes, bounded narrative parity, runtime mirrors, freshness, AI runtime surfaces) |
...
| `generate-docs-export`                   | `scripts/docs/build/generate_docs_export.py`                    | Generate the merged documentation export artifact from the tracked input manifest                   |
| `passports`                              | `scripts/docs/passports/cli.py`                                 | Generate/check pipeline and workflow passport projections (CI: `python -m scripts.docs passports check`) |
...
| ---------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------- |
| `check-links`                            | After editing docs; validates internal links, spec files, contracts, workflow/provider inventory parity, legacy path guardrails, and ensures each local skill mirror page is explicitly classified in `mkdocs.yml` via `nav` or `not_in_nav` | CI gate (`docs.yml`, every PR) |
...
| `build-site`                             | When you need a deterministic local or CI MkDocs site build without depending on ad-hoc shell wiring. Defaults to `mkdocs build --strict`; pass `--no-strict` to opt out.                                  | Manual, pre-merge, or CI-style local smoke                                 |
| `check-drift`                            | After renaming classes, moving modules, changing ports, updating active runtime docs, or editing high-traffic narrative docs such as `README.md` and workflow guides; detects doc/code desync, bounded narrative regressions, runtime mirror drift, freshness conflicts, and AI-surface policy regressions | CI gate (`docs.yml` `python -m scripts.docs verify`; weekly `docs-kpi-weekly.yml` for `--runtime-mirrors --freshness`) |
| `check-docstrings`                       | After adding new modules/classes/functions; enforces coverage thresholds (modules 100%, classes 95%, functions 90%)                                                                                        | CI gate (`docs.yml` `python -m scripts.docs verify`)                       |
...
| `generate-cleanup-inventory`             | Before documentation cleanup/archive/delete work; derives status, inbound links, duplicates, diagram family, generated route, and recommended action from tracked repo state                               | Manual and CI-friendly via `--check`                                       |
| `generate-docs-export`                   | When producing the merged documentation export from the tracked input manifest                                                                                                                             | Manual                                                                     |
| `passports`                              | After changing pipeline/workflow facts that feed generated passport projections; CI drift guard for executable passports                                                                                    | CI gate (`docs.yml`: `python -m scripts.docs passports check`)             |
