...
DEPRECATED_PALETTE = {
    ...
    "#0d47a1",
    "#b71c1c",
    # NOTE: #f59e0b is canonical per ADR-040 for Composition/Bronze stroke — NOT deprecated.
    "#00838f",
    "#4e342e",
    "#546e7a",
    "#90a4ae",
...
def check_node_count_policy(path: Path, lines: list[str]) -> list[Issue]:
    ...
    if path.name.startswith("00-legend"):
        return issues
    # Diagrams annotated %% @reference are parent/full references with
    # decomposed sub-diagrams — exempt from SIZE checks (ADR-040 D6).
    if any(ln.strip().startswith("%% @reference") for ln in lines):
        return issues
...
def check_nbsp_padding(path: Path, lines: list[str]) -> list[Issue]:
    ...
    Files with ``%% @uniform`` are exempt — their &nbsp; padding is
    generated by ``uniform_diagram_sizes.py`` for consistent node sizing.
    """
    issues: list[Issue] = []
    fname = str(path)
    # Exempt @uniform-managed files — padding is auto-generated.
    if any(ln.strip().startswith("%% @uniform") for ln in lines):
        return issues
