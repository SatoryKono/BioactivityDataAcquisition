    ...
    def _check_import(self, module, line):
        ...
        if "structlog" in module and self.sector_id not in ("S3", "S6"):
            self.add_issue(
                ...
                "AP-002",
                "HIGH",
                "Direct structlog import outside infrastructure",
                line,
            )
        ...
        if module.startswith("bioetl."):
            ...
            if len(parts) > 1:
                layer = parts[1]
                if self.sector_id == "S1" and layer in [
                    "application",
                    "infrastructure",
                    "composition",
                    "interfaces",
                    ...
                    self.add_issue(
                        ...
                        "ARCH-001",
                        "CRITICAL",
                        f"Domain layer imports {layer}",
                        line,
                    )
                if self.sector_id == "S2" and layer in [
                    "infrastructure",
                    "composition",
                    "interfaces",
                ]:
                    self.add_issue(
                        ...
                        "ARCH-001",
                        "CRITICAL",
                        f"Application layer imports {layer}",
                        line,
                    )
                if self.sector_id == "S3" and layer in [
                    "application",
                    "composition",
                    "interfaces",
                ]:
                    self.add_issue(
                        ...
                        "ARCH-001",
                        "CRITICAL",
                        f"Infrastructure layer imports {layer}",
                        line,
                    )
        ...
        if not node.name.startswith("_"):
            if not node.returns:
                self.add_issue(
                    ...
                    "TYPE-001",
                    "LOW",
                    f"Missing return type for {node.name}",
                    node.lineno,
                )
            if not getattr(node, "args", None) or not all(
                arg.annotation
                ...
                self.add_issue(
                    ...
                    "TYPE-002",
                    "LOW",
                    f"Missing parameter types for {node.name}",
                    node.lineno,
                )
        self.generic_visit(node)
    ...
    def visit_Assign(self, node):
        for target in node.targets:
            if isinstance(target, ast.Name):
                if target.id.isupper() and any(
                    kw in target.id for kw in ["SECRET", "PASSWORD", "TOKEN", "KEY"]
                ):
                    if isinstance(node.value, ast.Constant) and isinstance(
                        node.value.value, str
                    ):
                        if (
                            len(node.value.value) > 4
...
def format_report(sector, files, total_loc, issues):
    ...
    if crit:
        ...
        for i in crit:
            report += (
                f"- {i['rule']} in `{i['file']}:{i['line']}`: {i['description']}\n"
            )
...
def main():
    ...
    for sector in sectors_config:
        ...
        Path(
            f"reports/review/{sector['id']}_{sector['name'].replace(' ', '_')}.md"
        ).write_text(report, encoding="utf-8")
        summary_lines.append(
            f"| {sector['id']} {sector['name']} | {len(files)} | {loc} | {score:.1f} | {status} |"
        )
