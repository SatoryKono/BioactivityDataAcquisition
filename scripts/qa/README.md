...
| `check-architecture`          | When reproducing or migrating the legacy infrastructure architecture check without calling `src/tools/scripts/check_architecture.py` directly     | Manual, Makefile migration path            |
| `check-app-deps`              | When reproducing or migrating the legacy application dependency check without calling `src/tools/scripts/check_application_deps.py` directly      | Manual, Makefile migration path            |
| `check-constructor-args`      | When reproducing or migrating the legacy constructor-args audit without calling `src/tools/scripts/check_constructor_args.py` directly            | CI migration path, manual use              |
...
| `analyze-duplicate-functions` | When you need the older duplicate-function AST report without calling `src/tools/scripts/duplicate_function_analyzer.py` directly                | Manual, audit/reporting                    |
...
`src/tools/scripts/check_architecture.py`, `check_application_deps.py`, and
`check_constructor_args.py` remain supported as legacy direct paths during the
migration window, but new integrations should use the grouped QA commands above.
