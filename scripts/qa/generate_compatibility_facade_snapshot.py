...
def render_snapshot(
    ...
    *,
    ...
    import_violation_modules = sorted(measured_only_import_violations)
    ...
    sections = [
        ...
        f"- First-party src imports targeting measured-only modules: `{len(import_violation_modules)}`",
    ...
    if args.check:
        ...
        if unexpected or missing or measured_only_import_violations or ratchet_violations:
            ...
            if measured_only_import_violations:
                print("[drift] first-party src imports measured-only modules:")
                for module, importers in sorted(measured_only_import_violations.items()):
    ...
    if unexpected or missing or measured_only_import_violations or ratchet_violations:
        ...
        if missing:
            ...
            for path in sorted(missing):
                print(f"  - {path}")
        if measured_only_import_violations:
            print("[warn] first-party src imports measured-only modules:")
            for module, importers in sorted(measured_only_import_violations.items()):
                joined = ", ".join(importers)
                print(f"  - {module}: {joined}")
        if ratchet_violations:
            print("[warn] measured-only ratchet violations:")
            for violation in ratchet_violations:
                print(f"  - {violation}")
        return 1
    return 0
