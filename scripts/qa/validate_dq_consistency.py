...
class DQConsistencyValidator:
    ...
    def validate_config_contract_references(self, config_path: Path) -> bool:
        ...
        try:
            ...
            if has_contract_ref or has_contract_version or has_rule_bundle:
                ...
                if not isinstance(config_data["contract_ref"], str) or not config_data["contract_ref"]:
                    self.issues.append(
    ...
    def validate_provenance_consistency(
        ...
        if "dq_rule_provenance" not in metadata or metadata["dq_rule_provenance"] is None:
        ...
        for i, entry in enumerate(provenance_entries):
            if not isinstance(entry, dict):
                self.issues.append(
                    f"Provenance entry {i} is not a dict: {type(entry)}"
                )
                continue
    ...
    def validate_disposition_determinism(
        ...
        if outcome1 != outcome2:
            self.issues.append(
                f"Non-deterministic outcome for {rule_id}"
            )
            return False
    ...
    def run_all_checks(self) -> bool:
        ...
        for config_dir in config_paths:
            if config_dir.exists():
                for config_file in config_dir.glob("*dq*.yaml") or config_dir.glob("*dq*.json"):
