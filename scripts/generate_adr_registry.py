...
class ADRRegistryGenerator:
    ...
    def extract_adr_number(self, file_path: Path) -> Optional[str]:
    ...
    def extract_adr_relationships(self, content: str) -> Dict[str, List[str]]:
        ...
        supersedes_match = re.search(r'supersedes:\s*([ADR-\d+,\s]+)', content, re.IGNORECASE)
        if supersedes_match:
            relationships['supersedes'] = [adr.strip() for adr in supersedes_match.group(1).split(',')]
        
        superseded_match = re.search(r'superseded\s+by:\s*([ADR-\d+,\s]+)', content, re.IGNORECASE)
        if superseded_match:
            relationships['superseded_by'] = [adr.strip() for adr in superseded_match.group(1).split(',')]
        
        related_match = re.search(r'related:\s*([ADR-\d+,\s]+)', content, re.IGNORECASE)
        if related_match:
            relationships['related'] = [adr.strip() for adr in related_match.group(1).split(',')]
    ...
    def extract_adr_metadata(self, file_path: Path) -> Optional[ADRMetadata]:
        ...
        try:
            ...
            adr_metadata = ADRMetadata(
                ...
                title=front_matter.get('title', file_path.stem.replace(f'ADR-{adr_number}-', '').replace('-', ' ')),
    ...
    def generate_registry_index(self) -> str:
        ...
        lines.append("")
        lines.append("This registry provides a comprehensive index of all Architecture Decision Records")
        ...
        for status in status_order:
            if status in status_groups:
                ...
                lines.append(f"## {status_icon} {status.capitalize()} ADRs")
                lines.append("")
                ...
                for adr in adrs:
                    ...
                    lines.append("")
                    lines.append(f"**Status**: `{adr.status}` | **Category**: `{adr.category}` | **Owner**: `{adr.owner}`")
                    lines.append("")
                    ...
                    lines.append(f"[📄 View Full ADR]({doc_path}) | [🔗 Permalink](#adr-{adr.adr_number})")
                    lines.append("")
                    lines.append("---")
                    lines.append("")
        ...
        lines.append("- **Deprecated ADRs**: No longer recommended but may still be in use")
    ...
    def generate_status_dashboard(self) -> str:
        ...
        lines.append(f"- **Active ADRs**: {active_count}/{total_count} ({active_percentage:.1f}%)")
        lines.append(f"- **Maintenance Ratio**: {active_count}:{total_count - active_count}")
        lines.append(f"- **Average ADRs/Year**: {total_count / 3:.1f} (assuming 3-year project)")
...
def main():
    ...
    print(f"   - Created index, dashboard, and JSON registry")
    print("")
    print("🎯 Next steps:")
