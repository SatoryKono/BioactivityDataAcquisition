...
# Extract actual failing tests from /tmp/test_results.txt
failed_tests = []
if os.path.exists('/tmp/test_results.txt'):
    with open('/tmp/test_results.txt', 'r') as f:
        content = f.read()
        failed_lines = re.findall(r'^FAILED (.+)$', content, re.MULTILINE)
        failed_tests.extend(failed_lines)

if failed_tests:
    final_report = final_report.replace("26 failing tests", f"{len(failed_tests)} failing tests")
    final_report = final_report.replace("| Failed | 26 | 26 | 0 | ❌ |", f"| Failed | {len(failed_tests)} | {len(failed_tests)} | 0 | ❌ |")
    final_report = final_report.replace("Deterministic failures | 26 |", f"Deterministic failures | {len(failed_tests)} |")

    # Add them to the P1 backlog
    backlog_list = "\n".join([f"1. `{test}`" for test in failed_tests])
    final_report = final_report.replace("1. Fix 26 failing tests across integration, architecture, and unit layers.", backlog_list)

with open("reports/test-swarm/SWARM-001/FINAL-REPORT.md", "w") as f:
    f.write(final_report)
print("Updated FINAL-REPORT.md with realistic tests")
