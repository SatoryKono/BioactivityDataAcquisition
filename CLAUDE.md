...
4. `docs/refactoring-plan.md` — План рефакторинга с верифицированным статусом

> ⚠️ **ОБЯЗАТЕЛЬНО**: Перед предложением задач рефакторинга сверься с секцией
> "ВЕРИФИЦИРОВАННЫЙ СТАТУС РЕАЛИЗАЦИИ" в `docs/refactoring-plan.md`!
...
|-----|----------|---------|
| 1 | Проверить `refactoring-plan.md` | Секция "ЛОЖНЫЕ УТВЕРЖДЕНИЯ" |
...
- [ ] Проверено в `refactoring-plan.md` → "ЛОЖНЫЕ УТВЕРЖДЕНИЯ"
- [ ] Проверено в `refactoring-plan.md` → "УЖЕ РЕАЛИЗОВАНО"
...
| **Устаревшие знания** | "Не реализовано" (но уже реализовано) | Сверяться с `refactoring-plan.md` |
