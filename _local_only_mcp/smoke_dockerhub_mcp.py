"""One-shot Docker Hub MCP smoke: initialize, tools/list, namespace tool.

Uses DOCKER_API_KEY from .env (preferred) via mcp_dockerhub_wrapper.ps1.
Does not print secret values.

Exit codes:
  0  full success (tool content without transport/auth errors)
  2  initialize failed
  3  tools/list failed
  4  no namespace tool
  5  tools/call timeout
  6  tools/call MCP error
  7  tool returned error-like text content (often network to hub.docker.com)
"""
...
def main() -> int:
    ...
    if not wrapper.exists():
        print(f"FAIL missing wrapper {wrapper}")
        return 1
    ...
    try:
        ...
        if re.search(
            ...
            print(
                "RESULT tool_content_error "
                "(MCP connected; Hub API unreachable or token rejected)"
            )
            return 7
        print("RESULT ok")
        return 0
